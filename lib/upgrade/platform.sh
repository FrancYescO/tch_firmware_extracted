...
get_flash_partition_mmc() {
	local bank=$(platform_streaming_bank)
	if [ $bank == "bank_2" ]; then
		kernel_block=/dev/`cat /proc/partitions | grep bank_2 -m 1 |  cut -d' ' -f 16`
		kernel_blk_size=$(($(cat /proc/partitions | grep bank_2 -m 1 |  cut -d' ' -f 15) * 1024))
		fs_block=/dev/`cat /proc/partitions  | grep bank_2_root | cut -d' ' -f 16`
		fs_blk_size=$(($(cat /proc/partitions | grep bank_2_root |  cut -d' ' -f 15) * 1024))
	else
		kernel_block=/dev/`cat /proc/partitions | grep bank_1 -m 1 |  cut -d' ' -f 16`
		kernel_blk_size=$(($(cat /proc/partitions | grep bank_1 -m 1 |  cut -d' ' -f 15) * 1024))
		fs_block=/dev/`cat /proc/partitions  | grep bank_1_root | cut -d' ' -f 16`
		fs_blk_size=$(($(cat /proc/partitions | grep bank_1_root |  cut -d' ' -f 15) * 1024))
	fi
}

get_flash_type() {
	local block=$(cat /proc/partitions | grep "mtd" -m 1 | sed 's/.*\(mtd\).*/\1/')
	if [ -z $block ]; then
		block=$(cat /proc/partitions | grep "mmc" -m 1 | sed 's/.*\(mmc\).*/\1/')
		if [ -z $block ]; then
			show_error 13 "Could not find mtd/mmc in /proc/partitions"
			return 1
		fi
	fi
	echo $block 2>/dev/null
}

platform_is_dualbank() {
	local block=$(get_flash_type)
	if [ $block == "mtd" ]; then
		grep bank_2 /proc/mtd >/dev/null
		return $?
	elif [ $block == "mmc" ]; then
		grep bank_2 /proc/partitions >/dev/null
		return $?
	fi
}
...
platform_check_image_imp() {
	...
	if [ $block == "mmc" ]; then
		# Create file for storing extracted data
		if [ ! -d /storage ]; then
			show_error 1 "Not enough storage available to proceed"
			return 1
		fi
		local rbi_file="$(mktemp /storage/rbiXXXXXX)"
		add_temp_file $rbi_file
	fi
	...
	if [ -n $bank ]; then
		if [ $block == "mmc" ]; then
			if [ ! -f /tmp/emmcwrite ]; then
				(dd of=$rbi_file bs=51200 <$mtd_pipe) &
			else
	...
	if [ -n $bank ]; then
		if [ $block == "mmc" ] && [ ! -f /tmp/emmcwrite ]; then
			get_flash_partition_mmc
			dd count=$(( $kernel_blk_size / 20480 )) bs=20480 if=$rbi_file of=$kernel_block 2>/dev/null || {
			show_error 16 "Write failure while programming $kernel_block"
			return 1
			}
			sync
			dd skip=$(( $kernel_blk_size / 20480 )) bs=20480 if=$rbi_file of=$fs_block 2>/dev/null || {
			show_error 16 "Write failure while programming $fs_block"
			return 1
			}
			sync
			touch /tmp/emmcwrite
...
mount_overlay_if_necessary() {
	...
	if ! ( mount | grep '/dev/mtdblock[0-9] on /overlay type jffs2' >/dev/null || mount | grep '/overlay type ext4' >/dev/null || mount | grep '/overlay type ubifs' >/dev/null ); then
		...
		local block=$(get_flash_type)
			...
			mkdir -p /overlay
			device=/dev/$(grep -E "(ext)" /proc/partitions |  cut -d' ' -f15)
			mount $device /overlay -t ext4
			sleep 1
			mount -o remount,rw /overlay
			sleep 1
		fi
...
platform_check_image() {
	# Since commit 30f61a34 this function is called from both the sysupgrade script
	# and procd when 'ubus call system sysupgrade' is called. This means that the
	# image is downloaded, checked and written to flash twice. Since the second
	# time is redundant and causes watchdog timeouts the check is not performed
	# when called the second time from procd (the init process).
	if [ "$PPID" -ne 1 ] && ! platform_check_image_imp "$@"; then
		local bank=$(platform_streaming_bank)
		if [ -n $bank ]; then
			local block=$(get_flash_type)
			if [ $block == "mtd"]; then
				...
				get_flash_partition_mmc
				dd count=10 bs=20480 if=/dev/zero of=$kernel_block 2>/dev/null || {
				show_error 16 "Flash failure while clearing $kernel_block"
				}
				dd count=10 bs=20480 if=/dev/zero of=$fs_block 2>/dev/null || {
				show_error 16 "Flash failure while clearing $fs_block"
				}
			fi
		fi
		return 1
...
platform_do_upgrade_bank() {
	...
	if [ $block == "mtd" ]; then
		...
		dd bs=4 count=1 if=/dev/zero of=$kernel_block 2>/dev/null || {
		show_error 16 "Flash failure while clearing FVP"
		return 1;
		}
	fi
