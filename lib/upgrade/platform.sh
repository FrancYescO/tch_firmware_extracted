...
get_image() { # <source> [ <command> ]
	...
	case "$from" in
		http://*|ftp://*) cmd="curl --connect-timeout 300 -m 900 -S -s";;
...
platform_check_image_imp() {
	...
	set -o pipefail
		RBIINFO=$(get_image "$1" | tee $SIGCHECK_PIPE | (bli_parser > $HDRINFO && bli_unseal)| tee $MTDPIPE | lua /lib/upgrade/rbi_vrss.lua)
	if [ $? -ne 0 ]; then
