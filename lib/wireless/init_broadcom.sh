...
if [ "$BOARD" = "VANT-Y" ] ; then
	echo "EXECUTING BOARD SPECIFIC WIFI CONFIG FOR $BOARD" > /dev/console
	#For now keep mu-mimo disabled (given large fixed bug list in gold2.3 driver)

        wl -i wl1 down
        echo "DISABLING MU-MIMO" > /dev/console
        wl -i wl1 txbf_bfe_cap 1
        wl -i wl1 txbf_bfr_cap 1

	wl -i wl1 up
	wl -i wl1 radarthrs 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6b4 0x30 0x6b4 0x30 0x6b4 0x30
	wl -i wl1 down
fi
...
if [ "$BOARD" = "VBNT-J" ] ; then
	...
	wl -i wl1 up
	wl -i wl1 radarthrs 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30
	wl -i wl1 down
fi
