...
create_jump() {
  ...
  if [ "$transport_type" == "undefined" ]; then
    iptables -t nat -C "${MMPBX_CHAIN}" -p "udp" -m "udp" --dport "$local_port" -m comment --comment "DMZ_Exception_SIP" -j ACCEPT 2>/dev/null ||
      iptables -t nat -I "${MMPBX_CHAIN}" -p "udp" -m "udp" --dport "$local_port" -m comment --comment "DMZ_Exception_SIP" -j ACCEPT
    iptables -t nat -C "${MMPBX_CHAIN}" -p "tcp" -m "tcp" --dport "$local_port" -m comment --comment "DMZ_Exception_SIP" -j ACCEPT 2>/dev/null ||
      iptables -t nat -I "${MMPBX_CHAIN}" -p "tcp" -m "tcp" --dport "$local_port" -m comment --comment "DMZ_Exception_SIP" -j ACCEPT
  else
