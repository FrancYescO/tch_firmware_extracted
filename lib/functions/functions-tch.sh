...
# setrtprio <name> <prio> [rr|fifo|other]
# [rr|fifo|other] is an optional parameter
#    rr to set task to SCHED_RR
#    fifo to set task to SCHED_FIFO
#    other to set task to SCHED_OTHER
...
setrtprio() {
	...
	if [ -z "$kthr_pid" ]; then
		kthr_pid=`ps | grep $name | grep -v grep | awk '{printf $1}'`
	fi
