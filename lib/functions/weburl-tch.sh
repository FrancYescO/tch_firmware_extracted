...
insert_weburl_rule () {
    ...
    iptables -A weburlfilter -m connmark --mark 0x0/0x$mark_mask_value -p tcp $site_match_str $dev_str $mac_str -j $act_str
    [ "$ipv6_enable" == "1" ] && ip6tables -A weburlfilter -m connmark --mark 0x0/0x$mark_mask_value -p tcp $site_match_str $dev_str $mac_str -j $act_str
}
...
insert_tables() {
    ...
    [ "$ipv6_enable" == "1" ] && ip6tables -N weburlfilter
