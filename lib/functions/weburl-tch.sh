...
global_enable=0
tod_enable=0     # the profile for conntrack HTTP helper should be loaded in case tod is enabled, to support tod redirection.
urlfilter_enable=
exclude=
...
check_helper() {
    ...
    local ip6t_enable=$ipv6_enable

    [ "$tod_enable" == "1" ] && { enable="1"; ip6t_enable="1"; }

    [ "$table" == "ip6tables" -a "$ip6t_enable" != "1" ] && enable=0

    if [ "$enable" == "1" -a -z "$http_help_exist" ]
    then
        ${table} -t raw -A helper_binds -i br-lan -p tcp --dport 80 -j CT --helper http
    fi
}
...
turn_on_off_http_helper() {
    ...
    if [ "$global_enable" != "1" -a "$tod_enable" != "1" ]
    then
	# global disabled
...
start() {
    ...
    tod_enable="$(uci -q get tod.global.tod_enabled)"
    ...
    if [ -z "$blocked_page_redirect" ]; then
        ...
        if [ -z "$hostname" ]; then
            echo "no hostname given, webredirect section is empty!"
            return
        fi
        blocked_page_redirect="http://$hostname.$(uci get dhcp.@dnsmasq[0].domain)$(uci get web.parentalblock.target 2>/null)"
    fi
