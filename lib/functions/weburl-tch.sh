...
clean_table() {
    ...
    local tmp="$(iptables-save -t filter | grep forwarding_rule | grep weburlfilter)"
    if [ -n "$tmp" ]
    then
        tmp=${tmp:2}   # remove the head "-A"
	iptables -t filter -D $tmp
    fi

    local tmp="$(iptables-save -t filter | grep SKIP_HTTP_HELPER)"
    [ -n "$tmp" ] && iptables -t filter -D ${tmp:2}
...
insert_tables() {
    ...
    # never block packet from modem
    iptables -I INPUT -p tcp --dport 80 -m comment --comment SKIP_HTTP_HELPER -j CONNMARK --set-mark 0x$skip_mark_value/0x$mark_mask_value

    # insert the chain into the forwarding_rule
    iptables -I forwarding_rule -p tcp --dport 80 -m connmark --mark 0x0/0x$mark_mask_value -j weburlfilter
...
turn_on_off_http_helper() {
    ...
    then
	iptables -t raw -A helper_binds -p tcp --dport 80 -d 192.168.1.1 -j RETURN
        iptables -t raw -A helper_binds -p tcp --dport 80 -j CT --helper http
    elif [ "$global_enable" != "1" -a -n "$http_help_exist" ]
    then
        iptables -t raw -D helper_binds -p tcp --dport 80 -j CT --helper http  > /dev/null 2>&1
    fi
...
start() {
    ...
    [ -n "$blocked_page_redirect" ] || blocked_page_redirect="http://$(uci get network.lan.ipaddr)$(uci get web.parentalblock.target 2>/null)"
