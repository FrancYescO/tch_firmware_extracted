...
setup_routing()
{
    ip rule delete fwmark ${http_mark} lookup tod 2>/dev/null
...
load_action() {
    ...
    # re-use marks from weburl
    local stop_processing_mark="$(uci -q get parental.general.stop_processing_mark)"
    local redirected_mark="$(uci -q get parental.general.redirected_mark)"

    [ -z "$stop_processing_mark" ] && stop_processing_mark="2000000"
    [ -z "$redirected_mark" ] && redirected_mark="4000000"
    mask_value=$(( 0x${stop_processing_mark} | 0x${redirected_mark} ))
    ...
    http_mark="0x${stop_processing_mark}/${mask_value}"
