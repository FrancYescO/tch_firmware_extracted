...
create_tod_consecutive_days()
{
    local mode="$1"
    local id="$2"
    local day1_start_time="$3"
    local day2_stop_time="$4"
    local day_one="$5"
    local day1_stop_time="23:59:59"
    local day2_start_time="00:00:00"
    Mon=Tue; Tue=Wed; Wed=Thu; Thu=Fri; Fri=Sat; Sat=Sun; Sun=Mon;

    for day_one in $day_one
    do
      day_two=$(eval echo \$${day_one})
      local day1_option_time="-m time --kerneltz --weekdays ${day_one} --timestart ${day1_start_time} --timestop ${day1_stop_time}"
      local day2_option_time="-m time --kerneltz --weekdays ${day_two} --timestart ${day2_start_time} --timestop ${day2_stop_time}"

      create_tod_rule_mac_${mode} "${id}" "${day1_option_time}"
      create_tod_rule_mac_${mode} "${id}" "${day2_option_time}"
    done
}
...
create_tod_rule_mac()
{
    ...
    if [ "${start_time//':'}" -gt "${stop_time//':'}" ] ; then
      create_tod_consecutive_days "${mode}" "${id}" "${start_time}" "${stop_time}" "${weekdays}"
      return
    fi
