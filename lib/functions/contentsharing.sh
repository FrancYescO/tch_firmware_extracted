...
cs_del_device_dlnadmedia() {
  ...
  # Remove the alias from the mvfs configuration file (/tmp/.mvfs/mvfs.ini).
  # Reloading triggers mvfs to reread his configuration and to release any fds
  # active.
  cs_get_device_symbolic_alias "${device}" "alias"
  if [ -n "${alias}" ] ; then
    /etc/init.d/mvfs del_volume "${alias}"
    /etc/init.d/mvfs reload
...
cs_cleanup_device () {
  ...
  # Actions if minidlna is running
  if cs_is_dlna_running ; then
    cs_del_device_dlnamedia "${device}"
    cs_update_dlnadirs
  fi
  # Actions if dlnad is running
  if cs_is_dlnad_running ; then
    cs_del_device_dlnadmedia "${device}"
  fi
