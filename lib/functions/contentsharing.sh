...
_cs_is_dlna_db_on_mount () {
  ...
  local mountdpath="$(uci_get mountd mountd path "/mnt/usb/")"
  local found_twice=$(find -L ${dir}/files.db | grep -c ${mountdpath})
...
cs_del_device_dlnamedia() {
  ...
  #
  # Important: ${NAME} is only set inside hotplug.d scripts.
  #
...
cs_eject_device () {
  ...
  local lines=$(lsof /tmp/run/mountd/${device} | tail -n +2)
  ...
  /bin/umount "/tmp/run/mountd/${device}"
