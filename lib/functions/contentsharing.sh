...
# Mountd fixed mountpath
# Mountd uci config has an option 'path'. Inside this 'path' a symbolic link is
# made to an entry inside the fixed path. The fixed path is used throughout the
# script because symbolic links are disallowed for samba for security reasons.
CS_MOUNTDPATH="/tmp/run/mountd"
...
_cs_it_devices () {
  ...
  local present=$(cat /proc/partitions | grep -c ${section})
...
# cs_add_device_sambauci <sharename> <shareconfig> <device> <filesystem> <mountpoint>
# Add a section type 'sambashare' with section name <sharename> to
# /var/state/samba.
#
cs_add_device_sambauci () {
  if ! _cs_valid_args "5" "${@}" ; then
...
cs_del_device_dlnamedia() {
  ...
  if [ -z "${mountpoint}" ] ; then
    mountpoint="${CS_MOUNTDPATH}/${device}"
  fi
...
# Eject the device <device>. Clean up samba and DLNA + have mountd ignore the
# device.
#
cs_eject_device () {
  if ! _cs_valid_args "1" "${@}" ; then
  ...
  local lines=$(lsof ${CS_MOUNTDPATH}/${device} | tail -n +2)
  ...
  /bin/umount "${CS_MOUNTDPATH}/${device}"
