...
parse_telnet_setup() {
	...
	iptables -t filter -I zone_lan_input -p tcp -m tcp  --dport 23 -m comment --comment "Allow_Telnet_Conn" -j REJECT
	iptables -t filter -I zone_Guest_input -p tcp -m tcp  --dport 23 -m comment --comment "Allow_Telnet_Conn" -j REJECT
    allowedipv6=$(( allowedipv6 + 1 ))
    ip6tables -t filter -I zone_${zone}_input $allowedipv6 -p tcp -m tcp  --dport 23 -m comment --comment "Allow_Telnet_Conn" -j DROP
	ip6tables -t filter -I zone_lan_input -p tcp -m tcp  --dport 23 -m comment --comment "Allow_Telnet_Conn" -j REJECT
	ip6tables -t filter -I zone_Guest_input -p tcp -m tcp  --dport 23 -m comment --comment "Allow_Telnet_Conn" -j REJECT
}
