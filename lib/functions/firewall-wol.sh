...
iptables-save -t nat | grep "Wake on LAN over the Internet" | sed 's/^\-A\ /iptables -t nat -D /g' | while read LINE
    do
        eval $LINE
    done
...
if [ -n "$zone" -a -n "$src_dport" -a -n "$dest_ip" ]; then
    ...
    iptables -t nat -I prerouting_${zone}_rule -p udp -m udp --dport $src_dport -m comment --comment "Wake on LAN over the Internet" -j DNAT --to-destination $dest_ip
fi
