...
ppp_generic_setup() {
	...
	[ -x /usr/lib/pppd/*/connstate.so ] && connstate=1
...
proto_pppoe_init_config() {
	...
	proto_config_add_string "pppoe_backoff"
	proto_config_add_boolean "graceful_restart"
...
proto_pppoe_setup() {
	...
	json_get_var pppoe_backoff pppoe_backoff
	json_get_var graceful_restart graceful_restart
	json_get_var host_uniq host_uniq

	if [ "$graceful_restart" = 1 ]; then
		graceful_restart="/etc/ppp/pppoesession_$iface"
	else
		rm -f /etc/ppp/pppoesession_$iface
		unset graceful_restart
	fi
	...
	ppp_generic_setup "$config" \
		...
		${pppoe_backoff:+pppoe_backoff "$pppoe_backoff"} \
		${graceful_restart:+graceful_restart "$graceful_restart"} \
		"nic-$iface" \
...
proto_pppoa_setup_tch() {
	# Wrapper to allow configuring the PVC without specifying the vpi/vci in
	# the network section.  This parses an intf name with the form atm_{vpi}_{vci}
	json_get_vars ifname
	if [ -z "$vpi" -a -z "$vci" -a "${ifname:0:4}" = "atm_" ]; then
		local newvpi newvci
		newvpi=$(echo $ifname | sed -e "s/^atm_\([0-9]\+\)_\([0-9]\+\)$/\1/")
		newvci=$(echo $ifname | sed -e "s/^atm_\([0-9]\+\)_\([0-9]\+\)$/\2/")
		if [ -n "$newvpi" -a -n "$newvci" ]; then
			vpi=$newvpi
			vci=$newvci
		fi
	fi
