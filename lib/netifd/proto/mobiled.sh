...
check_and_remove() {
	...
	if ubus list network.interface.${interface}; then
		local ipv4_addr=$(ubus call "network.interface.${interface}" status | jsonfilter -e '@["ipv4-address"][0].address')
	        if [ -n "$ipv4_addr" ]; then
			conntrack -D conntrack --src-nat $ipv4_addr
			conntrack -D conntrack --dst-nat $ipv4_addr
		fi
