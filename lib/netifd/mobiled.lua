...
events['mobiled'] = function(msg)
	...
	if msg.event == "device_initialized" or msg.event == "network_registered" and msg.dev_idx == dev_idx then
	...
	elseif msg.event == "device_removed" and msg.dev_idx == dev_idx then
		local cmd = string.format("/lib/netifd/mobiled.script %d %d %s %s", session_id, msg.dev_idx, interface, "teardown")
		os.execute(cmd)
	end
end
