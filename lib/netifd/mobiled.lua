...
local session_id, profile_id, dev_desc, interface, bridge
...
for option, argument in helper.getopt(arg, 's:p:d:i:b:o') do
	...
	elseif(option == "b") then
		if argument ~= "" then bridge = argument end
...
local event_data = { event = "session_activate", session_id = session_id, profile_id = profile_id, interface = interface, optional = optional, bridge = bridge }
...
events['mobiled'] = function(msg)
	...
	local dev_idx = get_device_idx(dev_desc)
	if msg.event == "device_initialized" and msg.dev_idx == dev_idx then
		event_data.dev_idx = dev_idx
		conn:send("mobiled", event_data)
	elseif msg.event == "session_state_changed" and tonumber(msg.session_id) == session_id and msg.dev_idx == dev_idx and msg.session_state then
		local pdp_type = msg.pdp_type or "ipv4v6"
		local cmd = string.format("/lib/netifd/mobiled.script %d %d %s %s %s", session_id, msg.dev_idx, interface, pdp_type, msg.session_state)
		os.execute(cmd)
	elseif msg.event == "device_removed" and msg.dev_idx == dev_idx then
		local cmd = string.format("/lib/netifd/mobiled.script %d %d %s %s %s", session_id, msg.dev_idx, interface, "ipv4v6", "teardown")
		os.execute(cmd)
	end
end
...
events['mobiled.network'] = function(msg)
	if (msg.action == "ifup" or msg.action == "ifupdate") and msg.interface and string.match(msg.interface, interface) then
		local cmd = string.format("/lib/netifd/mobiled.script %d %d %s %s %s", session_id, event_data.dev_idx, interface, "ipv4v6", "augment")
		os.execute(cmd)
	end
end
