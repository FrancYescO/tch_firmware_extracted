...
function netifd_helper:ipv6_trigger_rs()
	local ipv6_interface = string.format("%s_6", self.config.interface)
	-- send kill command for signal SIGALARM (14) for IPv6 interface
	self.ubus:call("network.interface", "notify_proto", { interface = ipv6_interface, action = 2, signal = 14})
end

function netifd_helper:ipv6_defaultroute_address_validity_timer_expired()
	log:debug("IPv6 route or address validity expired on interface %s_6", self.config.interface)
	self:ipv6_trigger_rs()
	self.ipv6_defaultroute_address_validity_timer = nil
end
			...
			if event_data.interface == self.config.interface .. "_6" then
				...
				if data then
					...
					if data.route then
						for _, route in pairs(data.route) do
							...
							if route.target == "::" and valid_duration then
								min_timer_duration = valid_duration
								break
							end
						end
					end
					...
					if data['ipv6-address'] and #data['ipv6-address'] > 1 then
						log:debug("Triggering ifup because more than one IPv6 address present on %s", event_data.interface)
						self:ipv6_trigger_rs()
					end
					-- if LTE NW sending multiple addresses then consider the minimum validity duration among the received address
					if data['ipv6-address'] then
						for _, ipv6_address in pairs(data['ipv6-address']) do
							if ipv6_address.valid then
								local addr_valid_duration = tonumber(ipv6_address.valid)
								min_timer_duration = math.min(addr_valid_duration, min_timer_duration or math.huge)
							end
						end
					end
					-- cancel the timer if it is already running
					if self.ipv6_defaultroute_address_validity_timer then
						self.ipv6_defaultroute_address_validity_timer:cancel()
					end
					-- start a single timer with the minimum validity timer among the multiple addresses and default route
					if min_timer_duration then
						self.ipv6_defaultroute_address_validity_timer = uloop.timer(function() self:ipv6_defaultroute_address_validity_timer_expired() end)
						log:debug("Starting IPv6 route or address validity expiry timer of %d seconds on interface %s", min_timer_duration, self.config.interface)
						self.ipv6_defaultroute_address_validity_timer:set(min_timer_duration * 1000)
					end
				end
			end
			self:queue_action({
