...
gpon_setup_l3_interface() {
	...
	# gpon_create_vlanif creates VLAN interface which triggers the calling of hotplug.d/iface/00-qos.
	# At the same time, rules are getting removed and added by this script.
	# The simultaneous action of rules removal/addition results in incorrect rule entries.
	# So, adding a delay of 1 second to allow the script to complete its processing
	# and so that this script can continue with the rule configurations further.
	sleep 1
