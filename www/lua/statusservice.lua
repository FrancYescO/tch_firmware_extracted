...
local function do_get(map)
  local get = map.get
  local data = {}
  if type(get) == "function" then
    local rc, res = pcall(get)
...
local function do_getset(uri)
  ...
  for k,v in pairs(uri) do
    if k ~= "act" and k ~= "service" and k ~= "_" and k ~= "cmd" and k ~= "nvget" and k ~= "auth" then
      seturi[k] = v
      setflag = true
    end
  end
...
function M.action(uri)
  ...
  if rc then
    if uri.cmd ~= "3" or uri.auth ~= true then
