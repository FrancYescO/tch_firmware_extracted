...
local em_new_layout = proxy.get("uci.env.var.em_new_ui_layout")
em_new_layout = em_new_layout and em_new_layout[1] and em_new_layout[1].value or "0"

function M.getBandSteerState(bspeerap, ap, multiap_cred_secondary_path, isguest)
  local band_steer_supported, band_steer_enabled, bandSteer_id
  if bspeerap then
    ...
    if multiap_cred_secondary_path then
      ...
      if isguest ~= "1" then
        if em_new_layout == "0" and post_helper.isFeatureEnabled("bandsteerDisabled" , role) then
          content_band_steer.band_steer_state = bs_path
        end
        ...
        bandSteer_id = proxy.get("uci.wireless.wifi-ap.@" .. ap .. ".bandsteer_id") and proxy.get("uci.wireless.wifi-ap.@" .. ap .. ".bandsteer_id")[1].value or ""
        if bandSteer_id then
          bandSteer_id = string.untaint(bandSteer_id)
          content_band_steer.band_steer_id = "uci.wireless.wifi-bandsteer.@"..bandSteer_id..".state"
        end
      end
    end
    ...
    if content_band_steer.band_steer_id == "" then
      content_band_steer.band_steer_id = "1"
    end
    if content_band_steer.band_steer_id ~= "" then
      band_steer_supported = true
      if em_new_layout == "0" and post_helper.isFeatureEnabled("bandsteerDisabled", role) then
	 if not multiap_cred_secondary_path and (content_band_steer.band_steer_state == "1") then
	 ...
	 if not multiap_cred_secondary_path and (content_band_steer.band_steer_id ~= "0") then
      ...
      if multiap_cred_secondary_path and content_band_steer.multiap_cred_secondary_state == "0" then
        band_steer_enabled = true
      end
    end
  end
