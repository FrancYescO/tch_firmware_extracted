...
M.days = {T"Mon", T"Tue", T"Wed", T"Thu", T"Fri", T"Sat", T"Sun"}
...
local prod_name = proxy.get("uci.env.var.vf_prod_name")[1].value or ""
local board_Mnemonic = proxy.get("uci.env.rip.board_mnemonic")[1].value or ""
local look_4 = "/img/look_4/"
if board_Mnemonic == "VBNT-R" then
--    look_4 = "/img/vbnt-r/"
elseif board_Mnemonic == "VBNT-Z" then
    look_4 = "/img/vbnt-z/"
end
local wanColour = "yellow"
if board_Mnemonic == "VBNT-R" or board_Mnemonic == "VBNT-Z" then
    wanColour = "red"
end
local device_picture = look_4.."activation/boxes/160705_VFH-500-s_1.115_front-right_corrected.jpg"
if board_Mnemonic == "VBNT-R" then
	device_picture = look_4.."overview/slack-imgs.com.jpg"
end

function M.productName()
  return prod_name
end
function M.look4()
  return look_4
end
function M.wanColour()
  return wan_Colour
end
function M.devicepicture()
  return device_picture
end
function M.board()
  return board_Mnemonic
end

function M.getDataInterface()
  dataInterface = {
  "adsl_wholesales",
  ...
  if M.getVariant() == "VF-UK" then
    return dataInterfaceVox
  else
...
function M.calculateDHCPStartAndLimitAddress(ipAddress, netmask, start, limit)
  if start and limit and ipAddress and netmask then
    ...
    local endAddress
    if M.getVariant() == "VF-UK" then
      endAddress = startAddress+limit
    else
      endAddress = startAddress+limit - 1
    end
    local limit = ipmax - network
    startAddress = post_helper.num2ipv4(startAddress)
    if endAddress > ipmax then
      endAddress = ipmax
    end
    endAddress = post_helper.num2ipv4(endAddress)
...
-- @param #function function to validator
...
function M.calculateIPRange(value)
  ...
  return post_helper.num2ipv4(startIP), post_helper.num2ipv4(endIP)
end
...
-- returns a predefined dummy value ******** if there is a password
...
M.excluded_wan_interfaces = setmetatable({
  dummy     = true,
  loopback  = true,
  gt0       = true,
  auto_iptv = true
...
function M.handlePOST(callback, arguments, onlyPost, response_format)
  if type(callback) ~= "function" then
    return nil
  end
  arguments = type(arguments) == "table" and arguments or {}
  response_format = response_format and "json" or "html"

  if not onlyPost and ngx.req.get_method() == "GET" then
    return callback(unpack(arguments))
  end

  if ngx.req.get_method() == "POST" then
    local success, error_msg = callback(unpack(arguments))
    error_msg = type(error_msg) == "table" and error_msg or {}
    local response = next(error_msg) and "error" or (success and "success" or "error")
    if response_format == "json" then
      ngx.header.content_type = "application/json"
      ngx.print(format('{ "status":"%s" }', response))
      ngx.exit(ngx.HTTP_OK)
    else
      ngx.print(format('<input type="hidden" id="post_status" value="%s">', response))
    end
...
local bandSteerParams = {
  ...
  securityMode =  "uci.wireless.wifi-ap.@ap1.security_mode",
  password     =  "uci.wireless.wifi-ap.@ap1.wpa_psk_key",
}

function M.handleBandsteer(bandSteerEnable, suffix)
  local bandSteerData   =  {}
  bandSteerData.bandSteer24 = bandSteerEnable
  bandSteerData.bandSteer5  = bandSteerEnable

  -- If bandSteer value is Enabled in GUI ssid, securityMode and password will be copied from 2.4GHZ ap for 5GHZ ap.
  if bandSteerEnable == "bs0" then
    local wifi24Values = {
      ssid         =  "uci.wireless.wifi-iface.@wl0.ssid",
      securityMode =  "uci.wireless.wifi-ap.@ap0.security_mode",
      password     =  "uci.wireless.wifi-ap.@ap0.wpa_psk_key",
    }
    content_helper.getExactContent(wifi24Values)
    bandSteerData.ssid           =  wifi24Values.ssid
    bandSteerData.securityMode   =  wifi24Values.securityMode
    bandSteerData.password       =  wifi24Values.password
  else
    local ssid_5g = proxy.get("uci.wireless.wifi-iface.@wl1.ssid")[1].value
    if ssid_5g then
      bandSteerData.ssid = ssid_5g .. suffix
    end
  end
  local bandSteerDataSet
  bandSteerDataSet = content_helper.setObject(bandSteerData, bandSteerParams)
  return bandSteerDataSet
end
