...
function M.getWifiTodRuleLists()
   ...
   for _,rule in pairs(rulePath) do
      ...
      if (#daysList == 2) then
         oldTodRules[#oldTodRules + 1] = {start_time = rule.start_time, stop_time = rule.stop_time, weekday = "All"}
      else
         for _,day in pairs(daysList) do
            if day.value ~= "" then
               oldTodRules[#oldTodRules + 1] = {start_time = rule.start_time, stop_time = rule.stop_time, weekday = day.value}
            end
         end
      end
   end
...
function M.validateTodRule(value, object, key, todRequest)
   ...
   --The DUT will block/allow all the time if none of the days are selected
   if (#value == 2) then
      newTODRule[#newTODRule + 1] = {start_time = object["start_time"], stop_time = object["stop_time"], weekday = "All"}
   else
      -- index start with 3 because userdata is reserved index 1 and 2
      for index = 3, #value do
         newTODRule[#newTODRule + 1] = {start_time = object["start_time"], stop_time = object["stop_time"], weekday = value[index]}
      end
   end
