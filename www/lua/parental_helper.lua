...
local table, untaint = table, string.untaint

--Find Mac in hostdata to find extender
local sysHostData = content_helper.convertResultToObject("sys.hosts.host.",proxy.get("sys.hosts.host."))

function M.checkMACinMAP(mac)
  mac = string.untaint(mac)
  for _, device in pairs(sysHostData) do
    if mac == device.MACAddress then
      if (device.ProductClass == "OWA0131" or device.ProductClass == "OWA3111" or device.ProductClass == "Accesspoint" or device.L2Interface:match("wds") == "wds") then
         return false
       end
       return true
     end
...
function M.getTod()
  setlanguage()
  local mac_temp = M.get_hosts_ac()
  mac_value = {}
-- Remove extenders from the list and update host list to avoid the tod, parental control rules applied on them
  for index, mac in pairs(mac_temp) do
    if M.checkMACinMAP(mac[1]) then
       mac_value[#mac_value + 1] = mac_temp[index]
    end
  end
  ...
  local host_temp = M.get_hosts_ac()
  local host_ac = {}
-- Remove extenders from the list and update host list
  for index, mac in pairs(host_temp) do
    if M.checkMACinMAP(mac[1]) then
       host_ac[#host_ac + 1] =  host_temp[index]
    end
  end
