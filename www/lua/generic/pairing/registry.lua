...
local function insertCertificate(self, cert)
  local isCertFound = false
  local clients = proxy.get("uci.generic_app.certificate.")
  local cert_name = cert:commonName()
  for _, options in pairs(clients) do
    if options.value == cert_name then
      isCertFound = true
    end
  end
  if not locateCertByCommonName(self, cert:commonName()) or not isCertFound then
...
function Registry:paired(pemdata)
  local certData, idx = findCertByPem(self, pemdata)
  ...
  if idx then
    ...
    local isCertFound = false
    cursor:foreach("generic_app", "certificate", function(s)
      if s.name == certData._name then
         isCertFound = true
      end
    end)
    if not isCertFound then
      removeCertAt(self, idx)
      return false
    end
    return true
