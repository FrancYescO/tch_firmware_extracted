...
function M.isIPinOtherRange(ipAdd, ipNet, all_intfs, curintf)
	...
	local ipvalue = validateStringIsIP(ipAdd) and post_helper.ipv42num(ipAdd)
	local ipnetmask = validateStringIsIP(ipNet) and post_helper.ipv42num(ipNet)
	...
	for _,intf in pairs(all_intfs) do
		if intf.paramindex ~= curintf then
			...
			local baseip = validateStringIsIP(ipaddr) and post_helper.ipv42num(ipaddr)
			local netmask = validateStringIsIP(mask) and post_helper.ipv42num(mask)
