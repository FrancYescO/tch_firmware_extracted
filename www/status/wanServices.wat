...
service_simp_port_mapping.get = function()
  ...
  for k,v in pairs(upnp) do
    ...
    get["rule_"..(k-1).."_ext_port"] = v.src_dport
    get["rule_"..(k-1).."_int_port"] = v.dest_port
...
local function getVirtualServerList()
  ...
  for k,v in pairs(pfw) do
    ...
    data["svc_"..(k-1).."_code"] = PMNameCodeMap[untaint(v.name)]
...
service_virtual_server_add.set = function(args)
  ...
  for i=1, total_num do
    if args["ip_"..(i-1)] ~= "unknown" then
      if not validateLanIP(args["ip_"..(i-1)]) then
         return nil, "Invalid IP in service_virtual_server_add set:" .. string.untaint(args["ip_"..(i-1)])
      end
...
local function getBlList()
  ...
  for k,v in pairs(res) do
    if (v.name == "Dev_Deny_Access" or v.name == "Dev_Allow_Access") and v.src_mac ~= "" then
...
service_bl_device_list.set = function(args)
  ...
  local res = content_helper.convertResultToObject(bl_path, proxy.get(bl_path))
  for k,v in pairs(res) do
    if v.name == "Dev_Deny_Access" or v.name == "Dev_Allow_Access" then
      if v.src_mac == "" then
        proxy.del(bl_path..(v.paramindex)..".")
      else
        paths[bl_path..(v.paramindex)..".target"] = mode
        paths[bl_path..(v.paramindex)..".name"] = (mode == "ACCEPT") and "Dev_Allow_Access" or "Dev_Deny_Access"
        proxy.set(paths)
      end
    end
  end
  ...
  if getCurDevCount() ~= 0 and mode == ACCEPT then
    ...
    paths[bl_path..index..".name"] = "Dev_Deny_Access"
    proxy.set(paths)
  end
  proxy.apply()
  return true
...
service_bl_device_add.set = function(args)
  ...
  for k,v in pairs(res) do
    ...
    if v.name == "Dev_Deny_Access" and v.src_mac == "" then
      proxy.del(bl_path..(v.paramindex)..".")
      del = 1
    end
  end
  ...
  if exist == 0 then
    ...
    paths[bl_path..index..".name"] = (mode == "ACCEPT") and "Dev_Allow_Access" or "Dev_Deny_Access"
    proxy.set(paths)
  end
  if del == 1 then
    local index = proxy.add(bl_path)
...
service_bl_device_del.set = function(args)
  ...
  for k,v in pairs(res) do
    ...
    if getCurDevCount() == 0 and  v.name == "Dev_Deny_Access" and v.src_mac == "" then
      proxy.del(bl_path..(v.paramindex)..".")
    end
  end
  proxy.apply()
