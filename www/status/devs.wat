...
local parental_control_path = "uci.parental.URLfilter."
  ...
  for i,v in ipairs(host) do
    if not host[v.paramindex] then
      ...
      if v.devicetype == "1" then
        ...
        family_devs["dev_"..(count2).."_parental"] = v.parental_ctl or "0"
...
local function get_devices_columns(status_type)
  ...
  for i,v in ipairs(hosts) do
    if not hosts[v.paramindex] then
      ...
      devices_columns[#devices_columns+1] = {
        ...
        ipaddress = v.IPAddress:match("^%d+%.%d+%.%d+%.%d+"),
        macAddress = v.MACAddress,
        port = v.Port,
        connectedtime = v.ConnectedTime,
        devicetype = (find(v.DeviceType, "family") ~= nil) and "1"  or "0" ,
        deviceicon = match(v.DeviceType, ":(%d+)$") or "7",
  ...
  for k,v in ipairs(actions) do
    for index, value in ipairs(devices_columns) do
      if find(v.object, value.macAddress) then
        ...
        local enabled = proxy.get(action_path  .. v.paramindex  .. ".enabled")[1].value
        local stop_time_weekdays = proxy.get(timer_path .. "@" ..  timer_index  .. ".stop_time")[1].value
        local stop_time = match(stop_time_weekdays, "%w+:(%d+:%d+)")
        local update_index = ""
        if find(v.paramindex, "routine") and status_type == "family" then
          local start_time_weekdays = proxy.get(timer_path .. "@" ..  timer_index  .. ".start_time")[1].value
          ...
          value[update_index .."enabled"] = enabled
        ...
        if find(v.paramindex, "online") and status_type == "connected" then
          ...
          if convert2Sec(stop_time) == -1 then
            value[update_index .."duration"] = "-1"
          elseif convert2Sec(stop_time) < convert2Sec(start_time) then
            value[update_index .."duration"] = convert2Sec(stop_time) + 24 *3600 - convert2Sec(start_time)
          else
            value[update_index .."duration"] = convert2Sec(stop_time) - convert2Sec(start_time)
          end
          value[update_index .."start"] = start_time
        end
      end
  ...
  local all_parental_filters = content_helper.convertResultToObject(parental_control_path, proxy.get(parental_control_path))
  if type(all_parental_filters) == "table" then
    for k,v in ipairs(all_parental_filters) do
      for index, value in ipairs(devices_columns) do
        if value.macAddress == v.mac and v.action == "ACCEPT" then
          value.parental_ctl = "1"
        end
      end
    end
  end
...
local function family_device_update(args, host_index)
  ...
  if tonumber(host_index) > 0 then
    ...
    local devtype = format("family:%s", args.icon_id)
  ...
  local parental_dev_indexs = {}
  local all_parental_filters = content_helper.convertResultToObject(parental_control_path, proxy.get(parental_control_path))
  if type(all_parental_filters) == "table" then
    for k,v in ipairs(all_parental_filters) do
      if args.mac == v.mac then
         parental_dev_indexs[k] = v.paramindex
      end
    end
  end
  if #parental_dev_indexs > 0  then
    local parental_value = (args.parental_ctl == "1") and "ACCEPT" or "DROP"
    for k,v in pairs(parental_dev_indexs) do
      proxy.set(parental_control_path .. v ..".action", parental_value)
    end
  end
end
...
service_family_device_add.set = function(args)
  ...
  if args.stop_scheduled == nil then
    if dev_type == "" then
      proxy.set(hostpath .. (host_index) .. ".DeviceType", "family:7")
    else
      proxy.set(hostpath .. (host_index) .. ".DeviceType", "family" .. dev_type)
    end
  else
...
service_generic_device_edit.set = function(args)
  ...
  for k,v in ipairs(hosts) do
    if v.MACAddress == args.mac then
      local devtype = proxy.get(hostpath .. (v.paramindex) .. ".DeviceType")[1].value
      local icon_id = (args.icon_id == "") and "7" or args.icon_id
      devtype = (devtype ~= "") and gsub(devtype, "%d+", icon_id) or (":" .. icon_id)
