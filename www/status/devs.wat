  ...
  for k,v in ipairs(actions) do
    for index, value in ipairs(devices_columns) do
      ...
      if find(v.object, value.macAddress) then
        ...
        if status_type == "connected" then
          local routine_timer_index
          local rt = {
            enabled = untaint(action_path  .. v.paramindex .. ".enabled"),
            start_time = untaint(timer_path .. v.paramindex .. ".start_time"),
            stop_time = untaint(timer_path .. v.paramindex .. ".stop_time"),
          }

          if find(v.paramindex, "online") then
            routine_timer_index = gsub(v.paramindex, "action_online", "routine")
          ...
          if find(v.paramindex , "boost_action_") then
          ...
          elseif find(v.paramindex , "stop_action_") then
