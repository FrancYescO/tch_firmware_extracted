...
local function reload_wait(timeout, sleep_time, valid_path)
    for i=1, timeout do
      local valid_value =  proxy.get(valid_path)
      if valid_value then
        break;
      else
...
service_wl5g_adv.set = function(args)
  ...
  if args.wl0_auto_channel == "Auto" then
    local old_channel =  proxy.get(wl5g_adv_path["wl0_auto_channel"])[1].value
...
local function get_bsslist_columns(type, bsslistData_2G, bsslistData_5G)
  ...
  local bl_2g = content_helper.convertResultToObject(pathwl_ssid_2g, bsslistData_2G or proxy.get(pathwl_ssid_2g))
  local bl_5g = content_helper.convertResultToObject(pathwl_ssid_5g, bsslistData_5G or proxy.get(pathwl_ssid_5g))
...
service_wlssid_list.get  = function(args)
  ...
  if args.do_scan == "1" then
    ...
    local step = 0
    while step <= 15 do
      bsslistData_2G = proxy.get(pathwl_ssid_2g) or {}
      if next(bsslistData_2G) then
        bsslistData_5G = proxy.get(pathwl_ssid_5g) or {}
