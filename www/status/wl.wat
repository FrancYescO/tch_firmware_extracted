...
local function wifitod_status()
    ...
    local eco_timeslot = 0
    if eco_status.start ~= "" and eco_status.stop ~= "" then
    ...
    return eco_status.enabled, eco_timeslot
end
...
local function wl_total_set(args)
  ...
  if args.wl0_ssid ~= nil or args.wl1_ssid ~= nil then
    ...
    local status = {
      wl_2g = pathradio_2g .."admin_state",
      wl_5g = pathradio_5g .."admin_state",
      wps_2g = pathap_2g .. "wps.admin_state",
      wps_5g = pathap_5g .. "wps.admin_state",
    }
    content_helper.getExactContent(status)
    if args.wl1_enabled == "0" and status.wl_5g == "0" and (status.wps_2g == "1" or status.wps_5g == "1") then
       wps_status.gui_enabled = "0"
    elseif args.wl0_enabled == "0" and status.wl_2g == "0" and (status.wps_2g == "1" or status.wps_5g == "1") then
       wps_status.gui_enabled = "0"
    else
       wps_status.gui_enabled = "1"
    end
    --when wifitod is enabled and in timeslot, it has higher priority
    local eco_enabled, eco_timeslot = wifitod_status()
    if eco_enabled == "1" and eco_timeslot == 1  then
       params[pathradio_2g .."admin_state"] = nil
       params[pathradio_5g .."admin_state"] = nil
    end
    proxy.set(params)
    proxy.apply()
    os.execute("sleep 3")
...
service_eco.set = function(args)
  ...
  --when disable wifitod in timeslot, execute right now here instead of in wifitod_script
  local eco_enabled, eco_timeslot = wifitod_status()
  if args.eco_dis == "0" and eco_enabled == "1" and eco_timeslot == 1  then
    eco_status[pathradio_2g .."admin_state"] = "1"
    eco_status[pathradio_5g .."admin_state"] = "1"
  end
...
local wl_guest_mapParams_path = {
  ...
  hotspot_broadcast_ssid = "uci.wireless.wifi-ap.@" .. ap_hotspot .. ".public",
...
local function get_wl_guestaccess()
  ...
  for k,v in ipairs(actions) do
    if v.paramindex == "@action_guest_restriction" then
      wl_guest_mapParams.hotspot_filtering = find(v.object,"[A|a][L|l][L|l]") and "all" or "web"
...
service_wl_guestaccess.set = function(args)
  ...
  if args.hotspot_enable == "1" then
    if duration and duration ~= "-1" then
      ...
      wl_guest_mapParams["uci.tod.timer.@timer_guest_restriction.lease"] = maxtime[tostring(untaint(args.hotspot_timeout))] or ""
    ...
    elseif duration then
      ...
      wl_guest_mapParams["uci.tod.timer.@timer_guest_restriction.lease"] = ""
    end
    ...
    if args.hotspot_broadcast_ssid then
      wl_guest_mapParams["uci.wireless.wifi-ap.@" .. (ap_hotspot) .. ".public"] = args.hotspot_broadcast_ssid
    end
    if filter then
