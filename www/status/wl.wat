...
local find, match, format, untaint = string.find, string.match, string.format, string.untaint
...
local function wl_total_set(args)
  ...
  if args.wl0_ssid ~= nil or args.wl1_ssid ~= nil then
    ...
    if args.wl1_enabled == "1" then
      params[pathap_5g .. "wps.admin_state"] = args.wl1_enabled
    end
    if args.wl0_enabled == "1" then
      params[pathap_2g .. "wps.admin_state"] = args.wl0_enabled
    end
...
local function web_get_eco()
  ...
  if web_eco_status.eco_dis then
    ...
    elseif len < 4 then
      ...
      web_eco_status.eco_fri =  find(eco_start,"Fri") and "1" or "0"
      web_eco_status.eco_sat =  find(eco_start,"Sat") and "1" or "0"
      web_eco_status.eco_sun =  find(eco_start,"Sun") and "1" or "0"
    end
  else
    web_eco_status.eco_mon = "0"
    web_eco_status.eco_tue = "0"
...
service_eco.set = function(args)
  ...
  if weekstr == "" then
    weekdays_start_time = os.date("%a", os.time()) .. ":" .. args.eco_start_time
    if (convert2Sec(args.eco_start_time) > convert2Sec(args.eco_end_time)) then
      weekdays_end_time = (os.date("%a", os.time() + 24*3600) .. ":")  .. args.eco_end_time
    else
      weekdays_end_time = os.date("%a", os.time()) .. ":" .. args.eco_end_time
    end
  else
    weekdays_start_time = weekstr .. args.eco_start_time
    weekdays_end_time = weekstr .. args.eco_end_time
  end

  eco_status["uci.tod.action.@action_wifidisable.enabled"] = args.eco_dis
  eco_status["uci.tod.timer.@timer_wifidisable.periodic"] = (weekstr == "") and "0" or "1"
  eco_status["uci.tod.timer.@timer_wifidisable.start_time"] = weekdays_start_time
  eco_status["uci.tod.timer.@timer_wifidisable.stop_time"]= weekdays_end_time
