...
local led_status_paths = {
  background_enabled1 = "sys.class.leds.@ambient1:white.brightness",
  background_enabled2 = "sys.class.leds.@ambient2:white.brightness",
  background_enabled3 = "sys.class.leds.@ambient3:white.brightness",
  background_enabled4 = "sys.class.leds.@ambient4:white.brightness",
  background_enabled5 = "sys.class.leds.@ambient5:white.brightness",
  background_schedule = "uci.tod.action.@ledtod.enabled",
  background_timer = "uci.tod.action.@ledtod.timers.@1.value",
  line_red_path = "sys.class.leds.@broadband:red.brightness",
  line_green_path = "sys.class.leds.@broadband:green.brightness",
  wifi_red_path = "sys.class.leds.@wireless:red.brightness",
  wifi_green_path = "sys.class.leds.@wireless:green.brightness",
  wifi_orange_path = "sys.class.leds.@wireless:orange.brightness",
  wps_red_path = "sys.class.leds.@wps:red.brightness",
  wps_green_path = "sys.class.leds.@wps:green.brightness",
  wps_orange_path = "sys.class.leds.@wps:orange.brightness",
...
local function getLedStatus()
  ...
  if paths.background_enabled1 ~= "0" and paths.background_enabled2 ~= "0"
     and paths.background_enabled3 ~= "0" and paths.background_enabled4 ~= "0"
     and paths.background_enabled5 ~= "0" then
    ret["background_enabled"] = "1"
  else
    ret["background_enabled"] = "0"
  end
  ret["background_schedule"] = paths.background_schedule
  if paths.line_red_path == "255" then
    ret["led_0"] = "0"
  else
    ret["led_0"] = "1"
  end
  if paths.wifi_red_path == "255" then
    ret["led_1"] = "0"
  else
    ret["led_1"] = "1"
  end
  if paths.wps_red_path == "255" then
    ret["led_2"] = "0"
  else
    ret["led_2"] = "1"
  end
  if paths.background_timer ~= "" then
    ...
    ret["background_start"] = proxy.get(tab.background_start)[1].value
    ret["background_end"] = proxy.get(tab.background_end)[1].value
  end
  return ret
end
...
service_led_status.set = function(args)
  ...
  local paths = {}
  --Must disable/enable LED tod first, then process the tast of ON/OFF the ambient light
  proxy.set("uci.tod.action.@ledtod.enabled", args.background_schedule)
  proxy.apply()
  if args.background_schedule == "0" then
  -- When LED tod is disabled, ON/OFF the light directly
    paths["rpc.ambientLed.enabled"] = args.background_enabled
  else
  --If LED tod is enabled, then ON/OFF function is assigned to LED TOD, we cannot control it directly.
...
local function  pingTest()
  ...
  local dns1_ping_state = proxy.get(dns1_ping_path)[1].value
  if num_servers >= 1 and dns1_ping_state ~= "Requested" then
    proxy.set(dns1_ping_path, "Requested")
  end
  local dns1_ping_state = proxy.get(dns2_ping_path)[1].value
  if num_servers >= 2 and dns2_ping_state ~= "Requested" then
    proxy.set(dns2_ping_path, "Requested")
  end
  proxy.apply()
  ------Must wait 3 seconds, otherwise cannot get the latest ping result
  os.execute("sleep "..3)
  ...
  if num_servers == 1 and pingTable.dns1_ping == "Success" then
    get["next_dns_ping"] = "1"
  elseif num_servers == 2 and (pingTable.dns1_ping == "Success" and pingTable.dns2_ping == "Success") then
    get["next_dns_ping"] = "1"
  else
    get["next_dns_ping"] = "0"
  end
