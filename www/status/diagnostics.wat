...
local function get_usb_info(usb)
  usb["usb_port1"] = "Not Connect"
  usb["usb_port2"] = "Not Connect"
  local usbobj = api.GetObjects("sys.usb.device.")
  if #usbobj >= 2 then
    usb["usb_port1"] = "Connected"
    usb["usb_port2"] = "Connected"
  elseif #usbobj == 1 then
    if usbobj[1].path:find("-1") then
      usb["usb_port1"] = "Connected"
    end
    if usbobj[1].path:find("-2") then
      usb["usb_port2"] = "Connected"
    end
  end
end
