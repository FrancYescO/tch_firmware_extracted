...
local function get_usb_info()
      ...
      if key == "name" then
        ...
        if #partition_ids > 0 then
          for i=1, tonumber(v.partitionOfEntries) do
            ...
            if key == "fs" then
              if (not data[path] or data[path] == "") and value ~= ""then
                data[path] = value
              end
            else
  ...
  data["3g_pin"] = "********"
  data["samba_share_on"] = "lan"
  data["disk_protected"] = "0"
  data["disk_username"] = ""
  data["disk_password"] = ""
  data["usb_status"] = "end"
  return data
...
service_lan_status.set = function(args)
  ...
  paths[lan_map.dhcpState] = args.DHCP_enabled == "1" and "0" or "1"
  paths[lan_map.dhcpStart] = tostring(start)
  paths[lan_map.dhcpLimit] = tostring(limit)
  paths[lan_map.leaseTime] = args.DHCP_duration
  dm.set(paths)
  dm.apply()
  return true
end
