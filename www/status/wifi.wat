...
local WLTheOther = {
  ["0"] = "rpc.wireless.radio.@radio_2G.admin_state",
  ["1"] = "rpc.wireless.radio.@radio_5G.admin_state",
}
...
local function set_sec_info(args, id)
  ...
  local status = {
    the_other = WLTheOther[id],
    wps_2g = "rpc.wireless.ap.@ap4.wps.admin_state",
    wps_5g = "rpc.wireless.ap.@ap0.wps.admin_state",
  }
  content_helper.getExactContent(status)
  if (args[key] == "0" and status.the_other == "0" and (status.wps_2g == "1" or status.wps_5g == "1")) then
...
local service_wl_wps_status = {
  ...
  get = function()
    local status = {
      radio_2g = "rpc.wireless.radio.@radio_2G.admin_state",
      radio_5g = "rpc.wireless.radio.@radio_5G.admin_state",
      wps_2g = "rpc.wireless.ap.@ap4.wps.admin_state",
      wps_5g = "rpc.wireless.ap.@ap0.wps.admin_state",
    }
    content_helper.getExactContent(status)
    if (status.radio_2g == "0" and status.radio_5g == "0" and (status.wps_2g == "1" or status.wps_5g == "1")) then
      wps_status.gui_enabled = "0"
    else
      wps_status.gui_enabled = "1"
    end
    if status.wps_2g == "1" or status.wps_5g == "1" then
