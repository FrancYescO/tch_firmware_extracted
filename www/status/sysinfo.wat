...
local function getSysInfo()
  ...
  local fw_content = {
    ...
    version_override = "uci.versioncusto.override.fwversion_override",
  }
  if fw_content["firmware_version"] then
    local version = content["fw_version"]
    if fw_content["fwversion_prefix"] ~= "" then
      content["fw_version"] = fw_content.fwversion_prefix .. sub(version, 1, find(version, "%-[-]*$") - 1) .. fw_content.fwversion_suffix
      if fw_content.fwversion_override then
        if fw_content.fwversion_override == "override1" then
          content["fw_version"] = fw_content.fwversion_prefix .. sub(version, 1, find(version, "%-") -1) .. fw_content.fwversion_suffix
        end
      end
    end
  ...
  if find(content_wan.wan_ll_intf, "atm_") == 1 then
    wan_type = "ADSL"
