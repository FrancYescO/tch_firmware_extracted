...
function generateApplicationIframe(id, callback) {
  ...
  $(frame).attr("src", "/apps/" + id + "/desktop/index.html??v=<%= dumaos_version %>&lang=<%= lang %>&theme=<%= current_theme %>&themeVersion=<%= current_theme_version %>");
...
function bindDumaosSuspend(){
  $("#dumaos-suspend").click(function(){
    window.location.replace("/cgi-bin/restart.htm?cache=0&STOP=true")
  })
}
