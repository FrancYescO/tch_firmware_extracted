...
local function retries_left_reduce(oldunlock, oldunblock)
	local newunlock = utils.getContent("rpc.mobiled.device.@1.sim.pin.unlock_retries_left")
	local newunblock = utils.getContent("rpc.mobiled.device.@1.sim.pin.unblock_retries_left")
	if newunlock.unlock_retries_left < oldunlock .unlock_retries_left or newunblock.unblock_retries_left < oldunblock.unblock_retries_left then
		return false
	end
	return true
end
local function execute_action(post_data)
	local ret, msg
	local unlock_retries = utils.getContent("rpc.mobiled.device.@1.sim.pin.unlock_retries_left")
	local unblock_retries = utils.getContent("rpc.mobiled.device.@1.sim.pin.unblock_retries_left")
	...
	elseif post_data["action"] == "change" then
		...
		proxy.set("rpc.mobiled.device.@1.sim.pin.change", post_data["old_pin"] .. ',' .. post_data["new_pin"])
		ret = retries_left_reduce(unlock_retries, unblock_retries)
		if ret ~= true then
	...
	elseif post_data["action"] == "disable" then
		...
		proxy.set("rpc.mobiled.device.@1.sim.pin.disable", post_data["pin"])
		ret = retries_left_reduce(unlock_retries, unblock_retries)
		if ret ~= true then
	...
	elseif post_data["action"] == "enable" then
		...
		proxy.set("rpc.mobiled.device.@1.sim.pin.enable", post_data["pin"])
		ret = retries_left_reduce(unlock_retries, unblock_retries)
		if ret ~= true then
	...
	elseif post_data["action"] == "unlock" then
		...
		proxy.set("rpc.mobiled.device.@1.sim.pin.unlock", post_data["pin"])
		ret = retries_left_reduce(unlock_retries, unblock_retries)
		if ret ~= true then
	...
	elseif post_data["action"] == "unblock" then
		...
		proxy.set("rpc.mobiled.device.@1.sim.pin.unblock", post_data["pin"] .. ',' .. post_data["puk"])
		ret = retries_left_reduce(unlock_retries, unblock_retries)
		if ret ~= true then
