...
$("select").on('change', function() {
  ...
  var broadcast_value = $( "#ap_broadcast_ssid"+apVal ).val();
  if (this.value == "wep"){
    $(".wep_warning"+apVal).removeClass("hide").addClass("show")
    $(".broadcast_ssid_warning"+apVal).removeClass("show").addClass("hide")
  }
  else{
    $(".wep_warning"+apVal).removeClass("show").addClass("hide")
...
hideWEP();
var bandsteer_toggle_exists = ($("#band_steer_enabled").length)
if (!bandsteerSupport && multiap_enabled == "false" && bandsteer_toggle_exists != "0") {
...
function loadBroadCastWarning(){
  $('.broadcast_switch_class').click(function() {
    ...
    var wep_warning = $(".wep_warning"+apVal).hasClass("show")
    if(value == "0" || wep_warning){
      $(".broadcast_ssid_warning"+apVal).removeClass("show").addClass("hide")
      if (enableWPS) {
        $(".broadcast_ssid_warning_on"+apVal).removeClass("hide").addClass("show")
      }
    }
    else{
      $(".broadcast_ssid_warning"+apVal).removeClass("hide").addClass("show")
      if (enableWPS) {
        $(".broadcast_ssid_warning_on"+apVal).removeClass("show").addClass("hide")
      }
    }
  });
}
