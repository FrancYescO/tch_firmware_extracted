...
var validations = {
  ruleName: validateStringLength(1, 63),
...
$(function() {
  ...
  $("#psm-btn-add, #psm-btn-mobadd").click(function() {
    $("#psm-sel-day").val("Individual Days").trigger("chosen:updated");
  ...
  $("#psm-btn-save").click(function() {
    ...
    $('<tr class=\"cloned\" style=\"display: table-row;\" data-type=\"add\">\
      <input type=\"hidden\" class=\"RowIndex\" value=""/>\
 ...
 //params.push({name: "tableRequest", value:tableData}, {name: "status", value: statusButton}, {name: "CSRFtoken", value:$("meta[name=CSRFtoken]").attr("content") })
   params.push({name: "tableRequest", value:tableData}, {name: "status", value: statusButton}, {name: "CSRFtoken", value:tokenCSRF })
   postHandle("modals/schedule.lp", params, true);
  });
  ...
  $("#psm-tbl-mobschedule").on("click", ".button-delete", function() {
    ...
    $("#psm-tbl-desschedule .button-delete").eq(thisIndex).trigger("click");
    noRuleMessage("psm-tbl-desschedule");
  });
  $("#global-cancel").click(function() {
    $("#content").load("/modals/schedule.lp");
  });
...
function postHandle(target, params) {
  $.post(target, params, function(responseText, status) {
    ...
    if (responseText.status === "success") {
      setTimeout(function(){
      $("#content").load("/modals/schedule.lp");}, 3000);
    } else {
