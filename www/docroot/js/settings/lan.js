...
if (vdfVariant =="NZ" || $("#home-sel-mode").val() == "35") {
  $("#ipv4-div-dnsServer, #ipv4-div-dnsProxy").removeClass("hidden");
} else {
  $("#ipv4-div-dnsServer, #ipv4-div-dnsProxy").addClass("hidden");
}
...
function getStatus(target){
  $.get(target, function(responseTxt, statusTxt, xhr){
    if ( isDone || !(responseTxt.status != "") ){
      ...
      window.location = location.protocol+"//"+ipaddress;}, 5000);
      return;
    }else {
      window.setTimeout(function(){
      getStatus("modals/settings/lan.lp?getstatus=true");}, 5000);
    }
  }).fail(function(data){
    isDone = true;
    window.setTimeout(function(){
    getStatus(location.protocol+"//"+ipaddress+"/login.lp?rb=1")}, 5000);
  });
}

var isIPChanged, ipstart, ipend, lanhostname, domainname;
var areChangesDone = false;

$(document).on('change', 'input,select', function(){
  areChangesDone = true;
});
$("#ipv4-btn-dhcpEnabled, #ipv4-btn-dnsProxy, .button-delete").click(function(){
  areChangesDone = true;
});
...
$(function() {
  ...
  $("#global-apply, #modal-apply").click(function(){
    ...
    var dhcpv4State = $("#ipv4-btn-dhcpEnabled").hasClass("button-on") ? "" : "disabled";
    var params = [];
    ...
    if (vdfVariant =="NZ" || $("#home-sel-mode").val() == "35") {
      params.push({name: "dnsCheck", value: "1"});
    ...
    if  (isIPChanged == "1"){
      ...
      postHandler(target, params, "json");
    }
  });
  ...
  $("#ipv4-tbl-staticdhcp").on("click",".button-edit",function(){
    ...
    var rowEditIndex = element.find(".row-index");
    var mac;
    var macFound = false;
  ...
  $("#global-cancel").click(function(){
    ...
    $("#content").load("/modals/settings/lan.lp");
  });
