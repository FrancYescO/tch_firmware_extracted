...
$("#global-apply").click(function() {
  ...
  var validations = {
    ...
    ingressGain: portValidation,
    egressGain: portValidation,
  ...
  if ((regExForPort.test(rtpMin) == false) || (Number(rtpMin) < 0) || (Number(rtpMin) > portRange ) || (Number(rtpMin) >= Number(rtpMax))) {
    classCheck = 1;
    $("#phnset_btn_rtpmin").addClass("input-error");
  } else {
    $("#phnset_btn_rtpmin").removeClass("input-error");
  }
  if ((regExForPort.test(rtpMax) == false) || (Number(rtpMax) < 0) || (Number(rtpMax) > portRange ) || (Number(rtpMax) <= Number(rtpMin))) {
    classCheck = 1;
  ...
  $(".phone-settings-profile").each(function(index){
    ...
    if (uriSelect[i] != uri[i])
    {
	  ...
	  {
            classCheck = 1;
            $("#phnset_num_txt"+i+"").addClass("input-length");
          }
          else{
            $("#phnset_num_txt"+i+"").removeClass("input-length");
          }
        }
      }
    }
