function cap2info(x){
  ...
  if (xhex & parseInt(60000000,16)) type = type + "802.11ax";
  var ant = ""
  if (xhex & parseInt(10,16)) ant = ant + " 1x1";
  if (xhex & parseInt(20,16)) ant = ant +" 2x2";
  if (xhex & parseInt(40,16)) ant = ant +" 3x3";
  if (xhex & parseInt(80,16)) ant = ant +" 4x4";
  if (xhex & parseInt(8000000, 16)) ant = ant +" 8x8";
  var Features = ""
  ...
  if (xhex & parseInt(80000, 16)) Features = Features +" AMSDU";
  if (xhex & parseInt(400000, 16)) Features = Features +" MUMIMO";
  var Bond = ""
  if (xhex & parseInt(200,16)) Bond = Bond + " 40MHz";
  if (xhex & parseInt(10000,16)) Bond = Bond + " 80MHz";
  if (xhex & parseInt(10000000, 16)) Bond = Bond + " 160MHz";
  var SGI = ""
...
function get_random_color() {
  ...
  function initializeAnalyzer() {
    ...
    $("#key").empty().append(
      ...
      if (radio != "radio_2G") { $("#key").append($("<div>").addClass("col_str").text(eightyMHz)); $("#key").append($("<div>").addClass("col_str").text(onesixtyMHz)); }
      $("#key").append($("<div>").addClass("col_rssi").text(signalStrength),$("<br/>"));
  }

  function generateGraph(ssid, channel, rssi, cap, sec, macaddr, chan_descr, extra, channeldisplay, description){
    ...
    if (graphGeneration && isextenderSupported) {
      ...
      */
      ...
      var str80 = "";
      var str160 = "";
      var chan_descr80 = chan_descr.match(/(u)|(l)|(\/80)/g);
      var chan_descr160 = chan_descr.match(/(u)|(l)|(\/160)/g);
      if (chan_descr == "u" || chan_descr == "l") {
        Mhz = 2;
        Str = '*';
        str40 = "*";
      } else if (chan_descr80 == "/80") {
        Mhz = 4;
        str80 = "*";
      } else if (chan_descr160 == "/160") {
        Mhz = 8;
        str160 = "*";
      }
   }
    ...
    if (channelList && isextenderSupported) {
      ...
      for (i =0; i <= chlist160.length; i++) {
        if (channel == chlist160[i] ) {
          str160 = "*";
          Mhz = 8;
        }
      }
    ...
    var xcoord = 0
    if  (channel <= 64) {
       xcoord = 90+((channel-36)*7.5);
    }else{
       xcoord = 390+((channel-100)*7.5);
    }
	...
	channel = channel_text;
     ...
     img.src = "img/button-info-desktop.png";

     //Creating JSON object so as to get the details of each ssid dynamically.
     var dataobj = {"ssid":ssid,"channel":channeldisplay,"rssi":rssi,"cap":cap,"sec":sec,"macaddr":macaddr, "chan_descr":chan_descr, "description":description}
     ...
     (radio == "radio_2G") ? $(row).append($("<div"+divinfo+">").addClass("col_str").text(Str)) : $(row).append($("<div"+divinfo+">").addClass("col_str").text(str40),$("<div"+divinfo+">").addClass("col_str").text(str80), $("<div"+divinfo+">").addClass("col_str").text(str160));
  ...
  function deviceInfo(ssid){
    ...
    $("#ssidName").text(objDetails.ssid).css("word-break", "break-all");
    ...
    if (objDetails.description == "OWN_SSID"){
      $("#signalStrength").closest('.deviceinfoline').hide();
    }
    else {
      $("#signalStrength").closest('.deviceinfoline').show();
    }
    $("#protMode").text(secmode.replace("WPAWPA","WPA"));
    var channelDescr = objDetails.chan_descr;
    var channelWidth = "20 Mhz";
    if(channelDescr){
      channelDescr = channelDescr.toString();
  ...
  $("div[id^='rescan-cancel_']").click(function() {
    ...
    $(document).ready(function(){
      var target = "/modals/wireless-radio-modal-newEM.lp";
      $("div[id^='rescan-confirm_']").on("click",function(a) {
        ...
        $.post(target, params, function(acsData){
          tch.removeProgress();
          initializeAnalyzer();
          if ( acsData.length > 0 ) {
	      ...
	      generateGraph(data.ssid, data.channelCentre, data.rssi, data.cap, data.sec, macaddr, data.chan_descr, data.channelExtra, data.channel, description);
            });
          }
        });
      });
      initializeAnalyzer();
      $.get(target+"?isWifiAnalyzer="+getWifiAnalyzer+"&getAcsData=true&radio="+radio,function(acsData){
