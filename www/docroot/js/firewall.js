...
if ($("#src_ip_v6").val() == "custom") {
  $("#src_ip_v6").replaceWith($('<input/>',{'type':'text', 'name':'src_ip_v6','id' : 'src_ip_v6Input', 'class':'span2'}));
}
...
function portChange(portName,portId){
  $("[name = " + portName + "]").change(function () {
    if (((this.value) == "custom") && (portName == "src_ip" || portName == "dest_ip" || portName == "src_ip_v6" || portName == "dest_ip_v6")) {
...
$('table tr').each(function(){
  if ($(this).hasClass('line-edit')) {
    $(this).children('td').children('.switch').css({"pointer-events":"all","opacity":"1"})
  }else{
    $(this).children('td').children('.switch').css({"pointer-events":"none","opacity":"0.5"})
  }
})
