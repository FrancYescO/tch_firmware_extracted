...
ipv6_prefix();
$("#dhcpv6State").change(function(){
  ipv6_prefix();
});

function public_lan_disable() {
  if ($("#public_lan").parent().hasClass("active")) {
    $("#Hide_Advanced_id").prop("disabled", true)
  } else {
    clearInterval(lan_interval)
  }
}
var lan_interval = setInterval(public_lan_disable, 1000)
