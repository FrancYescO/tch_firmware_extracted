...
function get_random_color() {
  ...
  function initializeAnalyzer() {
    ...
    if (radio == "radio_2G") {
      var chn_diff = 50;
      for (i = 1; i<=13; i++) {
        ctx.moveTo((i*chn_diff)+30, 350);
        ctx.lineTo((i*chn_diff)+30, 355);
        ctx.stroke();
        ctx.font="9px Arial";
        ctx.fillText(i.toString(), 27+(chn_diff*i), 370);
      }
    } else {
      var chn_diff = 30;
      var chnl = 0;
  ...
  function generateGraph(ssid,channel,rssi,cap, chan_descr,macaddr, sec){
    ...
    ctx.moveTo(xcoord-(40*Mhz), 350);
    ctx.quadraticCurveTo(xcoord, get_curve_y_from_sig(maximum, minimum, range, sig), xcoord+(40*(Mhz)), 350);
    ...
    if (sig > maximum){sig = maximum }else if (sig < minimum){sig = minimum}
     ...
     var dataobj = {"ssid":ssid,"channel":channel,"rssi":rssi,"cap":cap,"chan_descr":channel_descr,"macaddr":macaddr,"sec":sec}
     var dataString = JSON.stringify(dataobj);

     // Add columns to the new row
     var divinfo = " data-toggle='tooltip' title='cap2info(cap)' data-placement='right'";
     $(row).append($("<div"+divinfo+">").addClass("col_ssid").text(ssid), $("<div"+divinfo+">").addClass("col_channel").text(channel));
        ...
        $.post("/modals/wireless-modal.lp", params, function(acsData){
          ...
          if ( acsData.length > 0 ) {
            $.each(acsData, function(i, data){
              ...
              generateGraph(data.ssid,data.channel,data.rssi,data.cap,data.chan_descr,macaddr,data.sec);
            });
          }
        });
      });
