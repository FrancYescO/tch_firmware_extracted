  $(function() {
    ...
    $("#password_apply").click(function() {
      ...
      if ($.inArray(newPassword, invalidKeys) >= 0) {
        ...
        return false;
      } else if(asciiLevel < 2 && hexLevel != 3){
        $(".weak-password").css("display", "block");
        return false;
      } else {
        if (wifiType  == "24G") {
  ...
  $("#global-apply").click(function() {
    ...
    var password5 = $("#wifiGen-txt-pwd5").val();
  ...
  function passwordValidate(password) {
    ...
    if ($.inArray(password, invalidKeys) >= 0) {
      ...
      // if password contains valid Ascii characters then give 1 point
      if (password.match(/^[ -~]{8,63}$/)) {
        asciiLevel++;
      }
      // if password contains valid hexadecimal characters in a valid range then give 3 points
      if (password.match(/^[a-fA-F0-9]+$/) && (password.length == "64")) {
        hexLevel = 3;
      }
      if (asciiLevel != "0" || hexLevel != "0") {
        ...
        if (password.match(/[!@#$%^&*?_~\-()]/) && password.length != 64) {
          asciiLevel++;
        }
      }
    }
      if (asciiLevel <= "1" && hexLevel < "1") {
          $('#passStrength').removeClass();
          ...
          $("#password-span-strwk").removeClass("hide").addClass("show");
      } else if ((asciiLevel <= "3" && asciiLevel > "1") || (hexLevel == "3")){
          $('#passStrength').removeClass();
