...
local function set_wifi_doctor(x, enabled)
    ...
    x:set("wifi_conductor", "global", "enabled", enabled)
    x:commit("wifi_conductor")
    os.execute("/etc/init.d/wifi-conductor restart")
end
...
function M.mobiled_enable(runtime, enabled, mobileiface)
    ...
    if ims_session.session_id and ims_session.profile then
        ...
        if enabled == "1" then
            ...
            if ims_session.activated == false then
                ...
                if not mobilenet_profile_enabled(runtime) then
                    if not runtime.mobiled_ims_timer then
                        runtime.mobiled_ims_timer = mobiled_ims_timer_cb(runtime, "0", ims_session)
                    end
                end
            end
        else
    ...
    local fixedifaceIsUp = scripthelpers.checkIfInterfaceIsUp("wan")
    local mobileifaceIsUp = scripthelpers.checkIfInterfaceIsUp(mobileiface)
    -- when mobileifaceIsUp is false, send event mobiled interface is down
    if not mobileifaceIsUp then
    ...
    if fixedifaceIsUp then
        ...
        if voipiface ~= "wan" and check_voip_running() then
            set_voip_iface(x, "wan", "wan6", true)
        end
    elseif enabled == "1" and mobileifaceIsUp then
        -- TR069 over mobile
        if cwmpdiface ~= mobileiface4 then
            reload_cwmpd_timer_cancel(runtime)
        ...
        -- ddns over mobile
        if ddnsiface ~= mobileiface4 then
            set_ddns_iface(mobileiface4, mobileiface6)
        end

        -- upnpd over mobile
        if upnpdiface ~= mobileiface4 then
            set_upnpd_iface(mobileiface4)
        end

        if raiface ~= mobileiface4 then
            set_ra_iface(x, mobileiface4, mobileiface6)
        end

        -- voip over mobile
        if voiceOnFailover == "1" and voipiface ~= mobileiface4 and check_voip_running() then
            set_voip_iface(x, mobileiface4, mobileiface6)
        end
    end

    if mobileifaceIsUp or wifidoctorurl == "" then
        -- disable wifi-doctor
        if wifidoctor == "1" then
            set_wifi_doctor(x, "0")
        end
    elseif wifidoctor == "0" then
        -- enable wifi-doctor
        set_wifi_doctor(x, "1")
    end
end
...
function M.goto_mobile_mode(runtime)
    ...
    if not primary_wan_mode or primary_wan_mode:upper() ~= "MOBILE" then
        return false
    end

    local calls_ongoing = next(runtime.ubus:call("mmpbx.call", "get", {}) or {})
    if calls_ongoing then
        runtime.logger:notice("postponing switching to MOBILE primary WAN mode because a call is ongoing")
        return false
    end
