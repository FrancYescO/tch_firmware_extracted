...
--local dbg = io.open("/tmp/sle.txt", "a") -- "a" for full logging
...
function M.start(cb)
    ...
    events['network.interface'] = function(msg)
        if msg ~= nil and msg.interface ~= nil then
            ...
            if wanevent == true then
                if msg.action:gsub('[^%a%d_]','_') == "ifup" then
                  cursor:load("env")
                  if (cursor:get("env", "custovar", "ONTmode") == "1") then
                    cb('gpon_ploam_5')
                    if led.broadband.status == "no_line" or led.broadband.status == "sync" or led.broadband.status == "initial" then
                        update_led_status(cb, "broadband", "link_connected", "persistent", "green-solid")
                        linkstatus = "up"
                    end
                  else
    ...
    events['gpon.ploam'] = function(msg)
			--dbg:write("TRIGGER GPON\n")
			--dbg:flush()
        if msg ~= nil and msg.statuscode ~= nil then
					--dbg:write("msg not nil\n")
					--dbg:flush()
					--dbg:write("msg.statuscode: ".. msg.statuscode .."\n")
					--dbg:flush()
            if msg.statuscode ~= 5 then
							--dbg:write("msg.statuscode ~= 5\n")
							--dbg:flush()
                    ...
                    if gponcode >= 2 and gponcode <= 4 then
                        ...
                        linkstatus = "up"
											...
											if (cursor:get("env", "custovar", "ONTmode") == "1") then
												...
												--dbg:flush()
											cb('gpon_ploam_10')
												update_led_status(cb, "broadband", "no_line", "persistent", "red-blink")
                        linkstatus = "down"
											else
											--dbg:write("RED solid\n")
												--dbg:flush()
                        ...
                        linkstatus = "down"
							...
							cursor:load("env")
								--dbg:write("ONTMode = " .. cursor:get("env", "custovar", "ONTmode") .. "\n")
								--dbg:flush()
								if (cursor:get("env", "custovar", "ONTmode") == "1") then
									--dbg:write("ONTMode = 1\n")
									--dbg:flush()
    ...
    events['gpon.omciport'] = function(msg)
      cursor:load("env")
        if (cursor:get("env", "custovar", "ONTmode") == "0") then
          if msg ~= nil and msg.statuscode ~= nil then
