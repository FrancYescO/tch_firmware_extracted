...
local rssi_value_is_too_good, rssi_value_is_good, rssi_value_is_bad
...
rssi_value_is_too_good, err = cursor:get(ledcfg,'rssi_values','too_good')
rssi_value_is_good, err = cursor:get(ledcfg,'rssi_values','good')
rssi_value_is_bad, err = cursor:get(ledcfg,'rssi_values','bad')
...
backhaul_link, err=cursor:get(multiapcfg,'agent','backhaul_link')
...
local function get_backhaul_link()
  local config = "multiap"
  local x = uci.cursor(nil,"/var/state")

  x:load(config)
  backhaul_link, error = x:get("multiap","agent","backhaul_link")
  x:close()
...
--]]
  ...
  events['wireless.supplicant'] = function(msg)
      if msg ~= nil and msg.role ~= nil then
          if msg.role == "ap" or msg.role == "repeater" then
             cb("wifi_state_on")
          end
      end
  end
  ...
  local function read_rssi()
    ...
    if endpoint_data and next(endpoint_data) then
      ...
      if rssi and rssi > tonumber(rssi_value_is_too_good) then
         cb('too_good_connection')
      elseif rssi and (rssi <= tonumber(rssi_value_is_too_good) and rssi > tonumber(rssi_value_is_good)) then
         cb('good_connection')
      elseif rssi and (rssi <= tonumber(rssi_value_is_good) and rssi > tonumber(rssi_value_is_bad)) then
         cb('average_connection')
      elseif rssi and rssi <= tonumber(rssi_value_is_bad) then
         cb('bad_connection')
      end
  ...
  events['map_agent.onboarding_event'] = function(msg)
    if easymesh_enabled and show_easymesh_status_enabled and msg ~= nil and msg.status ~= nil then
	  ...
	  get_backhaul_link()
          if gateway_l2interface:match("^eth") or backhaul_link:match("^eth")  then
              onboarded_rssi = true
          end
