...
local ledcfg='ledfw'
lcur:load(ledcfg)
local info_service_led_timeout, service_led_if_mobile, statusled_enabled, err
...
events['wireless.ssid'] = function(msg)
    if msg ~= nil then
        local radio = conn:call("wireless.radio", "get", {})
        if radio == nil then
            error("Cannot retrieve wireless radio")
        end
        local ssid = conn:call("wireless.ssid", "get", {})
        if ssid == nil then
            error("Cannot retrieve wireless ssid")
        end
        if radio["radio_2G"].oper_state == 1 and (msg.name == "wl0_1" and ssid["wl0"].oper_state == 0 or msg.name == "wl0" and ssid["wl0_1"].oper_state == 1) then
            local packet = {}
            packet["ifname"] = "wl0_1"
            packet["radio_admin_state"] = radio["radio_2G"].admin_state
            packet["radio_oper_state"] = radio["radio_2G"].oper_state
            packet["bss_admin_state"] = ssid["wl0_1"].admin_state
            packet["bss_oper_state"] = ssid["wl0_1"].oper_state
            packet["acl_state"] = 0
            packet["sta_connected"] = 0
            packet["security"] = "wpapsk"
            conn:send("wireless.wlan_led", packet)
        end
    end
end
