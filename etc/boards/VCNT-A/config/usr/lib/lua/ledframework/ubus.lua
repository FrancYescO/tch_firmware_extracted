...
--               cb('hostman_voip_down')
    ...
    events['gpon.ploam'] = function(msg)
        if msg ~= nil and msg.statuscode ~= nil then
            cb('gpon_ploam_O' .. msg.statuscode)
        end
    end
    ...
    events['usb'] = function(msg)
        if msg ~= nil and msg.event ~= nil and string.match(msg.dev_desc, "-") then
            cb(msg.event)
        end
    end
   ...
   events['mobiled'] = function(msg)
       if msg and type(msg) == "table" and msg.event == "device_removed" then
           cb("mobile_off")
       end
   end
    ...
    events['mmpbx.dect.paging'] = function(msg)
        if msg ~= nil then
    ...
    events['mmpbx.dect.callstate'] = function(msg)
        if msg ~= nil then
...
--  ********* onboarding timeout
    local onboarding_discovery_timeout=uloop.timer(function () cb('wfd_no_peer');end)
    local onboarding_error_timeout=uloop.timer(function () cb('wfd_onboarding_idle');end)
    local onboarding_success_timeout=uloop.timer(function () cb('wfd_onboarding_idle');end)
    ...
    events['wireless.wps_led'] = function(msg)
        if msg ~= nil and msg.wps_state ~= nil then
            ...
            if msg.wps_state == 'success' then
                onboarding_discovery_timeout:set(16000)
            end
        end
    end

    events['wifidoctoragent.onboarding'] = function(msg)
        if msg then
           if msg.event == "inprogress_peerdetected" then
              cb('wfd_onboarding_inprogress')
           elseif msg.event == "error" then
              cb('wfd_onboarding_error')
              onboarding_error_timeout:set(120000)
           elseif msg.event == "success" then
              cb('wfd_onboarding_success')
              onboarding_success_timeout:set(300000)
           end
        end
    end
    ...
    events['led.night_mode'] = function(msg)
      if msg ~= nil and msg.state ~= nil then
          cb('night_mode_' .. msg.state)
      end
    end
