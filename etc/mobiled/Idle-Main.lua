...
function M.check(runtime, event, dev_idx)
    ...
    if event.event == "timeout" then
        ...
        for _, session in pairs(dataSessionList) do
            ...
            if info then
                if info.session_state == "disconnected" or info.session_state == "" then
        ...
        elseif require('mobiled.scripthelpers').startswith(event.event, "session_") then
        ...
        elseif (event.event == "session_config_changed") then
            retState = "DataSessionSetup"
        elseif (event.event == "session_teardown") then
            retState = "DataSessionSetup"
        elseif (event.event == "session_setup") then
            retState = "DataSessionSetup"
