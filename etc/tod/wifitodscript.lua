                ...
                if value_in_wireless ~= value_to_operate then
                    ...
                    if option == "state" then
                      ...
                      if (ap_name == "ap0" or ap_name == "ap1") and value_to_operate == "0" then
                        cursor:set("wireless", ap_name, "bandsteer_id", "bs0")
                        cursor:set("wireless", "bs0", "state", "0")
                      end
                end
                    need_to_reload = true
                else
...
local function call_operation(start_or_stop, runtime, actionname, object)
    ...
    if need_to_reload then
        ...
        process.execute("/etc/init.d/hostapd", {"reload"})
        process.execute("/etc/init.d/network", {"reload"})
    end
