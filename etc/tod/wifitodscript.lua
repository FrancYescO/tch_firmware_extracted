...
function M.stop(runtime, actionname, object)
  ...
  local wifitod_dis = cursor:get("tod", "action_wifidisable", "enabled")
  if wifitod_dis == "1" then
    setStates(cursor, "1")
    refreshHostapd()
  end
  local periodic = cursor:get("tod", "timer_wifidisable", "periodic")
  if periodic == "0" then
    cursor:set("tod", "action_wifidisable", "enabled", "0")
    cursor:commit("tod")
  end
  return true
end
