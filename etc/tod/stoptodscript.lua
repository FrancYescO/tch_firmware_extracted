...
function do_action(runtime, actionname, object, start)
  ...
  if mac and activate and actionname then
    ...
    if not section then
      section = string.format("%s_%s", sectiontype, string.gsub(mac, ":", ""))
      cursor:set(config, section, sectiontype)
    ...
    if enabled ~= activate and (isroutine or not hasroutine) then
      cursor:set(config, section, "enabled", activate)
      reloadflag = true
    end

    if reloadflag then
      -- check device boost status and remove boost firstly
      if activate == "1" then
        local qos_section = nil
        cursor:foreach("qos", "classify", function(s)
          if s["srcmac"] == mac and s["target"] == "Boost" then
            qos_section = s[".name"]
            return false
          end
        end)
        if qos_section then
          cursor:delete("qos", qos_section)
          cursor:commit("qos")
          os.execute("/etc/init.d/qos reload; sleep 1")
          logger:notice("deactivate boost when boost is set in stop processing")
        end
      end
