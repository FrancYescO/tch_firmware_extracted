...
function M.check(runtime, l2type, event)
   ...
   if event == "timeout" then
      ...
      -- Check which L2type is used.
    if l2type == "ETH" then
            logger:notice("Running in "..l2type.." mode, so check opt125")
      ...
      -- Trigger WAN ifup if the option 125 is received but not configured
      if runtime.ubusReqOpt ~= nil and (opt125 == nil or opt125 == "0") then
         x:set("wansensing", "deployment", "opt125", "2")
         x:commit("wansensing")
         logger:notice("Option 125 received but MVLAN config not applied. Restart L2 config.")
         return "L2Sense"
      end

      local checkIPTV = x:get("network", "iptv", "auto")
      local checkVOIP = x:get("network", "voip", "auto")
      local checkMGMT = x:get("network", "mgmt", "auto")

      -- Trigger WAN ifup if the option 125 is not received but the rollback failed
      if runtime.ubusReqOpt == nil and (checkIPTV == "1" or checkVOIP == '1' or checkMGMT == '1') then
         x:set("wansensing", "deployment", "opt125", "2")
         x:commit("wansensing")
         logger:notice("Option 125 not received anymore switching to single VLAN via L2Sense")
         return "L2Sense"
       ...
       logger:notice("Running in "..l2type.." mode, so no check needed for opt125")
       if confg_dsl == "1" then
           logger:notice("Config "..l2type.." was done correctly and as configured_DSL_Network = 1")
       else
           logger:notice("Config "..l2type.." is NOT done correctly and go back L2Sense")
       return "L2Sense"
     end
    end
