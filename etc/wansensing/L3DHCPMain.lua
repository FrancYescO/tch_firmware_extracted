...
function M.check(runtime, l2type, event)
    ...
    if failoverhelper.goto_mobile_mode(runtime) then
        return "Mobile"
    end
    ...
    elseif event == "dhcp_renew_wan_failed" then
        logger:notice("DHCP renew failed on wan, to restart it")
	return "L3DHCPSense"
    elseif event == "dhcp_renew_wan6_failed" then
        logger:notice("DHCP renew failed on wan6, to restart it")

	-- MMPBX and CWMP will switch to wan6 intf by itself once it receives the wan6 down event
