...
local function checkDns(scripth, logger)
    ...
    if server_list ~= nil then
        ...
        do
            ...
            local status,hostname_or_error = scripth.dns_check('telstra.com',v,'telstra.com',nil,nil,nil,1,2)
            if status and hostname_or_error then
                return true
            end
        end
        logger:notice("Trying again - Launching DNS Request with GOOGLE DNS server")
        local status,hostname_or_error = scripth.dns_check('apple.com','8.8.8.8','apple.com',nil,nil,nil,1,2)
        if status and hostname_or_error then
            return true
        end
    else
        logger:notice("Launching DNS Request with default DNS server")
        local status,hostname_or_error = scripth.dns_check('telstra.com',nil,'telstra.com',nil,nil,nil,1,2)
        if status and hostname_or_error then
            return true
        end
    end
