...
local function non_novas_check(runtime, l2type, event)
    ...
    if event == "timeout" then
        ...
        elseif scripthelpers.checkIfCurrentL2WentDown(l2type, event, 'eth4') and PPPFailReason(conn, 'wan') ~= 'AUTH_TOPEER_FAILED' then
            return "L2Sense"
        end
    end
    return "L3PPPSense"
...
local function novas_check(runtime, l2type, event)
    ...
    if event == "timeout" or event == "network_interface_wan_ifup" or event == "network_interface_ppp_ifup" or event == "network_interface_ipoe_ifup" then
        if scripthelpers.checkIfInterfaceIsUp("wan") then
            logger:notice("The L3PPPSense main script sensed PPP no VLAN on l2type interface " .. tostring(l2type))
            return "L3PPP"
        end
        if scripthelpers.checkIfInterfaceIsUp("ppp") then
            logger:notice("The L3PPPSense main script sensed PPP VLAN 2 on l2type interface " .. tostring(l2type))
            return "L3Sense"
        end
        if scripthelpers.checkIfInterfaceIsUp("ipoe") then
            logger:notice("The L3PPPSense main script sensed DHCP on l2type interface " .. tostring(l2type))
            return "L3Sense"
        end
	else
        if l2type == 'ETH' then
