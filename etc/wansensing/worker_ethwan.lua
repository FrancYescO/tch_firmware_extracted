...
local function Toggle_ACS(runtime,New_Scenario)
	if New_Scenario == 1 then
		local handle = io.popen("ps | grep upgrade")
		local sysupgrade = handle:read("*a")
		sysupgrade = string.match(sysupgrade,".sysupgrade")
		if sysupgrade then
			return
		end
	end
...
-- set the ACS parameter of the new Sceanrio
	x:set("cwmpd", "cwmpd_config", "acs_url", oper_acs_url)
	x:set("cwmpd", "cwmpd_config", "acs_user", oper_acs_user)
	x:set("cwmpd", "cwmpd_config", "acs_pass", oper_acs_pass)
	x:set("cwmpd", "cwmpd_config", "connectionrequest_username", oper_connectionrequest_username)
	x:set("cwmpd", "cwmpd_config", "connectionrequest_password", oper_acs_connectionrequest_password)
	x:set("cwmpd", "cwmpd_config", "periodicinform_enable", oper_acs_periodicinform_enable)
	x:set("cwmpd", "cwmpd_config", "periodicinform_interval", oper_acs_periodicinform_interval)
	x:set("cwmpd", "cwmpd_config", "ssl_certificate", oper_acs_ssl_certificate)
	x:set("cwmpd", "cwmpd_config", "ssl_privatekey", oper_acs_ssl_privatekey)
...
-- Saving the standard Data-Vid to the variable, to assure possible changes during lifetime compared to factory default
	...
	x:set("dhcp", "dnsmasq_lan", "rebind_protection", "1")
...
-- Saving the standard Data-Vid to the variable, to assure possible changes during lifetime compared to factory default
		...
		x:set("dhcp", "dnsmasq_lan", "rebind_protection", "0")
