...
function M.exit(runtime, l2type, transition)
   ...
   if WS ~= "2" then
     ...
     local vlan = x:get("env", "var", "wan_vlan")
     if not vlan then 
        local default_vlan = x:get("env", "var", "wan_vlan_default")
        vlan = default_vlan
     end   
  
     if l2type == "ETH" then
        local mtu = x:get("env", "var", "mtu_eth")
     ...
     elseif l2type == "VDSL" then
        ...
        x:set("env", "var", "wan_vlan", vlan)
        x:set("env", "var", "wan_vlan_enabled", "1")
        x:set("env", "var", "setup", "VDSL")
        x:set("network", "bt_iptv", "auto", "1")
        x:set("ethoam", "config1", "vlan", vlan)
        x:set("ethoam", "config2", "vlan", vlan)
     ...
     elseif l2type == "ADSL" then
        local mtu = x:get("env", "var", "mtu_adsl")
        local vci = x:get("env", "var", "vci")   
        local vpi = x:get("env", "var", "vpi")
        local enc = x:get("env", "var", "enc")   
        local ulp = x:get("env", "var", "ulp")
        x:set("network", "wan", "ifname", "atm0")
        x:set("network", "wan", "proto", "pppoa")
        x:set("network", "config", "wan_mode", "pppoa")
     ...
     os.execute("/etc/init.d/ethoam restart")  --Restart as instructed as reload not working
     --Reenable the CWMPd if the env var is set
     local cwmpd_enabled = x:get("env", "var", "cwmpd_enabled")  
