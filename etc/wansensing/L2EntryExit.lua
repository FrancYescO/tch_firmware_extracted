...
function M.entry(runtime)
	...
	if origL2 == "SFP" then
          process.execute("/usr/bin/xdslctl", {"start"})
          process.execute("/usr/bin/xdslctl", {"connection", "--up"})
	end
...
function M.exit(runtime, l2type)
   ...
   if isSFP == "1" and (l2type == "SFP" or l2type == "VDSL" or l2type == "ADSL") then
      ...
      if lanwanmode ~= "1" then
        ...
        process.execute("/etc/init.d/ethernet", {"reload"})
      end
   end
...
logger:notice("FRS Timer-Interval: " .. check_tmrval)
      ...
      process.execute("/etc/init.d/pppoe-relay-tch", {"reload"})
...
-- Added for iptv interface, in case of l2type is not changed, iptv ifname should be updated
      local iptvdev = x:get("network", "iptv", "ifname")
      if iptvdev == "<set by script>" or iptvdev == "" then
        if l2type == "ETH" or l2type == "SFP" then
          iptvdev = x:get("network", "iptveth4", "ifname") and "iptveth4" or "eth4"
          x:set("network", "iptv", "ifname", iptvdev)
	elseif l2type == "VDSL" then
          iptvdev = x:get("network", "iptvptm0", "ifname") and "iptvptm0" or "ptm0"
          x:set("network", "iptv", "ifname", iptvdev)
	elseif l2type == "ADSL" then
          x:set("network", "iptv", "ifname", "atmiptv")
	end
          x:commit("network")
   ...
   if l2type == "ETH" or l2type == "SFP" then
      ...
      if isSFP == "1" and l2type == "SFP" then
         process.execute("/usr/bin/xdslctl", {"stop"})
      end
      process.execute("ifdown", {"wan"})
      process.execute("/etc/init.d/xtm", {"reload"})
