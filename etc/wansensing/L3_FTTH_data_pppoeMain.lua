...
local CurrentScenarioName="FTTH_data_pppoe" -- should match with ./lib/scenario.lua and config
...
function M.check(runtime, l2type, event)
   ...
   elseif UPevents[event] then
      ...
      -- ignore ifup on current scenario:
      if current_intf==InterfaceMAP[CurrentScenarioName] then
        runtime.logger:notice("L3_"..CurrentScenarioName.."Main("..l2type..", "..event.."): Ignoring up event on current interface.")
        return CurrentScenarioName
      end
