...
local CurrentScenarioName="ADSL_ull_data" -- should match with ./lib/scenario.lua
...
function M.entry(runtime, l2type)
   ...
   if InterfaceMAP[CurrentScenarioName] then
    ...
    --Enable QoS for ATM device atm_0_44
        x:set("qos", "atm_0_44", "enable", "1")
        x:commit("qos")
    ----Setup interface for VoIP static routes
        x:set("network" , "voip_staticroute", "interface" , "ull_voice")
        x:set("network" , "voip_staticroute2", "interface" , "ull_voice")
    --Setup correct source interface for auto_iptv
        x:set("network", "auto_iptv", "ifname", "atm_iptv")
    --Setup MMPBX on ull_voice interface (static IPoA)
        x:set("mmpbxrvsipnet", "sip_net", "interface", "ull_voice")
...
function M.exit(runtime, l2type)
   ...
   if existInterface(x,runtime,"dummy") then
    ...
    --Disable QoS for ATM device atm_0_44
        x:set("qos", "atm_0_44", "enable", "0")
        x:commit("qos")
    --Network commit for changes above
        x:commit("network")
        os.execute("/etc/init.d/network reload")
