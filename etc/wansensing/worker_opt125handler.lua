...
local logger = runtime.logger
	...
	elseif event == "network_interface_wan_ifup" and L2type then
		...
		if ubusReqOpt and wan_status.up and wan_status.proto=="dhcp" then
			...
			--Save ubusReqOpt(125) in a runtime var for periodic config check in L3Main
			runtime.ubusReqOpt = ubusReqOpt
			...
			if mgmtvid ~= "" and mgmtvid ~= nil and mgmtvid ~= "-1" then
			...
			if iptvvid ~= "" and iptvvid ~= nil and iptvvid ~= "-1" then
			...
			if voipvid ~= "" and voipvid ~= nil and voipvid ~= "-1" then
			...
			x:set("wansensing", "deployment", "opt125", "1")
			...
			logger:notice("Option 125 received and MVLAN config applied")
