...
createrule (){
   ...
   for arg; do
      ...
      local FWD_RULE="-t nat $ACT prerouting_rule -m tcp -p tcp --src "$arg" --dst $WAN_IP --dport $WAN_PORT -j REDIRECT --to-ports $LAN_PORT"
      local FWD_NULL="-t nat $ACT prerouting_rule -p tcp --src "$arg" --dst $WAN_IP --dport $LAN_PORT -j REDIRECT --to-port 65535"
      local ACCEPT_RULE="-t filter $ACT input_rule -p tcp --src "$arg" --dst $WAN_IP --dport $LAN_PORT -j ACCEPT"
