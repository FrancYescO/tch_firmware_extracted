...
oldConfig:foreach("tod", "host", function(s)
  local sectionname = s._key or s[".name"]
  newConfig:set('tod', sectionname, "host")
  for k,v in pairs(s) do
    if not k:find("^%.") then
      newConfig:set("tod", sectionname, k, v)
    end
  end
end)
...
oldConfig:foreach("tod", "action", function(s)
  ...
  local ssidValue = oldConfig:get("tod", ap[1], "ssid")
  newConfig:set("tod", "ap"..unique, "ap", apname)
  newConfig:set("tod", "ap"..unique, "state", state)
  if ssidValue then
    newConfig:set("tod", "ap"..unique, "ssid", ssidValue)
  else
    newConfig:set("tod", "ap"..unique, "ssid", "all")
  end
