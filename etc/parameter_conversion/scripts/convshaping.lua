...
oldConfig:foreach("qos", "class", function(s)
    ...
    if s.pbr ~= nil then
      newConfig:set("qos", name, "max_bit_rate", s.pbr)
    ...
    if s.max_burst_size then
      newConfig:set("qos", name, "max_burst_size", s.max_burst_size)
    end
    if s.max_bit_rate then
      newConfig:set("qos", name, "max_bit_rate", s.max_bit_rate)
    end
    if s.min_bit_rate then
      newConfig:set("qos", name, "min_bit_rate", s.min_bit_rate)
    end
end)
