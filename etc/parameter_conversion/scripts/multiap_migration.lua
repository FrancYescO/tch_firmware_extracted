...
local freq_bands = o:get('multiap', 'cred0', 'frequency_bands')
...
if old_map_controller == "0" and old_map_agent == "0" then
  ...
  if old_layout_enabled == "nil" then
    old_layout_enabled = "0"
  end
  local mainSplit
  ...
  local merge_enabled = "0"
  if (old_wl0_ssid == old_wl1_ssid and old_ap0_key == old_ap1_key and old_ap0_secmode == old_ap1_secmode) then
    merge_enabled = "1"
  end

 --NG-223739 If ssid, key, and mode are same for 2.4GHz and 5Ghz then merge mode will shown.

  if (merge_enabled == "1" and string.match(freq_bands, "radio_5G")) or (merge_enabled == "1" and old_layout_enabled ~= "nil" and old_layout_enabled == "0") then
    n:set('multiap','cred1','state','0')
    n:set('multiap','cred1','operational_state','0')
    n:set('multiap','cred0','frequency_bands','radio_2G,radio_5Gu,radio_5Gl')
  else
    n:set('multiap','cred1','state','1')
    n:set('multiap','cred1','operational_state','1')
