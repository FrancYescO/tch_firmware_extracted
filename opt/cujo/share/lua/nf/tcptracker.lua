...
-- Listening for TCP SYN packets and reporting them to userspace in aggregates
-- in "tcptracker" events.

local conns = {0}
for i = 1, config.tcptracker.maxentries do
...
local function send()
    ...
    if nf.send('tcptracker', pack) then
        conns[1] = 0
    else
...
-- luacheck: globals nf_tcptracker
local kf = {nil, nil, nil, nil}
function nf_tcptracker(frame, packet)
    if not tim then
        tim = timer.create(config.tcptracker.pollinterval * 1000, send)
    end
