...
function module.onconnect()
    connected = true
end

function module.ondisconnect()
    cujo.safebro.trackerblock.configure(false)
    connected = false
end

function module.onhibernate()
    local config = cujo.safebro.getconfig()
    if cujo.safebro.trackerblock.enable.enabled and
        config and config.trackerblock then
        wakereport = true
    end
end

function module.onwakeup()
    if not cujo.safebro.trackerblock.enable.enabled then
        wakereport = false
    end
end

function module.configure(enabled, settings)
    if enabled and settings and settings.trackerblock then
        if not connected then
            ...
            flushstats()
            cancel = shims.create_stoppable_timer('trackerblock-reporter', period, function()
                sendreport()
                return period
            end)
...
function module.initialize()
    cujo.safebro.trackerblock.stats:subscribe(function (message)
        collectstats(message.mac, message.uri, message.categories)
    end)

    cujo.nf.subscribe('trackerblock', cujo.safebro.trackerblock.stats)

    cujo.cloud.onconnect:subscribe(cujo.safebro.trackerblock.onconnect)
    cujo.cloud.ondisconnect:subscribe(cujo.safebro.trackerblock.ondisconnect)
    cujo.cloud.onhibernate:subscribe(cujo.safebro.trackerblock.onhibernate)
    cujo.cloud.onwakeup:subscribe(cujo.safebro.trackerblock.onwakeup)
end
