...
local multiap_enable = proxy.get("uci.multiap.controller.enabled")[1].value
...
if args[1] == "On" then
   ...
   if multiap_enable == "1" then proxy.set("uci.multiap.agent.enabled", "1") end
   ubus_conn:send("event",{ state="wifi_leds_on" })
elseif args[1] == "Off" then
   if multiap_enable == "1" then proxy.set("uci.multiap.agent.enabled", "0") end
