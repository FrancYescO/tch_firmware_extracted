...
_COMPANY_NAME=$(uci_get_3_opt wireless.global.wfa_manufacturer env.var.company_name)
_PROD_NAME=$(uci_get_3_opt wireless.global.wfa_model_name env.var.prod_name)
_PROD_NUMBER=$(uci_get_3_opt wireless.global.wfa_model_number env.var.prod_number)
_PROD_FRIENDLY_NAME=$(uci_get_3_opt wireless.global.wfa_device_name env.var.prod_friendly_name)
#variant_friendly_name does not always exist -> fallback to prod_friendly_name
_VARIANT_FRIENDLY_NAME=$(uci_get_3_opt wireless.global.wfa_friendly_name env.var.variant_friendly_name env.var.prod_friendly_name)
...
_SW_VERSION=`uci get version.@version[0].version`

#Get mac and replace : by -
_MACADDR=`uci get env.rip.eth_mac | sed -e "s/:/-/g"`
_WL_MACADDR=`uci get env.rip.wifi_mac | sed -e "s/:/-/g"`

#the variables are interpreted as strings delimited by NEW LINE, so don't add trailing spaces to your definition
...
_MACADDR=$_MACADDR
_WL_MACADDR=$_WL_MACADDR
_SW_VERSION=$_SW_VERSION" > $ENV_FILE
