...
logger -s -t "firmware-upgrade-ext" "Timeout value set to ${timeout} seconds" > /dev/console 2>&1
while [ 1 ] ; do
...
if [ "${status}" = "failure" ] ; then
	...
	if [ ${boot_failure_count} -lt ${max_boot_attempts} ] ; then
		...
		logger -s -t "firmware-upgrade-ext" "Giving up trying to boot this bank after ${max_boot_attempts} attempts.  Quectel FW upgrade failed. Not reverting to other bank." > /dev/console 2>&1
		# Reset the boot failure count
		uci set system.fwupgrade.bootfailurecount="0"
		uci commit system
	else
