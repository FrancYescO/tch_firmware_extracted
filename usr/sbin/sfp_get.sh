...
for i in x x x x x x x x x x x x x  # at most 13 '-' type arguments
do
  case "$1" in
    -s) port=$2;
	shift;
...
if [ "$TOPIC" = "state" -o "$TOPIC" = "allstats" ] ; then
	echo `(echo "$diag_cmd"; sleep 0.5; echo "$onu_state_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd} | grep ONU` > /tmp/onu_state.txt
...
if [ "$TOPIC" != "state" -a "$TOPIC" != "optical" -a "$TOPIC" != "config" ] ; then

	echo `(echo "$diag_cmd"; sleep 0.5; echo "$opt_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd}` > /tmp/sfp_counter.txt
