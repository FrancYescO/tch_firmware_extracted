...
if [ "$TOPIC" = "ont_sn" ] ; then
	if [ $telnet_protected = 1 ]; then
		echo `(echo "$user"; sleep 1; echo "$password"; sleep 1; echo "$diag_cmd"; echo "$ont_sn_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd} | grep serial` > /tmp/ont_sn.txt
	else
		echo `(echo "$diag_cmd"; echo "$ont_sn_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd} | grep "serial number"` > /tmp/ont_sn.txt
        fi
	sed -i 's/\r/\n/g' /tmp/ont_sn.txt
...
if [ "$TOPIC" = "state" -o "$TOPIC" = "allstats" ] ; then
	if [ $telnet_protected = 1 ]; then
		echo `(echo "$user"; sleep 1; echo "$password"; sleep 1; echo "$diag_cmd"; echo "$onu_state_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd} | grep ONU` > /tmp/onu_state.txt
	else
...
if [ "$TOPIC" = "reset" ] ; then
	if [ $telnet_protected = 1 ]; then
		echo `(echo "$user"; sleep 1; echo "$password"; sleep 1; echo "$diag_cmd"; echo "$counter_reset"; sleep 1; echo "$exit_cmd";)|${connect_cmd}`> /tmp/reset.txt
	else
		echo `(echo "$diag_cmd"; echo "$counter_reset"; sleep 1; echo "$exit_cmd";)|${connect_cmd}`> /tmp/reset.txt
	fi
fi

if [ "$TOPIC" != "state" -a "$TOPIC" != "optical" -a "$TOPIC" != "config" -a "$TOPIC" != "reset" -a "$TOPIC" != "reboot" -a "$TOPIC" != "telnetTest" -a "$TOPIC" != "ont_sn" ] ; then
	if [ $telnet_protected = 1 ]; then
		echo `(echo "$user"; sleep 1; echo "$password"; sleep 1; echo "$diag_cmd"; echo "$opt_cmd"; sleep 1; echo "$exit_cmd";)|${connect_cmd}` > /tmp/sfp_counter.txt
	else
