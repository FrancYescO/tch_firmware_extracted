...
function print (...)
  ...
  local fd_log = io.open("/var/log/mobiledongle/fg.log", "a+")
  assert(fd_log)
  --fd_log:write("@@@ log ::" )
  for i=1, select ('#', ...) do
    local arg = select(i, ...)
...
if not mbd_uci_cfg_begin.parm then
  ...
  elseif mbd_uci_cfg_begin.option_ppp then
    ...
    M.at_data='ttyUSB2'
    M.qmi_ctrl=nil
    local rv = at_helper.exec_at_cmd("ttyUSB0", "ATI", "10")
    local model = string.match(rv, "MODEL: E1690")
    if model then
      M.at_ctrl='ttyUSB0'
      M.at_data='ttyUSB0'
    end 

    model = string.match(rv, "MODEL: E3131")                                
    if model then                                       
        print("Find Huawei E3131")      
        M.at_ctrl='ttyUSB2'                           
        if M.at_data then M.at_data='ttyUSB0' end
    end  
