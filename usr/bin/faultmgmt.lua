...
-- timer to deal with kernel panic event, default is 120(second)
local kernel_panic_timer = 120
local last_kernel_alarm
...
local function init()
  ...
  kernel_panic_timer = tonumber(global["kernel_panic_timer"] or 120)
  last_kernel_alarm = global["last_kernel_alarm"]
...
local function handle_kernel_panic()
  local msg = {}
  local fd = open("/proc/prozone/panic", "r")
  if fd then
    local content = fd:read("*all")
    local panic_id = content:match("%-+%[ end trace ([0-9a-f]+) %]%-+")
    if panic_id and panic_id ~= last_kernel_alarm then
      local cause = content:match("([Oo]ops)") or content:match("([Pp]anic)")
      if cause then
        local text_pattern = format("%%c.*%%] (.*%s.*ARM)%%c", cause)
        local cpu, pc, lr = content:match("%] (CPU:.*)%c.*%] (PC is at.*)%c.*%] (LR is at.*)%c.*%] pc :")
        msg.ProbableCause = cause:match("[Oo]ops") and "Kernel Oops" or cause:match("[Pp]anic") and "Kernel Panic"
        msg.AdditionalText = content:match(text_pattern)
        msg.AdditionalInformation = format("%s, %s, %s", cpu, pc, lr)
        msg.EventType = "CPE kernel"

        cursor:set("faultmgmt", "global", "last_kernel_alarm", panic_id)
        cursor:commit("faultmgmt")

        handle_event_update(msg)
      end
    end
  end
end
...
-- Start event monitor
xpcall(fm_event_monitor.create_defaultGW_monitor, errhandler)
-- Idle loop
while true do
  -- Handle kernel panic events
  uloop.timer(handle_kernel_panic, kernel_panic_timer*1000)
