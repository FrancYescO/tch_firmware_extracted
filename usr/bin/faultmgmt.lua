...
-- ACS server connected successfully
local acs_connected_flag = false
...
local function init()
  ...
  event_tables.history.size = history_size
...
local function trigger_buffered_event()
  -- Call for generating valid key in transformer.db
  local smsg = {}
  for expedited_pointer,event in ipairs(expedited_events) do
    if event.EventType ~= "" then
      smsg.Key = format("%s|%d", "expedited", expedited_pointer)
      ubus_conn:send('faultmgmt.expeditedevent', smsg)
    end
  end
  for queued_pointer,event in ipairs(queued_events) do
    if event.EventType ~= "" then
...
local function handle_event_update(msg)
  ...
  if etype == 'ACS provisioning' and pcause == 'Inform success'  and not acs_connected_flag then
    trigger_buffered_event()
    acs_connected_flag = true
  end
...
local function handle_rpc_get(req, msg)
  ...
  if table_name == "supported" then
    ...
    -- for expedited,queued and history table, add the unpopulated entry
