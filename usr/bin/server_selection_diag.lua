...
local function dns_host_address(host_str, version)
  ...
  if version == "IPv6" then
    cmdline = format("dnsget -t AAAA %s 2>&1", host_str)
    ...
    cmdline = format("dnsget -t A %s 2>&1", host_str)
    local p = assert(io.popen(cmdline))
    output = p:read("*a")
    p:close()
...
local function get_addresses(host_str)
  ...
  if interface then
    ...
    if host_ip then
      cmdline = format("ip route get %s 2>&1", host_ip)
      local p = assert(io.popen(cmdline))
      output = p:read("*a")
      p:close()
    ...
    if (not host_ip) or (not interface_ip) then
      ...
      if host_ip then
        cmdline = format("ip -6 route get %s 2>&1", host_ip)
        local p = assert(io.popen(cmdline))
        output = p:read("*a")
        p:close()
        interface_ip = match(output, "src%s+([%S:]+)")
    ...
    if v and v:len() ~= 0 then
      ...
      if response[i]["state"] == "Completed" then
        ...
        if not timeout or timeout == 0 then
          cmdline = format("%s -I %s %s 2>&1", cmdline,ipaddress[i], host_ip)
        else
          local tm = math.modf(timeout/1000)
          if tm == 0 then
            tm = 1
          end
          cmdline = format("%s -W %d -I %s %s 2>&1", cmdline,tm,ipaddress[i], host_ip)
        end
  ...
  for i,v in ipairs(host_list) do
    ...
    if v and v:len() ~= 0 then
      if timeout and timeout > 0 then
        cmdline = format("%s --resfile %s --repetitions %d --protocol %s --individual 0 --intertime 1 --timeout %d", udpecho_prog, resfile, repetition, protocol_version, timeout)
      else
        cmdline = format("%s --resfile %s --repetitions %d --protocol %s --individual 0 --intertime 1", udpecho_prog, resfile, repetition, protocol_version)
      end
      response[i]["state"] = "Completed"
      local host
      local port
      ...
      if host then
        cmdline = format("%s --host %s", cmdline, host)
        if port and port:len() ~= 0 then
          cmdline = format("%s --port %s", cmdline, port)
        end
        if interface and interface:len() ~= 0 then
          cmdline = format("%s --intf %s", cmdline, interface)
        end
        local p = assert(io.popen(cmdline))
        p:close()
