...
local function send_bfdecho_msg(ipv4)
  ...
  if data and data["up"] then
    ...
    elseif intf and src_ip and timeout then
      ...
      if state then
        if state == "0" then
          bfd_sts[failed_counter] = bfd_sts[failed_counter] + 1
          log:info("send bfdecho failed " .. bfd_sts[failed_counter] .. (ipv4 and " ipv4 " or " ipv6 "))
          if bfd_sts[failed_counter] >= bfd_cfg.failed_limit then
            -- send bfdecho NOK only when failed counter reached failed limit
            ubus_conn:send("bfdecho", {interface = interface, state = "0"})
            bfd_sts[failed_counter] = 0
      ...
      log:error("send_bfdecho_msg : intf or src_ip is nil for" .. (ipv4 and " ipv4 " or " ipv6 "))
    end
  else
    bfd_sts[failed_counter] = 0
  end
...
local function get_DNS_servers(ipv4)
  ...
  if pipe then
    match_expr = ipv4 and "nameserver (%d+%.%d+%.%d+%.%d+)" or "nameserver%s+([a-f0-9A-F]*:[a-f0-9A-F:]*:[a-f0-9A-F.]*)%s*"
    for line in pipe:lines() do
      local result = line:match(match_expr)
      if result then
...
local function dns_run_dig(query, server, v6, attempts, timeout)
  ...
  cmd = cmd .. ' +nocomments +noquestion +noauthority +noadditional +noanswer ' .. ' ' .. query .. ' 2>/dev/null' -- show stats section only

  log:notice("dns_run_dig::trigger dns query by " .. cmd)

  --[[
    dig @2013:a6a6:4086:1::2 -6 -t AAAA +time=5 +tries=1 +nocomments +noquestion +noauthority +noadditional +noanswer apple.com

     ; <<>> DiG 9.11.2 <<>> @2013:a6a6:4086:1::2 -6 -t AAAA +time=5 +tries=1 +nocomments +noquestion +noauthority +noadditional +noanswer apple.cm
     ; (1 server found)
     ;; global options: +cmd
     ;; Query time: 0 msec
     ;; SERVER: 2013:a6a6:4086:1::2#53(2013:a6a6:4086:1::2)
     ;; WHEN: Sun Sep 30 21:24:09 AEST 2018
     ;; MSG SIZE  rcvd: 83
  ...
  if pipe then
    for line in pipe:lines() do
      msg_size = string.match(line, ";; MSG SIZE  rcvd:%s+([0-9]*)")
      if msg_size ~= nil then
        break
      end
    end
    pipe:close()
    if msg_size ~= nil then
      return query, "172.217.27.110", "2404:6800:4007:80e::200e" -- dummy ipaddress
    else
...
------------------------------------------------------------------
local function heartbeat_testing_set(ipv4)
  ...
  if server_list[1] ~= nil then
    ...
    local status, hostname_or_error = scripth.dns_check(domain_name, server_list[1], domain_name, nil, nil, (not ipv4), retry, timeout)
    if status and hostname_or_error then
      return true
    end
  else
...
local function send_heartbeat_query(ipv4)
  if global_sts.running_mode ~= "DNS" then -- not enabled for DNS supervision mode
    ...
    if enabled == "1" and interface ~= nil then
      ...
      log:info("send_heartbeat_query interface " .. interface .. " is " .. (status and "up" or "down"))
      if status then
        log:notice("send_heartbeat_query test")
        heartbeat_testing_set(ipv4)
      end
    end
  end
