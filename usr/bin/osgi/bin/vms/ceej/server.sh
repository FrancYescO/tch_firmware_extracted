...
# *	fi
	...
	# * set class path
	if [ -z "$FWAPPCP" ]; then 
		CP="-Xbootclasspath $BOOTCP:$_SERVER:$mCP -cp $MBS_ROOT/lib/framework/fwtime.jar:/usr/bin/jvms/lib/jsr166.jar";
	else 
		if [ -n "$BOOTCP" ]; then
			CP="-Xbootclasspath $BOOTCP -cp $_SERVER:$mCP:$MBS_ROOT/lib/framework/fwtime.jar:/usr/bin/jvms/lib/jsr166.jar";
		else
			CP="-cp $_SERVER:$mCP:$MBS_ROOT/lib/framework/fwtime.jar:/usr/bin/jvms/lib/jsr166.jar:/usr/bin/osgi/bundles/fs.lib.transformer.jar";
		fi
	fi
	# * remote debugging	
...
SetupVM () {
	...
	if [ -x "$VM_HOME/../siege" ]; then
		JAVA="$VM_HOME/../siege";
	elif [ -x "$VM_HOME/Binaries/siege4" ]; then
		JAVA="$VM_HOME/Binaries/siege4";
	elif [ -x "$VM_HOME/Binaries/siege" ]; then
		JAVA="$VM_HOME/Binaries/siege";
	else
		cat<<EOF
