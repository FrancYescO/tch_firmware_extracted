...
#TCH Changes
local EN_OSGI_SYSLOG
config_get EN_OSGI_SYSLOG config enable_syslog
if [ $EN_OSGI_SYSLOG -eq 1 ]; then
  config_load "siege"
  local EN_SIEGE_SYSLOG
  config_get EN_SIEGE_SYSLOG log enable_syslog
  if [ $EN_SIEGE_SYSLOG -eq 1 ]; then
    LOG_SIMPLE="on"
  else
    logger -t osgi "The log service for osgi is not configured properly.Enable syslog under siege config."
  fi
  config_load "osgi"
fi
...
Setup () {
	...
	EXTBOOTFILE=
	if [ -d $MBS_ROOT/bin/vms/extbootfiles ]; then
		boot_file_list=`ls -1 $MBS_ROOT/bin/vms/extbootfiles`
		for bootfile in $boot_file_list
		do
			if [ -z $EXTBOOTFILE ]; then
				EXTBOOTFILE="../extbootfiles/$bootfile"
			else
				EXTBOOTFILE="$EXTBOOTFILE;../extbootfiles/$bootfile"
			fi
		done
	fi
	...
	if [ -n "$FWLOG" ];          then FEATURES="$FEATURES -Dmbs.log.file.dir=$OSGI_LOGPATH -Dmbs.framework.log.level=$FWLOGLVL -Dmbs.util.ref.log.level=$LOGLEVEL"; fi
	if [ -n "$LOG_SIMPLE" ];     then FEATURES="$FEATURES -Dmbs.log.simple=true -Dmbs.framework.log.level=$FWLOGLVL -Dmbs.util.ref.log.level=$LOGLEVEL"; fi
