...
-- Check if MAC addresses are belonging to the same booster
-- Parameters:
-- - mac1, mac2: [string] the MAC addresses to be compared
-- - max_delta: [integer] maximum allowed difference between compared MACs
--
-- Returns:
-- - true if addresses are likely used by the same booster
local function do_macs_belong_to_same_booster(mac1, mac2, max_delta)
  if type(mac1) ~= "string" or string.len(mac1) ~= 17 or
     type(mac2) ~= "string" or string.len(mac2) ~= 17 then
    return false
  end

  local total_diff = 0
  for i=1,17,3 do
    local bt1 = tonumber(string.sub(mac1,i,i+1),16)
    local bt2 = tonumber(string.sub(mac2,i,i+1),16)
    if bt1 == nil or bt2 == nil then
      return false
    end

    if i == 1 then
      -- multicast addresses cannot be used
      if bt1 % 2 == 1 or bt1 % 2 == 1 then
	return false
      end
      -- ignore locally administered bit
      if math.floor(bt1 / 2) % 2 == 1 then
	bt1 = bt1 - 2
      end
      if math.floor(bt2 / 2) % 2 == 1 then
	bt2 = bt2 - 2
      end
    elseif string.sub(mac1,i-1,i-1) ~= ":" or string.sub(mac2,i-1,i-1) ~= ":" then
      return false
    end

    local btdiff = math.abs(bt1 - bt2)
    if btdiff > 1 then
      local altdiff = 8
      if bt1 % 16 == bt2 % 16 then
	-- when least significant hex digits are the same,
	-- check difference between most significant hex digits
	altdiff = math.abs(math.floor(bt1/16) - math.floor(bt2/16))
      end
      if altdiff > 1 then
	-- when diff between most significant hex digits is greater than 1
	-- check how many bits are different
	altdiff = 0
	while bt1 > 0 or bt2 > 0 do
	  if bt1 % 2 ~= bt2 % 2 then
	    altdiff = altdiff + 1
	  end
	  bt1 = math.floor(bt1/2)
	  bt2 = math.floor(bt2/2)
	end
      end
      -- use altdiff iso btdiff if it indicates a smaller difference
      if btdiff > altdiff then
	btdiff = altdiff
      end
    end

    total_diff = total_diff + btdiff
    if total_diff > max_delta then
      return false
    end
  end
...
local function get_map_agent_data(macaddress)
  ...
  if type(reply) == "table" then
	...
	local i = 1
	if repeater_macaddr == nil and do_macs_belong_to_same_booster(sv.MACAddress, macaddress, 1) then
	...
	while i <= sv.NoOfRadios do
	  ...
	  if type(rv) == "table" and type(rv.RadioID) == "string" and type(rv.BSSID) == "table" then
	    if repeater_macaddr == nil and do_macs_belong_to_same_booster(lower(rv.RadioID), macaddress, 1) then
	      repeater_macaddr = lower(sv.MACAddress)
	    end
		...
		bssids[bssid] = true
		if repeater_macaddr == nil and do_macs_belong_to_same_booster(bssid, macaddress, 1) then
		  repeater_macaddr = lower(sv.MACAddress)
		end
	      end
	    end
...
local function flush_repeater_state(macaddress)
  ...
  if type(repeater) == "table" then
    log:info("Flush repeater " .. macaddress .. " state");
...
local function get_map_agent_station_data(interface, macaddress)
  ...
  if id == nil then
	...
	log:info("Repeater " .. macaddress .. " has been detected");
...
local function handle_map_agent_station_update(msg)
  ...
  if event ~= "Connect" and event ~= "Disconnect" then
    log:info("Ignoring map_controller.ess_station event with unknown state")
    return
  end
...
local function handle_map_agent_update(msg)
  ...
  elseif event == "Update" then
    ...
    log:info("Ignoring improper map_controller.agent event with unknown state")
  end
end
