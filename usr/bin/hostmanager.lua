...
-- Retrieve MultiAP suppport Wireless Device Information
--
-- Parameters:
-- - macaddress: [string] the MAC address of the device
--
...
-- - nil if no information was found
-- - otherwise [boolean] true, [string] radio name, [string] access point name, [string] ssid name, [table] wireless station information
local function get_multiap_wireless_station_data(macaddress)
  local agent_info = ubus_conn:call("multiap.controller.agent_info", "get", {}) or {}
  local aps = ubus_conn:call("wireless.accesspoint", "get", {}) or {}
  local ssids = ubus_conn:call("wireless.ssid", "get", {}) or {}
  local radio, ssid
  local stai = {}
  for _, agentData in pairs(agent_info) do
    local radio_info = agentData.radio_info or {}
    for radio, radioData in pairs(radio_info) do
      radio = radioData.freq and radioData.freq
      local bss_info = radioData.bss_info or {}
      for assocMac, assocData in pairs(bss_info) do
        ssid = assocData.ssid
        local staList = assocData.sta_list or {}
        for _, mac in pairs(staList) do
          if mac == macaddress then
            for intf, intfData in pairs(ssids) do
              if intfData.radio == radio and intfData.ssid == ssid then
                for ap, apData in pairs(aps) do
                  if apData.ssid == intf then
                    stai.ssid = intf
                    stai.SSID = ssid
                    stai.parent = assocMac
                    return true, radio, ap, ssid, stai
                  end
                end
              end
            end
...
local function get_map_agent_station_data(interface, macaddress)
  ...
  -- WDS repeaters use interface names "wdsX_Y(-Z)?" where X s the part  from ssid interface wlX
  -- Y represents the repeater numeric ID
  -- and optional Z is the bridge where this VLAN interface was attached
  local _, _, id = find(interface, "^wds([_%d]+)_%d+%[-.]?%w*$")
  if id == nil then
  ...
  if (macaddress ~= repeater_macaddr and type(repeater_macaddr) == "string" and
	...
	log:error("Failed to set " .. macaddress .. " as " .. interface .. " repeater: " .. reason)
      ...
      local result, radio, accesspoint, ssid, stai = get_multiap_wireless_station_data(macaddress)
      if result then
        return result, radio, accesspoint, ssid, stai
      else
        result, radio, accesspoint, ssid, stai = true, ssid_entry.radio, ssid_entry.accesspoint, wlinterface,
...
local function is_wireless(interface, macaddress)
  ...
  result, radio, accesspoint, ssid, stai = get_multiap_wireless_station_data(macaddress)
  if result then
    return result, radio, accesspoint, ssid, stai
  end
...
local function update_ip_state(l3interface, mac, address, mode, action, conflictingmac, dhcp)
  ...
  if (action=="add") then
	...
	changed=true
      ...
      if dhcp["requested-options"] then
	-- Update lease params only when this event is generated by a DHCP transaction
	-- dnsmasq restart flushes extra data hold for current leases
	for _,key in ipairs({
	     "vendor-class" ,
	     "tags" ,
	     "manufacturer-oui" ,
...
local function handle_map_controller_ess_station_update(msg)
  ...
  if event ~= "connect" then
    if device["wireless"] then
      if device.state == "connected" and (device.wireless.parent == parent or device["mac-address"] == mac) then
        set_device_state(device, "disconnected")
        -- Publish our enriched device object over UBUS
        ubus_conn:send('hostmanager.devicechanged', transform_device_to_ubus_message(device))
      end
    else
      if device.state == "connected" and device["mac-address"] == mac then
        set_device_state(device, "disconnected")
        -- Publish our enriched device object over UBUS
        ubus_conn:send('hostmanager.devicechanged', transform_device_to_ubus_message(device))
      end
...
-- Handles interface events on UBUS
--
-- Parameters:
-- - msg: [table] the UBUS message
local function handle_interface_update(msg)
  local interface, action, bridge

  -- Reject bad events
  if (type(msg)~="table" or
       type(msg['interface'])~="string") then
    log:info("Ignoring improper event");
    return
  end
  ...
  interface = msg['interface']
  action = msg['action']
  if interface == nil or action ~= "ifup" then
    return
  end

  if cursor:get_all("hostmanager", interface) then
...
local function handle_connected_time_update(msg)
  ...
  if synced == "1" and  not time_changed then
    ...
    for _, device in pairs(alldevices) do
      if device.connected_time and ((oldTime < newTime and device.connected_time <= oldTime) or (oldTime > newTime and device.connected_time >= oldTime)) then
        device.connected_time = device.connected_time + time_diff
	if device.disconnected_time then
          device.disconnected_time = device.disconnected_time + time_diff
	end
        for _, mode in ipairs(ip_modes) do
          for _, ipentry in pairs(device[mode]) do
            if type(ipentry.dhcp) == "table" and ipentry.dhcp["expiration-time"] then
              ipentry.dhcp["expiration-time"] = ipentry.dhcp["expiration-time"] + time_diff
            end
          end
        end
      end
...
ubus_conn:listen({ ['network.neigh'] = handle_device_update,
                   ...
                   ['network.interface'] = handle_interface_update,
