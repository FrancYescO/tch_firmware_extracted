...
local function transform_device_to_ubus_message(device)
  ...
  --update device-type and user-friendly-name when ubus get
  local config = 'user_friendly_name'
  cursor:load(config)
  cursor:foreach(config, 'name', function(s)
    if(s['mac'] and s['mac'] == device["mac-address"]) then
