...
local function update_ip_state(l3interface, mac, address, mode, action, dhcp)
  local changed = false
  ...
  if (action=="add") then
	...
	iplist[redirectips[redirkey]].configuration ~= "dynamic" and changed) then
...
local function handle_device_update(msg)
  ...
  if device.l3interface ~= l3interface then
    if msg.action ~= "add" then
      -- ignore delete events coming from other interfaces than the one currently used
      return
    else
	...
	local redirkey = mac .. "," .. mode
	redirectips[redirkey] = nil
	update_device_redirects(mac, nil, mode)
      end
    end
  end
  ...
  for _, mode in ipairs(ip_modes) do
    ...
    if (ipaddr~=nil and type(ipaddr.address)=="string") then
      changed = update_ip_state(l3interface, mac, lower(ipaddr.address), mode, msg.action, msg.dhcp) or changed;
    end
  end
  ...
  if device.l2interface ~= nil then
    ...
    if (l2_is_wireless and ap_x) then
      ...
      if (hai ~= nil) and (hai.state ~= nil) then
	-- Received untrusted events but wireless device is ageing in 'brctl macshow' which makes device always be 'connected'.
	 if (hai.state == "Disconnected") then
           state = "disconnected"
         elseif (string.match(hai.state, "Associated")) then -- not all chipsets provide other states (authenticated, authorized ...)
           state = "connected"
	 end
      end
    end
...
local function handle_wireless_station_update(msg)
  ...
  -- Filter out unknown wireless devices
  if device == nil or device["technology"] ~= "wireless" or device["l2interface"] == nil then
    return
  end
  local l2_is_wireless, ap_x = is_wireless(device["l2interface"])
  if (not l2_is_wireless) or ap_x ~= msg["ap_name"] then
    return
  end
  ...
  if msg["state"] == "Disconnected" then
    state = "disconnected"
  elseif msg["state"] == "Associated" then
    state = "connected"
  end

  if state == "connected" then
    local hai = hostapd_info(ap_x, mac)
    if hai then
      local wireless = {
	encryption = hai.encryption,
	authentication = hai.authentication,
	...
	rx_phy_rate = hai.rx_phy_rate,
      ...
      if device.wireless.encryption ~= wireless.encryption or
	 device.wireless.authentication ~= wireless.authentication or
	 device.wireless.tx_phy_rate ~= wireless.tx_phy_rate or
	 device.wireless.rx_phy_rate ~= wireless.rx_phy_rate then
	device.wireless = wireless
...
local function scan_network_neigh_cachedstatus()
  ...
  if x and x.neigh then
    for _,v in ipairs(x.neigh) do
      if (type(v["mac-address"]) == "string" and v["action"] == "add") then
...
ubus_conn:listen({ ['network.link'] = handle_device_link} );
ubus_conn:listen({ ['ntp.connected'] = handle_connected_time_update} );
ubus_conn:listen({ ['wireless.accesspoint.station'] = handle_wireless_station_update} );
