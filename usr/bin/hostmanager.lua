...
local function update_ip_state(mac, address, mode, action, dhcp)
  ...
  if (action=="add") then
    ...
    if dhcp then
      if ipentry.configuration ~= "dynamic" or
         type(ipentry.dhcp) ~= "table" or
         ipentry.dhcp["vendor-class"] ~= dhcp["vendor-class"] or
         ipentry.dhcp["tags"] ~= dhcp["tags"] or
         ipentry.dhcp["manufacturer-oui"] ~= dhcp["manufacturer-oui"] or
         ipentry.dhcp["serial-number"] ~= dhcp["serial-number"] or
         ipentry.dhcp["product-class"] ~= dhcp["product-class"] then
