...
case "${action}" in
	...
	upload)
		...
		if [ -n "${url}" ]; then
			...
			if ! curl -m 360 -X POST -F "exe=kernel_panic" -F "version=${version}" -F "oid=${oid}" -F "serial=${factory_id}${serial}" -F "board=${board}" -F "file=@$tmpfile" ${tag:+-F tag="$tag"} "${url}" --capath "${capath}"; then
