...
local function get_configuration()
	...
	for name, setup in pairs(confhotspots) do
		if get_interface_cached_attribute(setup["bridge"], "up") then
			setup["gre-bridge-port"] = get_gre_port(cursor, config, setup["bridge"])
	...
	for name, setup in pairs(confhotspots) do
		if setup["gre-bridge-port"] then
			_, setup["gre-iface"], setup["vlan-id"] = get_gre_setup(cursor, config, setup["gre-bridge-port"])
	...
	for name, setup in pairs(tunnels) do
		if not setup["proto"] then
			local greiface
			_, greiface = get_gre_setup(cursor, config, name)
		...
		elseif tunnel["peers"] then
			...
			if tunnel["refresh-state"] or not valid_managed_peer_tunnel_state(tunnel["state"]) then
				...
				if tunnel["tunlink"] then
					intf_ipv4, intf_ipv6 = get_started_ipv4v6_interfaces(cursor, config, tunnel["tunlink"])
					...
					if peer then
						...
						if peer.family == "inet" then
							if tunnel["peeraddr"] ~= peer.address or tunnel["proto"] ~= "gretap" or tunnel["tunlink"] ~= intf_ipv4 then
								cursor:set(config, name, "peeraddr", peer.address)
								tunnel["peeraddr"] = peer.address
								cursor:delete(config, name, "peer6addr")
								tunnel["peer6addr"] = nil
								cursor:set(config, name, "proto", "gretap")
								tunnel["proto"] = "gretap"
								greportprefix = "gre4t-"
								if intf_ipv4 then
									cursor:set(config, name, "tunlink", intf_ipv4)
									tunnel["tunlink"] = intf_ipv4
								end
							end
						else
							if tunnel["peer6addr"] ~= peer.address or tunnel["proto"] ~= "grev6tap" or tunnel["tunlink"] ~= intf_ipv6 then
								cursor:set(config, name, "peer6addr", peer.address)
								tunnel["peer6addr"] = peer.address
								cursor:delete(config, name, "peeraddr")
								tunnel["peeraddr"] = nil
								cursor:set(config, name, "proto", "grev6tap")
								tunnel["proto"] = "grev6tap"
								greportprefix = "gre6t-"
								if intf_ipv6 then
									cursor:set(config, name, "tunlink", intf_ipv6)
						...
						if greportprefix then
							...
							for _, setup in pairs(hotspots) do
								if setup["gre-iface"] == name and setup["bridge"] then
									...
									set_gre_port(cursor, config, setup["bridge"], greport)
		...
		if state ~= setup["state"] then
			...
			if setup["state"] == nil or setup["state"] == global_states.up or state == global_states.up then
				...
				for _, wl in ipairs(setup["wifi-iface"]) do
					...
					cursor:set(config, wl, "hotspot_timestamp", timestamp)
					uci_state_changed = true
				end
			end
			setup["state"] = state
...
local function reload_configuration()
	...
	-- Disable wireless interfaces that are no longer controlled by this daemon
	config = "wireless"
	cursor:load(config)
	for name, setup in pairs(hotspots) do
		for _, wl in ipairs(setup["wifi-iface"]) do
			cursor:set(config, wl, "state", 0)
...
local function set_hotspot(name, newsetup)
	...
	if newsetup["wifi-iface"] then
		...
		greport = get_gre_port(cursor, config, bridge)
		if not greport then
			cursor:unload(config)
			return { error = "Invalid bridge setup" }
		end
		_, greiface, vlanid = get_gre_setup(cursor, config, greport)
	...
	if newsetup["gre-iface"] or newvlanid then
		...
		config = "network"
		cursor:load(config)
		gretype, greiface, vlanid, vlandevice = get_gre_setup(cursor, config, newsetup["gre-iface"] or setup["gre-bridge-port"])
		if newvlanid then
			vlanid = newvlanid
		end
		errstring = validate_gre_settings(name, greiface, vlanid, hotspots)
		...
		if greiface ~= setup["gre-iface"] or vlanid ~= setup["vlan-id"] then
			...
			elseif vlandevice then
				...
				cursor:set(config, newsetup["gre-iface"], "vid", vlanid)
			else
	...
	cursor:load(config)
	if newhotspot then
		cursor:set(config, name, "hotspot")
	end
	cursor:set(config, name, "wifi_iface", setup["wifi-iface"])
	cursor:set(config, name, "enable", setup["enable"] and "1" or "0")
	if setup["monitor-iface"] then
		cursor:set(config, name, "monitor_iface", setup["monitor-iface"])
	else
		cursor:delete(config, name, "monitor_iface")
	end
	cursor:set(config, name, "check_private_wifi", setup["check-private-wifi"] and "1" or "0")
	cursor:set(config, name, "check_private_wifi_encryption", setup["check-private-wifi-encryption"] and "1" or "0")
	cursor:set(config, name, "allow_ethwan_mode", setup["allow-ethwan-mode"] and "1" or "0")
	if setup["xdsl-rate-hysteresis"] then
		cursor:set(config, name, "xdsl_rate_hysteresis", setup["xdsl-rate-hysteresis"])
	else
		cursor:delete(config, name, "xdsl_rate_hysteresis")
	end
	if setup["min-xdsl-downstream-rate"] then
		cursor:set(config, name, "min_xdsl_downstream_rate", setup["min-xdsl-downstream-rate"])
	else
		cursor:delete(config, name, "min_xdsl_downstream_rate")
	end
	if setup["min-xdsl-upstream-rate"] then
		cursor:set(config, name, "min_xdsl_upstream_rate", setup["min-xdsl-upstream-rate"])
	else
		cursor:delete(config, name, "min_xdsl_upstream_rate")
	end
	if setup["upstream-rate"] then
		cursor:set(config, name, "upstream_rate", setup["upstream-rate"])
	else
		cursor:delete(config, name, "upstream_rate")
	end
	if setup["upstream-percentage"] then
		cursor:set(config, name, "upstream_percentage", setup["upstream-percentage"])
	else
		cursor:delete(config, name, "upstream_percentage")
	end
	cursor:commit(config)
	cursor:unload(config)
...
local function set_tunnel(name, newtunnel)
	...
	if tonumber(newtunnel["ping-silent-peer-interval"]) ~= nil then
		...
		cursor:set(config, name, "ping_silent_peer_interval", tunnel["ping-silent-peer-interval"])
	end
	if tonumber(newtunnel["xdsl-rate-hysteresis"]) ~= nil then
		tunnel["xdsl-rate-hysteresis"] = math.floor(newtunnel["xdsl-rate-hysteresis"])
		cursor:set(config, name, "xdsl_rate_hysteresis", tunnel["xdsl-rate-hysteresis"])
	end
	if tonumber(newtunnel["min-xdsl-downstream-rate"]) ~= nil then
		...
		cursor:set(config, name, "min_xdsl_downstream_rate", tunnel["min-xdsl-downstream-rate"])
	end
...
local function handle_rpc_config(req, msg)
	...
	if type(newenable) == "boolean" then
		...
		cursor:load(config)
		cursor:set(config, section, param, newenable and "1" or "0")
		cursor:commit(config)
		cursor:unload(config)
...
local function get_lowest_device(intf)
	...
	while device do
		...
		elseif tostring(status["type"]):lower() == "bridge" and
			...
			-- Shortcut for bridges with just one port
			if #status["bridge-members"] == 1 then
				device = status["bridge-members"][1]
				break
			end
			...
			for _, elem in pairs(interfaces) do
				if elem["interface"] then
					...
					if lowest_device then
						local xtmdevice = cursor:get(xtmconfig, lowest_device)
