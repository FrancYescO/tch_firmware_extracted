	...
	while device do
		...
		elseif tostring(status["type"]):lower() == "bridge" and
			...
			elseif intf["proto"] == "dhcp" then
				-- DHCP case
				-- Retrieve first the network.neigh cached status
			...
			elseif intf["proto"] == "dhcpv6" then
				-- DHCPv6 case
				-- Retrieve first the network.neigh status
		...
		for _,interfaces in pairs(dump) do
			for _, elem in pairs(interfaces) do
				if elem["interface"] then
					...
					local lowest_device = get_lowest_device(elem)
	...
	if type(msg) == "table" and msg["interface"] then
		if msg["action"] == "ifup" then
			...
			if type(status) == "table" then
				...
				local lowest_device = get_lowest_device(status)
				local is_over_xdsl, has_ipv4, has_ipv6
