...
DSCP_CODE="CS0:0,BE:8,CS1:8,AF11:10,AF12:12,AF13:14,CS2:16,AF21:18,AF22:20,AF23:22,CS3:24,AF31:26,AF32:28,AF33:30,CS4:32,AF41:34,AF42:36,AF43:38,CS5:40,EF:46,CS6:48,CS7:56"
TOS_CODE="NORMAL-SERVICE:0,MINIMIZE-COST:2,MAXIMIZE-RELIABILITY:4,MAXIMIZE-THROUGHPUT:8,MINIMIZE-DELAY:16"
...
mwan_dscp_tos_code_get() {
	local _var="$1"
	local _value_org="$2"
	local _is_tos="$3"
	local _value="$_value_org"
	local _hex_value=0
	local _inverse=""
	local _upper_value=""
	if [ -z "$_value" ]; then
	...
	if [ -n "$_value" ]; then
		...
		_upper_value=$(echo $_value | tr [a-z] [A-Z])
		case "$_upper_value" in
			0X*) _hex_value=1 ;;
			*) _hex_value=0 ;;
		esac
		if [ "$_hex_value" = "1" ]; then
			_hex_value=$((${_value}))
			if [ "$_inverse" = "1" ]; then
				export -n -- "$_var=!${_hex_value}"
			else
				export -n -- "$_var=${_hex_value}"
			fi
			return
		...
		[ -n "$_value" ] && [ "$_value" -eq "$_value" ] 2>/dev/null
		if [ $? -ne 0 ]; then
			local temp=""
			local temp1=""
			if [ "$_is_tos" = "1" ]; then
   				temp=$(echo $TOS_CODE | awk -F"$_upper_value:" '{print $2}')
			else
   				temp=$(echo $DSCP_CODE | awk -F"$_upper_value:" '{print $2}')
			fi
			temp1=$(echo $temp | awk -F\, '{print $1}')
			if [ "$_inverse" = "1" ]; then
				export -n -- "$_var=!${temp1}"
			else
				export -n -- "$_var=${temp1}"
...
mwan_parse_rule() {
	...
	local rule_dscp
	config_get rule_dscp $1 dscp ""
	mwan_dscp_tos_code_get rule_dscp $rule_dscp "0"
	mwan_get_negation rule_dscp '--dscp' "$rule_dscp"
	if [ -n "$rule_dscp" ]; then
		rule_dscp="-m dscp ${rule_dscp}"
	fi

	local rule_tos
        config_get rule_tos $1 tos ""
        mwan_dscp_tos_code_get rule_tos $rule_tos "1"
        mwan_get_negation rule_tos '--tos' "$rule_tos"
        if [ -n "$rule_tos" ]; then
                rule_tos="-m tos ${rule_tos}"
        fi
		...
		local rule_icmp_type; for rule_icmp_type in ${rule_icmp_types:-""}; do
			...
			mwan_iptables_mangle "add" "ipv4" "new_mwan_rules" "MARK" \
					{ $rule_src $rule_src_ip $rule_dest_ip \
					  ...
					  $rule_icmp_type $rule_dscp $rule_tos $rule_string "-m mark" "--mark 0x0/$MWAN_NF_MASK" \
