...
do
  ...
  GSDM_REQ = msg.tags.GSDM_REQ
  GSDM_RESP = msg.tags.GSDM_RESP
end
...
function M.getSDM(uuid, ...)
  ...
  -- construct message to send to Transformer
  msg:init_encode(GSDM_REQ, max_size, uuid)
  for i = 1, nb_args do
    local path = select(i, ...)
    if not encode_path(path) then
      return nil, "not string argument"
