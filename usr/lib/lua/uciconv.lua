...
local ucilist = {
	...
	new = {env='NEW_UCI_CONFIG', default='/etc/config', savedir='/tmp/.uci'},
	current = {}
}
...
function M.uci(old_or_new)
	...
	if not uci then
		if m.env then
			uci = require('uci').cursor(os.getenv(m.env) or m.default, m.savedir)
		else
			uci = require('uci').cursor(m.default, m.savedir)
		end
		m.uci = uci
	end
	return uci
