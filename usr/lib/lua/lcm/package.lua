...
local s_transient_states = state_machine.s_transient_states
...
local function logical_state(pkg)
  return pkg._pending_state or pkg.state
end

function Package:is_in_transient_state()
  return is_transient_state(logical_state(self))
end
...
local function pkgUpdateState(pkg, state)
  ...
  if changed then
    pkg.state = state
    pkg._pending_state = nil
    if is_persistent_state(state) then
...
function Package:add_pending_action(seqnr, operationID, desired_end_state)
  ...
  self[operationID] = "pending:"..desired_end_state
  save_package(self)
end
...
function Package:remove_pending_action(seqnr)
  ...
  if idx then
    local action = tremove(pending_actions(self), idx)
    self[action.operationID] = self.errormsg and "error:"..self.errormsg or "complete"
    save_package(self)
  end
end
...
local function start_stop_complete(package, next_state)
  package._pending_state = next_state
  return false --do not move to the next state
end
...
local edge_actions = {
  ...
  start_complete = start_stop_complete,
  stop_complete = start_stop_complete
}
...
function Package:advance_state(operation_ID, desired_state, return_value)
  logger:debug("Entered advance_state with ID %s, desired state %s and return_value %s",
    operation_ID,
    desired_state,
  ...
  if self:is_in_transient_state() then
    ...
    if not return_value then
      next_state, edge = get_success_transition(logical_state(self))
    else
      next_state, edge = get_failure_transition(logical_state(self))
    end
  else
    next_state, edge = find_next_state(logical_state(self), desired_state)
    return_value = nil
  end
  if not next_state then
    return self:set_state(
      self.state,
      operation_ID,
      s_errorcodes.WRONG_STATE,
      "Unable to progress from %s to %s",
      self.state,
      desired_state or "<unknown>"
    )
  end
  if edge_actions[edge] then
    -- We need to perform an internal action during this state transition. Perform it first.
    local proceed, new_edge, errmsg = edge_actions[edge](self, next_state, edge)
    next_state = proceed and next_state or self.state
    edge = new_edge or edge
    return_value = errmsg or return_value
  end
  return self:set_state(next_state, operation_ID, edge_errorcodes[edge], return_value)
end

function Package:action_complete(desired_end_state)
  return (logical_state(self) == desired_end_state) or
         (self.errormsg and true) or
         false -- ensure boolean result
end

local function check_for_running(executing)
  if executing then
    return s_stable_states.RUNNING
  end
end


local function check_for_stopped(executing)
  if not executing then
    return s_stable_states.INSTALLED
  end
end

local execution_handlers = {
  [s_stable_states.INSTALLED] = check_for_running,
  [s_stable_states.RUNNING] = check_for_stopped,
  [s_transient_states.STARTING] = check_for_running,
  [s_transient_states.STOPPING] = check_for_stopped,
}

function Package:update_execution_state(execenv)
  local exec_handler = execution_handlers[self.state]
  if not exec_handler then
    return
  end
  local executing = execenv:executing(self)
  local new_state = exec_handler(executing)
  if new_state then
