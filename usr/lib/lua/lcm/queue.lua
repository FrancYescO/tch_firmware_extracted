...
function ActionQueue:add_notification(pkg)
  local action = {
    package = pkg,
    notification = true,
  }
  append_action_to_queue(self, action)
  return true
end
...
local function process_step(self)
  ...
  local is_notification = next_action.notification
  if is_notification and next_action.data then
    -- Notification done, remove from queue
    -- TODO: If the action that was triggered by the notification failed,
    -- we need a way to communicate the failure back to LCM.
    logger:debug("process_queue: notification done")
    queue:drop_head()
    return true
  end
  local package = next_action.package
  if not is_notification and package:action_complete(next_action.desired_end_state) then
  ...
  elseif not is_notification and package:is_in_transient_state() and next_action.data then
  ...
  elseif is_notification or package:is_in_transient_state() then
    next_action.data = nil
    ...
    package.notify = is_notification
...
function ActionHandler:addNotification(pkg)
  local queued, errcode, errmsg = self.queue:add_notification(pkg)
  if not queued then
    return nil, errcode, errmsg
  end
  self.processor:trigger_processing()
