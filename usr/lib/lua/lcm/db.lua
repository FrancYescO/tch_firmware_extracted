...
local function property_value_matches(propval, match)
  if match=="*" then
    return propval~=nil
  end
  return propval==match
end

local function match_properties(pkg, properties)
  if properties then
    for property, match in pairs(properties) do
      if not property_value_matches(pkg[property], match) then
        return false
      end
    end
  end
