...
local boards = {
	...
	--[[
		VBNT-Y (DJA0231TLS) LTE control GPIO description:
	...
	--[[
		VCNT-Z LTE control GPIO description:
		GPIO_00 	USB_ID_LTE(USB OTG)
		GPIO_08		LTE_WAKEUP
		GPIO_09		W_DISABLE
		GPIO_14 	LTE_ON_OFF
...
boards["VCNT-Z"].capabilities.power_controls[1].power_on = function()
	helper.write_file("/sys/class/gpio/gpio54/value", "0")
end

boards["VCNT-Z"].capabilities.power_controls[1].power_off = function()
	helper.write_file("/sys/class/gpio/gpio54/value", "1")
end

boards["VCNT-Z"].capabilities.power_controls[1].reset = function()
	if helper.write_file("/sys/class/gpio/gpio54/value", "1") then
		helper.sleep(0.2)
		helper.write_file("/sys/class/gpio/gpio54/value", "0")
	end
end

boards["VCNT-Z"].capabilities.antenna_controls[1].antenna[1].select_antenna = function(antenna)
	local file = "/sys/class/gpio/gpio118/value"
	if antenna == "external" then
		helper.write_file(file, "1")
	else
		helper.write_file(file, "0")
...
boards["VCNT-Z"].capabilities.antenna_controls[1].antenna[1].antenna_state = function()
	local content = helper.read_file("/sys/class/gpio/gpio118/value")
	if content and content:match("1") then
		return "external"
	end
	return "internal"
...
boards["VCNT-Z"].capabilities.rf_controls[1].enable = function()
	return helper.write_file("/sys/class/gpio/gpio9/value", "0")
end

boards["VCNT-Z"].capabilities.rf_controls[1].disable = function()
	return helper.write_file("/sys/class/gpio/gpio9/value", "1")
end

boards["VCNT-Z"].capabilities.rf_controls[1].rf_state = function()
	local content = helper.read_file("/sys/class/gpio/gpio9/value")
	if content and content:match("1") then
		return "enabled"
	end
