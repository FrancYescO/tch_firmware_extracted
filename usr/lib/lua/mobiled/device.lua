...
function Device:get_info_from_cache(key, max_age, ...)
	if max_age then
		...
		if cached_result then
			...
			if cached_result and current_age < max_age then
...
function Device:get_network_info(max_age)
	return self:get_info_from_cache("get_network_info", max_age)
end
	...
	if info.proto then
		...
		if info.proto == "ppp" then
			...
			if session and session.profile_id then
				...
				if profile then
					...
					info.ppp.pdptype = profile.pdptype
		...
		elseif info.proto == "dhcp" then
			...
			if session and session.profile_id then
				local profile = runtime.mobiled.get_profile(self, session.profile_id)
				if profile then
					info.dhcp.pdptype = profile.pdptype
				end
			end
		end
