...
function Device:get_device_info()
	...
	if self.info.pid == "1709" then
		if next(self.info.network_interfaces) == nil then
			networks = helper.split(helper.capture_cmd("find /sys/class/ -name net 2>/dev/null", "r"), "\n")
			for _, network in pairs(networks) do
				local intf = helper.split(helper.capture_cmd("ls " .. network, "r"), "\n")
				if intf then
					for _, intf_val in pairs(intf) do
						intf_val = string.match(intf_val, "^[A-Za-z]+_[A-Za-z]+$")
						if intf_val then
							table.insert(self.info.network_interfaces, { name = intf_val})
						end
					end
				end
			end
