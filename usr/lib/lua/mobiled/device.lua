...
function Device:init_device()
    return self.__plugin.plugin.init_device(self.id)
end
...
function Device:destroy(force)
    return self.__plugin.plugin.destroy_device(self.id, force)
end
...
function Device:get_ip_info(session_id)
    return self.__plugin.plugin.get_ip_info(self.id, session_id)
end
...
function Device:get_device_info()
    local info = self.__plugin.plugin.get_device_info(self.id)
...
function Device:get_device_capabilities()
    return self.__plugin.plugin.get_device_capabilities(self.id)
end
...
function Device:get_network_info()
    return self.__plugin.plugin.get_network_info(self.id)
end
...
function Device:get_session_info(session_id)
    local info, errMsg = self.__plugin.plugin.get_session_info(self.id, session_id)
...
function Device:get_profile_info()
    return self.__plugin.plugin.get_profile_info(self.id)
end
...
function Device:get_sim_info()
    return self.__plugin.plugin.get_sim_info(self.id)
end

--! @brief Get PIN info like pin_state and reties left
--! @param pinType The PIN for which you want to request this info
--! @return A table containing the requested info on success. nil and an error message on failure

function Device:get_pin_info(pinType)
    return self.__plugin.plugin.get_pin_info(self.id, pinType)
end
...
function Device:network_scan(start)
    return self.__plugin.plugin.network_scan(self.id, start)
end
...
function Device:send_sms(number, message)
    return self.__plugin.plugin.send_sms(self.id, number, message)
end
...
function Device:delete_sms(message_id)
    return self.__plugin.plugin.delete_sms(self.id, message_id)
end
...
function Device:set_sms_status(message_id, status)
    return self.__plugin.plugin.set_sms_status(self.id, message_id, status)
end
...
function Device:get_sms_info()
    return self.__plugin.plugin.get_sms_info(self.id)
end
...
function Device:get_sms_messages()
    return self.__plugin.plugin.get_sms_messages(self.id)
end
...
function Device:set_power_mode(mode)
    return self.__plugin.plugin.set_power_mode(self.id, mode)
end
...
function Device:periodic()
    return self.__plugin.plugin.periodic(self.id)
end

--! @brief Unlock the SIM with the given PIN
--! @param pinType Which pin to unlock
--! @param pin The PIN to use
--! @return true on success. nil and an error message on failure

function Device:unlock_pin(pinType, pin)
    local info = self.__plugin.plugin.get_pin_info(self.id, pinType)
    local oldUnlockRetries = info.unlock_retries_left
    local ret, errMsg = self.__plugin.plugin.unlock_pin(self.id, pinType, pin)
    if not ret then
        return ret, errMsg
    end
    info = self.__plugin.plugin.get_pin_info(self.id, pinType)
...
function Device:change_pin(pinType, oldPin, newPin)
    local info = self.__plugin.plugin.get_pin_info(self.id, pinType)
    local oldUnlockRetries = info.unlock_retries_left
    local ret, errMsg = self.__plugin.plugin.change_pin(self.id, pinType, oldPin, newPin)
    if not ret then
        return ret, errMsg
    end
    info = self.__plugin.plugin.get_pin_info(self.id, pinType)
...
function Device:__enable_pin(pinType, pin, enable)
    local info = self.__plugin.plugin.get_pin_info(self.id, pinType)
    local oldUnlockRetries = info.unlock_retries_left
    local ret, errMsg
    if(enable) then
        ret, errMsg = self.__plugin.plugin.enable_pin(self.id, pinType, pin)
    else
        ret, errMsg = self.__plugin.plugin.disable_pin(self.id, pinType, pin)
    end
    if not ret then
        return ret, errMsg
    end
...
function Device:enable_pin(pinType, pin)
    return self:__enable_pin(pinType, pin ,true)
end
...
function Device:disable_pin(pinType, pin)
    return self:__enable_pin(pinType, pin ,false)
end
...
function Device:unblock_pin(pinType, puk, newPin)
    local info = self.__plugin.plugin.get_pin_info(self.id, pinType)
    local oldUnblockRetries = info.unblock_retries_left
    local ret, errMsg = self.__plugin.plugin.unblock_pin(self.id, pinType, puk, newPin)
    if not ret then
        return ret, errMsg
    end
    info = self.__plugin.plugin.get_pin_info(self.id, pinType)
...
function Device:start_data_session(session_id, profile)
    return self.__plugin.plugin.start_data_session(self.id, session_id, profile)
end
...
function Device:stop_data_session(session_id)
    return self.__plugin.plugin.stop_data_session(self.id, session_id)
end
...
function Device:register_network(params)
    return self.__plugin.plugin.register_network(self.id, params)
end
...
function Device:get_radio_signal_info()
    return self.__plugin.plugin.get_radio_signal_info(self.id)
end
...
function Device:create_default_context(profile)
    return self.__plugin.plugin.create_default_context(self.id, profile)
end
...
function Device:debug()
    return self.__plugin.plugin.debug(self.id)
end
...
function Device:firmware_upgrade(path)
    return self.__plugin.plugin.firmware_upgrade(self.id, path)
end
...
function Device:get_firmware_upgrade_info()
    return self.__plugin.plugin.get_firmware_upgrade_info(self.id)
end

function Device:activate_data_session(session_id, profile_id, interface)
...
function Device:get_network_interface(session_id, logical_interface)
    if self.info and self.info.network_interfaces then
