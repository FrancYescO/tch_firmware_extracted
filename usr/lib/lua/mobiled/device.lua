...
function Device:activate_data_session(session_id, profile_id, interface, optional, pdn_retry_timer_value)
    for i=#self.__session_profile_map,1,-1 do
        ...
        if m.session_id == session_id then
            if m.profile_id == profile_id then
                if m.deactivate then
                    m.deactivate = false
                    return true
                end
                return nil, "Already activated"
            else
                -- We want to use a different profile then the current one
    ...
    table.insert(self.__session_profile_map, { session_id = session_id, profile_id = profile_id, deactivate = false, changed = false, interface = interface, optional = optional or false, pdn_retry_timer = { default_value = pdn_retry_timer_value } })
    return true
end
