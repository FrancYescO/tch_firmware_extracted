...
function M.activate_data_session(device, session_id, profile_id, interface)
	...
	if ret then
		ret, errMsg = device:activate_data_session(session_id, profile_id, interface)
		if ret then
...
function M.store_pin_to_config(pinType, pin, iccid)
	...
	runtime.config.store_pin_to_config(pinType, pin, iccid)
end
	...
	if(event.event == "timeout") then
		...
		if event.event == "platform_config_changed" then
			...
			if config.platform then
				if config.platform.antenna then
					if M.platform.select_antenna then
						M.platform.select_antenna(config.platform.antenna)
					end
				end
				if config.platform.power_on then
					if M.platform.module_power_on then
						M.platform.module_power_on()
					end
				else
					if M.platform.module_power_off then
						M.platform.module_power_off()
					end
				end
			end
		end
		...
		if event.event == "device_disconnected" then
			...
			if not event.dev_idx then
				for _, sm in ipairs(stateMachines) do
					if sm.device and not detector.alive(sm.device) then
						sm:handle_event(event)
					end
				end
				return
		...
		if string.match(event.event, "session_") then
			...
			if event.event == "session_activate" then
				M.activate_data_session(device, event.session_id, event.profile_id, event.interface)
			elseif event.event == "session_setup" or event.event == "session_config_changed" then
				...
				if session then
					...
					if session.session_id == 0 then
						...
						if profile then
							device:create_default_context(profile)
						end
					end
				end
