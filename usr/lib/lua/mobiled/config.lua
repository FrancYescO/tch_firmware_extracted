...
local function device_config_changed(oldconfig, newconfig)
	local filter = {
		...
		-- This option is added for supporting MBN selection on Quectel modules:
		mbn_selection = ""
	}
...
local function get_default_device_config()
	...
	if not __config.device_defaults.mbn_selection then __config.device_defaults.mbn_selection= "none" end
...
function M.get_device_config(device)
	...
	if not device_config.radio_pref then
		device_config.radio_pref = "auto"
	end
	...
	-- This option is added for supporting MBN selection on Quectel modules:
	c.device.mbn_selection = device_config.mbn_selection
