...
function M.remove_pin_from_config(pinType, iccid)
	...
	c:foreach(mobiled_config, "sim", function(s) 
		if s.iccid == iccid then
			c:delete(mobiled_config, s[".name"], "pin")
			c:commit(mobiled_config)
			return false
		end
	end)
	config.sims = {}
	c:foreach(mobiled_config, "sim", function(s) table.insert(config.sims, s) end)
end

local function add_default_sim_config(defaults)
	local c = uci.cursor()
...
function M.store_pin_to_config(pinType, pin, iccid)
	...
	if not sim then
		add_default_sim_config({ iccid = iccid, pin = pin, pin_type = pinType })
	else
...
local function profile_equal(old, new)
	if old.apn ~= new.apn or old.pdptype ~= new.pdptype or old.username ~= new.username or old.password ~= new.password then
		return false
	end
	return true
end
...
local function device_config_has_changes(oldconfig, newconfig, filter)
	local devices = {}
	for _, oldDevice in pairs(oldconfig.devices) do
		local oldparams = extract_params(oldDevice, filter)
		local newparams
		for _, newDevice in pairs(newconfig.devices) do
			if newDevice.imei == oldDevice.imei then
				newparams = extract_params(newDevice, filter)
			end
		end
		if newparams then
			if not helper.table_eq(oldparams, newparams) then
				table.insert(devices, oldDevice.imei)
			end
		end
	end
	return devices
...
local function get_device_idx_by_imei(stateMachines, imei)
	for _, sm in pairs(stateMachines) do
		if sm.device and sm.device.imei == imei then
			return sm.dev_idx
		end
	end
end
...
function M.reloadconfig(stateMachines, plugins)
	...
	for _, imei in pairs(device_config_devices) do
		local dev_idx = get_device_idx_by_imei(stateMachines, imei)
		if dev_idx then
	...
	for _, imei in pairs(network_config_devices) do
		local dev_idx = get_device_idx_by_imei(stateMachines, imei)
		if dev_idx then
...
function M.get_config()
	...
	if config.platform then
		...
		c.platform.antenna = config.platform.antenna
...
local function add_default_device_config(device, defaults)
	local c = uci.cursor()
...
function M.get_device_config(device)
	if not device.imei then
		return nil, "IMEI not set for device"
	end

	local device_config
	for _, d in pairs(config.devices) do
		if d.imei == device.imei then
			device_config = d
			break
		end
	end
	...
	if not device_config then
		...
		device_config.imei = device.imei
