...
local function operators_config_changed(oldconfig, newconfig)
	return not helper.table_eq(oldconfig, newconfig)
end
...
function M.reloadconfig(stateMachines, plugins, force)
	...
	local operator_changed = operators_config_changed(__config.operators, newconfig.operators)
	...
	if operator_changed then
		runtime.log:notice("Operator config changed")
		events.send_event("mobiled", { event = "operator_config_changed" })
	end
...
function M.loadconfig()
	...
	cursor:foreach(mobiled_config_file, "init_script", function(section)
		local dev_descs = {}
		if section.dev_desc then
			if type(section.dev_desc) ~= "table" then
				runtime.log:warning("mobiled.%s.dev_desc is not a list, ignoring init script", section[".name"])
				return
			end
			for index, dev_desc in ipairs(section.dev_desc) do
				if type(dev_desc) ~= "string" then
					runtime.log:warning("mobiled.%s.dev_desc[%d] is not a string, ignoring init script", section[".name"], index)
					return
				end
				dev_descs[dev_desc] = true
			end
		end

		local blacklist = false
		if section.blacklist == '1' or section.blacklist == 'yes' or section.blacklist == 'true' then
			blacklist = true
		elseif section.blacklist and section.blacklist ~= '0' and section.blacklist ~= 'no' and section.blacklist ~= 'false' then
			runtime.log:warning("mobiled.%s.blacklist is not a boolean, ignoring init script", section[".name"])
			return
		end

		if type(section.command) ~= "string" then
			runtime.log:warning("mobiled.%s.command is not specified or is not a string, ignoring init script", section[".name"])
			return
		end
		local command = section.command

		local arguments = {}
		if section.argument then
			if type(section.argument) ~= "table" then
				runtime.log:warning("mobiled.%s.arguments is not a list, ignoring init script", section[".name"])
				return
			end
			for index, argument in ipairs(section.argument) do
				if type(argument) ~= "string" then
					runtime.log:warning("mobiled.%s.argument[%d] is not a string, ignoring init script", section[".name"], index)
					return
				end
...
function M.get_config()
	...
	c.init_scripts = __config.init_scripts
...
function M.get_session_config(device)
	...
	for _, session_config in pairs(__config.sessions) do
		if not session_config.dev_desc or session_config.dev_desc == device.desc then
			local session = {
...
function M.get_device_config(device)
	...
	if device_config.lapi_enabled and device_config.lapi_enabled == '1' or device_config.lapi_enabled == 'true' then
		c.device.lapi_enabled = true
	end
