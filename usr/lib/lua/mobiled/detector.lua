		...
		if allowed then
			for _, descriptor in ipairs(usb_descriptors) do
				...
				if found and not runtime.mobiled.device_exists(desc) then
					local dev, errMsg = runtime.mobiled.add_device({dev_desc = desc, dev_type = "usb", plugin_name = descriptor.plugin, network_interfaces = usb_info.network_interfaces, pid = usb_info.pid, vid = usb_info.vid, product = usb_info.product})
