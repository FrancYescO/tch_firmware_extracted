...
local function get_usb_info(desc)
	...
	local networks = helper.split(helper.capture_cmd("find /sys/bus/usb/devices/" .. desc .. "/ -name net", "r"), "\n")
	for _, network in pairs(networks) do
		local intf = helper.split(helper.capture_cmd("ls " .. network, "r"), "\n")
		if intf then
			for _, i in pairs(intf) do
				i = string.match(i, "[A-Za-z0-9]+")
				table.insert(info.network_interfaces, i)
			end
		end
	end
	table.sort(info.network_interfaces)
...
local function detect_usb_devices(runtime)
	...
	for _, desc in pairs(devices) do
		...
		for _, descriptor in ipairs(usb_descriptors) do
			local found = match_drivers(descriptor.driver_combinations, usb_info)
			if not found then
				found = match_identifiers(descriptor.device_identifiers, usb_info)
			end
			if found and not runtime.mobiled.device_exists(desc) then
				local dev, errMsg = runtime.mobiled.add_device({dev_desc = desc, dev_type = "usb", plugin_name = descriptor.plugin, network_interfaces = usb_info.network_interfaces, pid = usb_info.pid, vid = usb_info.vid})
...
local function detect_misc_devices(runtime)
	...
	if ret and string.match(ret, desc) and not runtime.mobiled.device_exists(desc) then
		local network_interfaces = helper.split(helper.capture_cmd("find /sys/class/net -name 'rmnet_data*' -exec basename {} \\;", "r"), "\n")
		table.sort(network_interfaces)
		return runtime.mobiled.add_device({dev_desc = desc, device_type = "ontarget", plugin_name = "libqmiqti", network_interfaces = network_interfaces})
	end
	return nil
end
		...
		for _, device in pairs(config.debug_devices) do
			...
			if device.network_interfaces then
				for i in string.gmatch(device.network_interfaces, "%S+") do
					table.insert(network_interfaces, i)
				end
			end
			if not runtime.mobiled.device_exists(dev_desc) then
				local dev, errMsg = runtime.mobiled.add_device({dev_desc = dev_desc, dev_type = "debug", plugin_name = plugin, network_interfaces = network_interfaces, debug_device = device})
				if dev then return dev end
				if errMsg then return nil, errMsg end
			end
		end
...
function M.scan(runtime)
	...
	if string.match(config.detectors, "eth") then
		dev, errMsg = detect_eth_devices(runtime)
		if dev then return dev end
		if errMsg then runtime.log:error(errMsg) end
	end
	if string.match(config.detectors, "usb") then
		dev, errMsg = detect_usb_devices(runtime)
		if dev then return dev end
		if errMsg then runtime.log:error(errMsg) end
	end
	if string.match(config.detectors, "misc") then
...
function M.alive(device)
	-- Checks if a certain USB device (e.g. 1-1.5) is still present in sysfs
	if device and device.type == "usb" then
		local vid = string.match(helper.read_file("/sys/bus/usb/devices/" .. device.desc .. "/idVendor") or "", "([0-9a-f]+)")
		if vid then return true else return nil end
	end
	return true
end
