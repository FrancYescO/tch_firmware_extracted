...
function M.get_led_info(device)
	...
	local pin_info = device:get_pin_info() or {}
	local call_info = device:call_info() or {}
	local session_info = device:get_session_info(0) or {}
	local network_info = device:get_network_info() or {}

	local call_state
	if type(call_info.calls) == "table" then
		for _, c in pairs(call_info.calls) do
			call_state = c.call_state
		end
	end
	call_state = call_state or "no_call"

	local nas_state = network_info.nas_state or "not_registered"
	local session_state = session_info.session_state or "disconnected"
