...
local function mobiled_get_uim_ctrl_key_info(req, msg)
	local conn = runtime.ubus
	local mobiled = runtime.mobiled
...
local function mobiled_unlock_pin(req, msg)
	...
	runtime.log:notice("PIN unlocked")
...
local function mobiled_unblock_pin(req, msg)
	...
	if not ret then
		local data = device:get_pin_info() or {}
		conn:reply(req, { pin_info = data, error = errMsg })
		return
	end
...
local function mobiled_get_radio_signal_info(req, msg)
	...
	if data.ber then
		data.ber = string.format("%.1f", data.ber)
	end
	if data.ecio then
...
local mobiled_sim_methods = {
	['mobiled.sim'] = {
		...
		uim_ctrl_key = {
			mobiled_get_uim_ctrl_key_info, {dev_idx = ubus.INT32, key = ubus.INT32}
		}
	}
}
