...
function UbusConn:reply(request, data)
	if self._ubus then
		local call_info = self._calls[request]
		if call_info then
			runtime.log:debug(
				"ubus call %s %s '%s' replied '%s'",
				call_info.object_name,
				call_info.method_name,
				json.encode(call_info.arguments or {}),
				json.encode(data or {})
			)
			self._calls[request] = nil
		else
			runtime.log:warning(
				"reply for unknown ubus call: '%s'",
				json.encode(data or {})
			)
		end
		self._ubus:reply(request, data)
	end
end

local function log_ubus_call(connection, object_name, method_name, method)
	return function(request, arguments)
		connection._calls[request] = {
		...
		runtime.log:debug(
			"ubus call %s %s '%s' called",
			object_name,
			method_name,
			json.encode(arguments or {})
		)

		local result = method(request, arguments)

		local call_info = connection._calls[request]
		if call_info then
			runtime.log:debug(
				"ubus call %s %s '%s' finished",
				object_name,
				method_name,
				json.encode(arguments or {})
			)
			connection._calls[request] = nil
		end
...
function UbusConn:add(objects)
	if self._ubus then
		local wrapped_objects = {}
		for object_name, methods in pairs(objects) do
			local wrapped_methods = {}
			for method_name, method in pairs(methods) do
				wrapped_methods[method_name] = {log_ubus_call(self, object_name, method_name, method[1]), method[2]}
			end
			wrapped_objects[object_name] = wrapped_methods
		end
		self._ubus:add(wrapped_objects)
	end
end
