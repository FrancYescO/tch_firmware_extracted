...
function M.tprints(tbl, indent)
	...
	for k, v in pairs(tbl) do
		...
		if type(v) == "table" then
			...
			data = data .. M.tprints(v, indent+1)
		elseif type(v) == 'boolean' then
			data = data .. formatting .. tostring(v) .. "\n"
		else
			data = data .. formatting .. v .. "\n"
		end
	end
	return data
end
...
function M.tprint (tt, indent, done)
	done = done or {}
	indent = indent or 0
	if type(tt) == "table" then
		...
		if tt then io.write(tostring(tt) .. "\n") end
	end
end
