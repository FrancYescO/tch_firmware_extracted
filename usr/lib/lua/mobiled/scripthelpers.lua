...
local function get_leap_days(year)
...
function M.seconds_since_epoch(year, month, day, hour, minute, second)
	if year < FIRST_YEAR then
		return nil, "invalid year"
	end

	local days_per_month = DAYS_PER_MONTH[month]
	if not days_per_month then
		return nil, "invalid month"
	end
	if month == LEAP_MONTH then
		days_per_month = days_per_month + get_leap_days(year)
	end

	if day < 1 or day > days_per_month then
		return nil, "invalid day"
	end

	if hour < 0 or hour >= HOURS_PER_DAY then
		return nil, "invalid hour"
	end

	if minute < 0 or minute >= MINUTES_PER_HOUR then
		return nil, "invalid minute"
	end

	if second < 0 or second >= SECONDS_PER_MINUTE then
		return nil, "invalid minute"
	end

	local total_days = day - 1
	for earlier_year = FIRST_YEAR, year - 1 do
		total_days = total_days + DAYS_PER_YEAR + get_leap_days(earlier_year)
	end
	for earlier_month = 1, month - 1 do
		total_days = total_days + DAYS_PER_MONTH[earlier_month]
	end
	if month > LEAP_MONTH then
		total_days = total_days + get_leap_days(year)
	end

	return (((total_days * HOURS_PER_DAY + hour) * MINUTES_PER_HOUR) + minute) * SECONDS_PER_MINUTE + second
end
