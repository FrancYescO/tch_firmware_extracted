...
--	       syslog.debug('callback -> ' .. 'network_interface_' .. msg.interface:gsub('[^%a%d_]','_') .. '_' .. msg.action:gsub('[^%a%d_]','_'))
...
--		     syslog.debug('callback -> ' .. 'network_interface_' .. msg.interface .. '_no_ip')
                     cb('network_interface_' .. msg.interface .. '_no_ip')
                  end
               end
            end
            if msg.pppinfo ~= nil  and msg.pppinfo.pppstate ~= nil then
--	       syslog.debug('callback -> ' .. 'network_interface_' .. msg.interface:gsub('[^%a%d_]','_') .. '_ppp_' .. msg.pppinfo.pppstate:gsub('[^%a%d_]','_'))
...
--               cb('hostman_voip_down')
    ...
    events['xdsl'] = function(msg)
        if msg ~= nil then
--	    syslog.debug('callback -> ' .. 'xdsl_' .. msg.statuscode)
...
--	    syslog.debug('callback -> ' .. 'wifi_wps_' .. msg.ifname .. '_' .. msg.wps_state)
            cb('wifi_wps_' .. msg.ifname .. '_' .. msg.wps_state)
        end
    end
...
--		syslog.debug('callback -> ' .. 'wifi_on_' .. msg.ifname)
		cb("wifi_on_" .. msg.ifname)
            else
                if msg.radio_oper_state == 0 and msg.bss_oper_state == 0 then
--		   syslog.debug('callback -> ' .. 'wifi_off_' .. msg.ifname)
    ...
    events['wifitod'] = function(msg)
        if msg ~= nil then
            syslog.debug('callback -> ' .. msg.event)
            cb(msg.event)
        end
    end
    ...
    events['statusled'] = function(msg)
        if msg ~= nil and msg.state ~= nil then
--	    syslog.debug('callback -> ' .. 'status_' .. msg.state)
    ...
    events['fwupgrade'] = function(msg)
        if msg ~= nil and msg.state ~= nil then
            if msg.state == "failed" then
                syslog.debug('event: fwupgrade_state_' .. msg.state .. ' callback -> fwupgrade_state_done')
                cb("fwupgrade_state_done")
            else
                syslog.debug('callback -> ' .. 'fwupgrade_state_' .. msg.state)
	...
	if msg ~= nil and msg.state ~= nil then
--	    syslog.debug('callback -> ' .. 'reset_button_' .. msg.state)
    ...
    events['mobiled'] = function(msg)
        if msg ~= nil then
            ...
            if msg.event == "device_connected" then
                -- syslog.debug('Mobile LED callback -> ' .. 'mobile_on')
		device_plugged = 1
                cb("mobile_on")
            elseif msg.event == "device_added" and device_plugged ~= 1 then
		-- syslog(posix.LOG_DEBUG,'Mobile LED callback -> ' .. 'mobile_on')
		-- This event is only for the scenario when the USB device is already connected while the GW is rebooting
		-- This will init the state transitions in the state machine of the mobiled
		-- so that further events are captured correctly and state transitions happens properly
		cb("mobile_on")
            elseif msg.event == "device_disconnected" then
                -- syslog.debug('Mobile LED callback -> ' .. 'mobile_' .. msg.event)
                cb("mobile_" .. msg.event)
            elseif msg.event == "session_state_changed" then
                if msg.session_state ~= nil then
                    if msg.session_state == "disconnected" then
                        -- syslog.debug('Mobile LED callback -> ' .. 'mobile_session_' .. msg.session_state)
                        ...
                        if radio ~= nil and radio.radio_interface ~= nil then
                            -- syslog.debug('Mobile LED callback -> ' .. 'mobile_session_' .. msg.session_state .. "_" .. radio.radio_interface)
            ...
            elseif msg.event == "call_state_changed" then
                if msg.call_state ~= nil then
                    ...
                    radio = conn:call("mobiled.radio", "signal_quality", {})
                    if radio ~= nil and radio.radio_interface ~= nil then
                        -- syslog.debug('Mobile LED callback -> ' .. 'mobile_voice_' .. msg.call_state .. "_" .. radio.radio_interface)
        ...
        if msg ~= nil then
            if type(msg) == "table" then
                if msg.sim_state ~= nil then
                    -- syslog.debug('Mobile LED callback -> ' .. 'mobile_sim_' .. msg.sim_state)
    ...
    events['mmpbx.callstate'] = function(msg)
        if msg ~= nil then
         if (msg.callState == 5) then
             ...
             if (string.match(msg.device, "fxs_dev_")) then
                  cb("ongoing_call")
             end
         end
        end
    ...
    if data ~= nil then
        ...
        if (string.match(data.device, "fxs_dev_")) then
            cb("outgoing_call")
        end
    end
    end
