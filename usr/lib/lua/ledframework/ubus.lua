...
local function switchLedEco(cb, switcheco, persistence, cursor)
    if switcheco == false then
        switcheco = true
        cb("status_active")
    else
        switcheco = false
    ...
    if persistence ~= nil and persistence ~= '0' then
        if switcheco == false then
            cursor:set("ledfw", "eco", "switchonoff", '')
        else
            cursor:set("ledfw", "eco", "switchonoff", '1')
        end
        cursor:commit("ledfw")
    end
    return switcheco
...
function M.start(cb)
    ...
    cursor:load("ledfw")
    local persistence = cursor:get("ledfw", "eco", "persistence")
    local switcheco = false
    if first == true and persistence ~= nil and persistence ~= '0' then
       local switchonoff = cursor:get("ledfw", "eco", "switchonoff")
       if switchonoff ~= nil and switchonoff ~= '0' then
           switcheco = true
           cb("status_active")
       end
       first = false
...
--               cb('hostman_voip_down')
    ...
    events['infobutton'] = function(msg)
        if msg ~= nil and msg.state ~= nil then
            switcheco = switchLedEco(cb, switcheco, persistence, cursor)
        end
    end
