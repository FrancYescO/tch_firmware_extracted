...
local profile0Registered = false
local profile1Registered = false
...
--               cb('hostman_voip_down')
    ...
    events['mmpbx.callstate'] = function(msg)
        if msg ~= nil and msg.profileType == "MMNETWORK_TYPE_SIP" then
            local newcall = true
            if (msg.profileUsable == true) and ((msg.reason == 'MMPBX_CALLSTATE_ALERTING_REASON_INCOMINGCALL') or (msg.reason == 'MMPBX_CALLSTATE_CALL_DELIVERED_REASON_OUTGOINGCALL') or (msg.reason == 'MMPBX_CALLSTATE_CONNECTED_REASON_LOCALCONNECT') or (msg.reason == 'MMPBX_CALLSTATE_CONNECTED_REASON_REMOTECONNECT')) then
    ...
    events['mmbrcmfxs.profile.status'] = function(msg)
        if msg~=nil and type(msg) == "table" then
            for k,v in pairs(msg) do
                if k == "fxs_dev_0" then
                    fxs0PfEn = (v.profileEnabled == "true") and true or false
                elseif k == "fxs_dev_1" then
                    fxs1PfEn = (v.profileEnabled == "true") and true or false
                end --elseif
            end --for
        end --msg~=nil
    end
    ...
    events['mmpbx.profile.status'] = function(msg)
        if msg~=nil then
           if msg.enabled=="1" then
              if (msg.name == "sip_profile_0") and (msg.sip.newest.registered == "Unregistered") then
                 cb('profile_line1_unusable_true')
                 profile0Registered = false
                 if (profile1Registered == false) then
              ...
              if (msg.name == "sip_profile_1") and (msg.sip.newest.registered == "Unregistered") then
                 ...
                 profile1Registered = false
                 if (profile0Registered == false) then
                     cb('fxs_profiles_usable_false')
                 end
              end
              if (msg.name == "sip_profile_0") and (msg.sip.newest.registered == "Registered") then
                 profile0Registered = true
                 cb('profile_line1_unusable_false')
              end
              if (msg.name == "sip_profile_1") and (msg.sip.newest.registered == "Registered") then
                 profile1Registered = true
