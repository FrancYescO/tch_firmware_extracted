...
local getMaxBrightness = comm.getMaxBrightness

local cursor = M.uci.cursor()
local inhibitedStates = cursor:get('ledfw', 'inhibited_states', 'state') or {}
local easymeshStatus = cursor:get('ledfw', 'easymesh_status', 'enable')
cursor:close()
...
local function getMode(mode,item)
    return find(type(mode) == "function" and mode() or mode, item) and 1 or 0
end
...
function M.netdevLedOWRT(name, device, mode, interv, div_fact)
    ...
    local led = {
        ...
        params = {
            brightness = maxB,
            device_name = function() return getL3Device(device) end,
            link = function() return getMode(mode, "link") end,
            rx = function() return getMode(mode, "rx") end,
            tx = function() return getMode(mode, "tx") end,
            interval = interv,
...
---
-- Returns true if the given state is configured as inihibited state in ledfw config, otherwise false
-- @function [parent=#ledhelper] isInhibitedState
-- @param string state state name
function M.isInhibitedState(state)
   for _, inhibitedState in pairs(inhibitedStates) do
     if state == inhibitedState then
       return true
     end
   end
   return false
...
---
-- Returns true if easymesh_status is enabled in ledfw config, otherwise false
-- @function [parent=#ledhelper] isEMStatusEnabled
-- @param no parameters
function M.isEMStatusEnabled()
   return easymeshStatus == "1" and true or false
end

---
-- Returns true if controller is enabled in multiap config, otherwise false
-- @function [parent=#ledhelper] isControllerEnabled
-- @param no parameters
function M.isControllerEnabled()
   local cursor = M.uci.cursor()
   local state = cursor:get('multiap', 'controller', 'enabled')
   cursor:close()
   return state == "1" and true or false
end
