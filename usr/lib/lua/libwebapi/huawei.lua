...
function Mapper:get_session_info(device, info, session_id)
	...
	if success then
		data, error = curl_get(device, string.format("http://%s/api/dialup/profiles", device.web_info.ip))
		if data then
			local tab = lom.parse(data)
			if tab then
					...
					if profiles then
						for _, profile in pairs(profiles) do
							if profile.tag == "Profile" then
								...
								local profileIndex
								if i and i[1] then profileIndex = tonumber(i[1]) end
								if profileIndex and profileIndex == currentProfile then
...
function Mapper:login(device, username, password)
	return login(device, username, password)
end
