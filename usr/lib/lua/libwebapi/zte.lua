...
local function configure_profile(device, profile)
	local index
...
function Mapper:get_session_info(device, info, session_id) -- luacheck: no unused args
	...
	info.router = {
		...
		ipv6_addr = data.ipv6_wan_ipaddr,
...
function Mapper:unblock_pin(device, pin_type, puk, pin) -- luacheck: no unused args
	local params = {
		goformId = "ENTER_PUK",
		PUKNumber = puk,
		PinNumber = pin,
...
function Mapper:set_attach_params(device, profile)
	set_parameter(device, { goformId = "DISCONNECT_NETWORK" })
	return configure_profile(device, profile)
end
