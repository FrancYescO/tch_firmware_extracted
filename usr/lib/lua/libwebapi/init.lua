...
function M.add_device(params)
	...
	if type(params.network_interfaces) == "table" then
		...
		if conn then
			conn:call("network", "add_dynamic", { name = "libwebapi_control" .. device.id, proto = "dhcp", ifname = params.network_interfaces[1].name, reqopts = "1 3 33 42 51", defaultroute = false })
		end
	end
	return device.id
end
