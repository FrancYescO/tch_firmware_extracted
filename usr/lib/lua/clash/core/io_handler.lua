      ...
      elseif match(lastword, "^[%-]+") then
        ...
        self.lapp.callback = function(parm,theArg,res)
          if cmd.options[parm] then
            cli_state = cli_states[cmd.options[parm].type] or UNKNOWN_STATE
          end
        end
...
local function showhelp(self, line)
  ...
  if lastword == "help" and #words > 1 then
    ...
    if commands and commands[command] then
      ...
      self.reader:println("")
      for helpline in commands[command].usage_msg:gmatch('([^\n]*)\n') do
        self.reader:println(helpline)
      end
      return
    end
  end
  if cli_state == EMPTY_STATE or cli_state == UNKNOWN_STATE then
    self.reader:println("AVAILABLE COMMANDS:")
    self.reader:println("===================")
    local commands = getCommands(self)
    ...
    self.reader:println(indent .. "reload")
    self.reader:println(indent .. "help")
    self.reader:println(indent .. "exit")
    return
  end
end
...
local function complete(self, line)
  ...
  if completers[cli_state] then
    ...
    local completions = completer(lastword)
...
function CliIO:process(line)
  ...
  if commands and #words > 0 and commands[words[1]] then
    ...
    if (line == "") then
      ...
      self.reader:println(format("invalid command: [%s]", tostring(line)))
      showhelp(self, line)
    end
  end
end
...
M.init = function(log_level, historyfile, prompt)
  ...
  -- TODO move this to main
  local file, msg = open("/etc/banner", "r")
  if file then
