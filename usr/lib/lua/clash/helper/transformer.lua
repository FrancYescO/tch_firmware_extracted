...
local blacklist = {
  get = {}, -- Table of paths to blacklist for get.
...
-- @param result a table from the array returned by a transformer get.
-- @return table argument with password values obscured.
local function obscure_password_value(result)
...
local function get(uuid, ...)
  ...
  for i=#results, 1, -1 do
    local path = results[i].path .. results[i].param
    if not authorize_path("get", path) then
      remove(results, i)
    else
      obscure_password_value(results[i])
    end
  end

  if errmsg then
    for i=#errmsg, 1, -1 do
      local path = errmsg[i].path .. errmsg[i].param
      if not authorize_path("get", path) then
        remove(errmsg, i)
      end
    end
