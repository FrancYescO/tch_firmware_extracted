...
function M.checkTimeInterval(param, time)
  ...
  if endTimeInEpoch - startTimeInEpoch < 1 then
    return nil, "Invalid time interval, Please retry"
  end
  return true, startTimeInEpoch, endTimeInEpoch, startTime
end

function M.generateAndSetRandomTime(param, time)
  local ok, startTimeInEpoch, endTimeInEpoch, startTime = M.checkTimeInterval(param, time)
  if not ok then
    runtime.log:error("Cannot generate random time due to invalid open or close time")
    return
  end
  local cursor = require('uci').cursor()
