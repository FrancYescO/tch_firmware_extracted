...
function Decoder:AGENT_ONBOARD(msg)
  ...
  decodedData.localInterfaceCount = tonumber(decode_byte(self)) or 0
  decodedData.localInterfaces = {}
  for index = 1, decodedData.localInterfaceCount do
    decodedData.localInterfaces[index] = runtime.agent_ipc_handler.formatMAC(decode_hex(self, 6))
  end
...
function Decoder:DATA_1905_AVAILABLE(msg)
  ...
  decodedData.uuid    = "0"
  if decodedData.msgType == 8000 then
      decodedData.uuid = decode_hex(self, 2)
      runtime.log:info("Decoding 1905 DATA received : ACK : "..decodedData.msgType..", uuid :"..decodedData.uuid)
  end
  ...
  if decodedData.msgType == 4 and decodedData.TLVType == 11 then
    ...
    runtime.log:info("TLV response type of 1905 message received resp_type: %d", decodedData.responseType)
