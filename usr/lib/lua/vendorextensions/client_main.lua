...
local function read_cb()
  ...
  elseif #data == 0 then
    process.execute("uci", {"del_list", "watchdog.@watchdog[0].pidfile=/var/run/multiap_vendorextensions.pid"})
    process.execute("uci", {"commit", "watchdog"})
    process.execute("/etc/init.d/watchdog-tch", {"reload"})
