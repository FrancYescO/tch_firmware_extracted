...
M.updateInternalAgentInfo = function(aleMac)
  ...
  onboardedAgents[aleMac].agentBasicInfo.DevInfo = {
    ...
    Resettime = "",
    powerOnTimeInSec = internalAgentOnboardedTime or ""
  }
...
M.handleAgentUpdateData = function(data)
  ...
  if data.msgType == "0009" then
    runtime.log:info("Agent update message type is WSC M1 received")
...
M.handleReceived1905Data = function(data)
  ...
  if data.msgType == 4 then
    if data.responseType == 1 then
      ...
      local internalAgentMac = cursor:get("env", "var", "local_wifi_mac")
      if not onboardedAgents[internalAgentMac] then
        onboardedAgents[internalAgentMac] = {}
      end
      M.updateInternalAgentInfo(internalAgentMac)
      local devInfo = onboardedAgents[internalAgentMac].agentBasicInfo.DevInfo
      local getBasicInfoResponse = {
        DevInfo = {
          sn = devInfo.sn,
          ip = devInfo.ip,
          netmask = devInfo.netmask,
          name = "Gateway",
          Type = 0,
        ...
        FwInfo = onboardedAgents[internalAgentMac].agentBasicInfo.FwInfo
      }
...
M.handleMulticastStatus = function(data)
  ...
  if multicastRequest and multicastRequest ~= "" then
    ...
    uciCursor:foreach("vendorextensions", "agent", function(s)
      for option in pairs(s) do
        uciCursor:delete("vendorextensions", s[".name"], option)
      end
    end)
