...
function get_retry_config()
    local retry_object = {}

    local cursor = runtime.uci.cursor()

    retry_object.ack_retry_cnt                = cursor:get("vendorextensions", "retry_vendorextension", "control_message_maximum_retry_count") or 6
    retry_object.response_retry_cnt           = cursor:get("vendorextensions", "retry_vendorextension", "control_message_response_maximum_retry_count") or 3
    retry_object.ack_timeout                  = cursor:get("vendorextensions", "retry_vendorextension", "ack_message_wait_timeout") or 2
    retry_object.response_timeout             = cursor:get("vendorextensions", "retry_vendorextension", "control_message_response_wait_timeout") or 60
    retry_object.wait_time_ack_timeout        = cursor:get("vendorextensions", "retry_vendorextension", "control_message_retry_wait_time") or 5
    retry_object.wait_time_response_timeout   = cursor:get("vendorextensions", "retry_vendorextension", "control_message_response_retry_wait_Time") or 1

    return retry_object;
end

M.retry_vendor_msg = function(req_type, resp_type, al_mac, data)

    local request_key  = req_type..al_mac
    local response_key = resp_type and resp_type..al_mac

    local cursor = runtime.uci.cursor()

    if runtime.retry_handler.check_existing_retry(request_key) ~= "found" then
        local object = get_retry_config();

        object.request_key  = request_key
        object.response_key = response_key

        runtime.retry_handler.register_callback(object, function (arg) 
                local uuid = runtime.action_handler.generateUuid()
                runtime.log:info(" retry regmsg : "..req_type..", al_mac: "..M.formatMAC(al_mac)..", uuid : "..uuid)
                runtime.client.send_msg(req_type, al_mac, runtime.OUI_ID, uuid, data);
                return uuid;
            end, nil);
        runtime.log:info("Retry registered for %s for : %s", req_type, M.formatMAC(al_mac))
    else
        runtime.log:info("Retry Already exists for %s for : %s", req_type, M.formatMAC(al_mac))
    end
    return
end


M.retry_software_upgrade = function(req_type, resp_type, al_mac, data)
    local request_key  = req_type..al_mac
    local response_key = resp_type and resp_type..al_mac

    local cursor = runtime.uci.cursor()

    local dwindow = cursor:get("vendorextensions", "agent_software_deployment", "deployment_window") or 180

    if runtime.retry_handler.check_existing_retry(request_key) ~= "found" then
        local object = get_retry_config();

        object.response_timeout           = dwindow
        object.wait_time_response_timeout = 1
        ...
        runtime.retry_handler.register_callback(object, function (arg)
                local uuid = runtime.action_handler.generateUuid()
                runtime.log:info(" retry fw upgrade msg : "..req_type..", al_mac: "..runtime.agent_ipc_handler.formatMAC(al_mac)..", uuid : "..uuid)
                runtime.client.send_msg(req_type, al_mac, runtime.OUI_ID, uuid, data);
                return uuid;
            end, nil);


        runtime.retry_handler.register_failure_callback(object, function(mac)
                local cursor = runtime.uci.cursor(nil, "/var/state");
                local agent_mac = runtime.agent_ipc_handler.formatMAC(al_mac)
                local upgradeStatus = cursor:get("vendorextensions", "upgradeFirmwareStatus", agent_mac);
                ...
                cursor:set("vendorextensions", "upgradeFirmwareStatus", agent_mac, "Failed without any response from agent")
                end
...
local function config_difference(data)
  if data["Radio"] and data["AclType"] then
      if macacl[data["Radio"]] and macacl[data["Radio"]].acltype ~= nil
        and macacl[data["Radio"]].stalist ~= nil then

          local radioInfo   = macacl[data["Radio"]]
          local stalist     = data["StaList"] or {}
          local acltype     = data["AclType"]

          if radioInfo.acltype ~= acltype then
              runtime.log:info("ret0 difference")
              return "difference"
          else
              if acltype > 0 and acltype <= 2 then

                  -- check if stalist count if acltype is same
                  if #stalist ~= #radioInfo.stalist then
                      runtime.log:info("ret1 difference")
                      return "difference"
                  else
                      -- check if stalist entries are different if stalist cnt is same
                      local found = 1
                      for k,v in ipairs(stalist) do
                          found = 0
                          for m,n in ipairs(radioInfo.stalist) do
                              if v == n then
                                  found = 1
                              end
                          end
                          if found == 0 then
                              runtime.log:info("ret2 difference")
                              return "difference"
                          end
                      end
                  end
          ...
          runtime.log:info("ret3 difference")
          return "difference"
      end
  end
  return nil;
...
M.updateAclList = function(data)
  runtime.log:info("CALLING UBUS CALL updateAclList")
  if not data or not next(data) then
    return nil
  end
  
  local radioType = nil
  for type,value in pairs(runtime.action_handler.radioTypeMap) do
      if value == data["Radio"] then
          radioType = type
      end
  end
  ...
  if data["Radio"] and data["AclType"] then
      local acltype    = data["AclType"]
      local staList    = {}
      local radio_name = data["Radio"]

      if data["StaList"] and #data["StaList"] > 0 then

          local duplicate = 0
          for _,mac in ipairs(data["StaList"]) do
              duplicate = 0
              for _,v in ipairs(staList) do
                  if mac == v then
                      duplicate = 1
                      break
                  end
      ...
      if #staList == 0 and acltype > 0 and acltype <= 2 then
          -- for acltype 1(whitelist), 2(blacklist), there should be staList present
          return nil
      end

      if acltype >= 0  and acltype <= 3 then
          local ret = config_difference(data);
          if ret and ret == "difference" then

              --initialise tables
              macacl[data["Radio"]] = macacl[data["Radio"]] or {}

              local radioInfo   = macacl[data["Radio"]]
              radioInfo.stalist = radioInfo.stalist or {}

              --remove the old entry
              while #radioInfo.stalist > 0  do
                  table.remove(radioInfo.stalist)
              end

              --update the correct value
              radioInfo.acltype   = acltype
              radioInfo.radiotype = radioType

              if acltype > 0 and acltype <= 2 then
                  for k,v in ipairs(staList) do
                      table.insert(radioInfo.stalist, v)
                  end
              end

              runtime.log:info("SEND GET_ACCESS_CONTROL_REQUEST: %s", json.encode(macacl))
              for k,v in pairs(onboardedAgents) do
                  M.retry_vendor_msg("GET_ACCESS_CONTROL_REQUEST", "GET_ACCESS_CONTROL_RESPONSE", string.gsub(k,":",""), "")
              end
          end
      end
  end
  runtime.log:info("Current acl conf: %s", json.encode(macacl))
  return
end
...
M.handleAgentOnboardData = function(data)
  ...
  if mac == internalAgentMac then
    ...
    M.retry_vendor_msg("GET_BASIC_INFO", "GET_BASIC_INFO_RESPONSE", data.aleMac, "")
    M.retry_vendor_msg("GET_ACCESS_CONTROL_REQUEST", "GET_ACCESS_CONTROL_RESPONSE", data.aleMac, "")
  end
  return true
end
...
local function setDefaultLedState(aleMac)
  ...
  if newAgent then
    ...
    M.retry_vendor_msg("SET_LED_STATUS", nil, aleMac, json.encode({data = {{idx = "0", clr = "2", bri = "2"}}}))
  end
end

local function sendSetAcl(aclinfo, bssidList, bssInfoResponse)
    local request = {}
    if aclinfo and bssidList and bssInfoResponse then
        runtime.log:info("sendSetAcl aclinfo=%s", json.encode(aclinfo))
        runtime.log:info("sendSetAcl bssidList=%s", json.encode(bssidList))
        runtime.log:info("sendSetAcl bssidInfoResp=%s", json.encode(bssInfoResponse))
        for _,m in ipairs(bssidList) do
            local bssid = string.gsub(m,":","")
            runtime.log:info("sendSetAcl 1 "..bssid)
            for k,v in ipairs(bssInfoResponse) do
                runtime.log:info("sendSetAcl 2 "..v.rule.." "..type(v.rule))
                if tonumber(v.rule) > 0 and tonumber(v.rule) <= 3 then
                    if tonumber(v.rule) == 3 or tonumber(v.rule) == aclinfo.acltype then
                    runtime.log:info("sendSetAcl 3 "..v.bssid.." "..type(v.bssid))
                    if v.bssid == bssid then
                        local info   = {}
                        local cnt    = 1
                        info.bssid   = bssid
                        info.rule    = tostring(aclinfo.acltype);
                        info.stalist = {}
                        for _, sta in ipairs(aclinfo.stalist) do
                            if cnt > tonumber(v.maxnum) then
                                break
                            end
                            cnt = cnt + 1
                            local sta_formatted = string.gsub(sta, ":", "")
                            table.insert(info.stalist, sta_formatted) 
                        end
    
                        table.insert(request, info)
                        runtime.log:info("sendSetAcl request=%s", json.encode(request))
                    end
                    end
                end
            end
...
local response_name = {
 [1]  = "GET_BASIC_INFO",
 [2]  = "GET_BASIC_INFO_RESPONSE",
 [3]  = "DEPLOYSOFTWARENOW_UNICAST",
 [4]  = "DEPLOYSOFTWARENOW_COMPLETE",
 [5]  = "WRITE_FIRMWARE_REQUEST",
 [6]  = "WRITE_FIRMWARE_RESPONSE",
 [7]  = "REBOOT",
 [8]  = "RTFD",
 [9]  = "GET_ACCESS_CONTROL_REQUEST",
 [10] = "GET_ACCESS_CONTROL_RESPONSE",
 [11] = "SET_ACCESS_CONTROL_REQUEST",
 [12] = "GET_LED_STATUS",
 [13] = "GET_LED_STATUS_RESPONSE",
 [14] = "SET_LED_STATUS",
 [15] = "GET_WIFI_CONFIG_REQUEST",
 [16] = "GET_WIFI_CONFIG_RESPONSE",
 [17] = "SET_WIFI_CONFIG_REQUEST_UNICAST",
 [18] = "SET_WIFI_CONFIG_RESPONSE",
 [19] = "WIFI_DR_ONBOARDING_REQUEST",
}
...
M.handleReceived1905Data = function(data)
  ...
  local mac = M.formatMAC(data.aleMac)
  runtime.log:info("Handling 1905 message from agent: %s, msg_type= %d, uuid = %s", mac, data.msgType, data.uuid)
  runtime.log:debug("Decoded 1905 data of agent %s is %s", mac, json.encode(data))
  if data.msgType == 8000 then
      runtime.log:info("Handling 1905 message ACK RECEIVED : %s, msg_type= %d", mac, data.msgType)
      runtime.retry_handler.ack_recv_handler(data.uuid)
  end

  if data.msgType == 4 then
      if data.responseType > 0 and data.responseType < 20 and data.responseType ~= 4 then
          -- check responses here for all response type, except for the firmware download(ie. data.responseType=4)
          local received_al_mac = string.gsub(mac,":","")
          runtime.retry_handler.response_recv_handler(response_name[data.responseType]..received_al_mac)
      end
  end

  if not onboardedAgents[mac] then
      if data.responseType == 10 then
          runtime.log:info("Recieved Vendor specific response frame Get access control respone, register retry handler")
          runtime.retry_handler.immediate_retry_handler_on_badresp(response_name[data.responseType]..data.aleMac)
      else
  ...
  if data.msgType == 4 then
    if data.responseType == 1 then
      ...
      M.retry_vendor_msg("GET_BASIC_INFO_RESPONSE", nil, data.aleMac, json.encode(getBasicInfoResponse))
    ...
    elseif data.responseType == 4 then
      ...
      if downloadFirmwareResponse.result == "3" or downloadFirmwareResponse.result == "103" then
          --If a Download FW complete is received with an error code (3 or 103), VE should wait until the end of dwindow
          runtime.log:info("If a Download FW complete is received with an error code (3 or 103), VE should wait until the end of dwindow")
          return true;
      end

      if downloadFirmwareResponse.result == "1"   or downloadFirmwareResponse.result == "2" or
         downloadFirmwareResponse.result == "101" or downloadFirmwareResponse.result == "102" then
         --If a Download FW complete is received with an error code (1,2,101 or 102), WE should immediately re-send the Download FW request message
        runtime.log:info("If a Download FW complete is received with an error code (1,2,101 or 102), WE should immediately re-send the Download FW request message")
        runtime.retry_handler.immediate_retry_handler_on_badresp(response_name[data.responseType]..received_al_mac)
        return true;
      end

      --stop the retry on success case
      runtime.retry_handler.response_recv_handler(response_name[data.responseType]..received_al_mac)

      if downloadFirmwareResponseCodeToResult[downloadFirmwareResponse.result] then
    ...
    elseif data.responseType == 10 then
      local bss_info = json.decode(data.vendorInfo)
      runtime.log:info("GET ACCESS CONTROL RESPONSE response message from agent: %s %s", mac, data.vendorInfo)
      if bss_info and bss_info["info"] and #bss_info["info"] > 0 then
          --cache the received BSS info locally
          local data = {
            request = {}
          }

          local index = 1
          local acl_agent_info = onboardedAgents[mac]
          if acl_agent_info then
              for index = 1, acl_agent_info.numberOfRadios do
                  local key = "radio_" .. index

                  if acl_agent_info[key] and acl_agent_info[key].radiotype then
                     for _,v in pairs(macacl) do
                         if v.acltype > 0 and v.acltype <= 3 then
                             if v.radiotype == acl_agent_info[key].radiotype then
                                 local bssid_list = nil
                                 if acl_agent_info[key].BSSID and acl_agent_info[key].BSSID["BSSID_Fronthaul"] and #acl_agent_info[key].BSSID["BSSID_Fronthaul"] > 0 then
                                     bssid_list = acl_agent_info[key].BSSID["BSSID_Fronthaul"]
                                 elseif acl_agent_info[key].BSSID and acl_agent_info[key].BSSID["BSSID_Fronthaul_Backhaul"] and #acl_agent_info[key].BSSID["BSSID_Fronthaul_Backhaul"] > 0 then
                                     bssid_list = acl_agent_info[key].BSSID["BSSID_Fronthaul_Backhaul"]
                                 end
                                 local request = nil
                                 request = sendSetAcl(v, bssid_list, bss_info["info"])
        
                                 for _,j in ipairs(request) do
                                     table.insert(data.request, j)
                                 end
          ...
          if #data.request >= 1 then
              runtime.log:info("SET_ACCESS_CONTROL_REQUEST %s", json.encode(data))
              M.retry_vendor_msg("SET_ACCESS_CONTROL_REQUEST", nil, string.gsub(mac,":",""), json.encode(data))
          end      
      end
