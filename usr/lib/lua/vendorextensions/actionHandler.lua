...
function M.rebootActionHandler(req, msg)
  ...
  if msg.Address then
    local mac = string.gsub(msg.Address, "%:", "")
...
function M.get_led_status(_, msg)
  ...
  local uuid = M.generateUuid()
  local mac = msg.Mac and string.gsub(msg.Mac, ":", "")
  if mac then
...
local function getWifiConfiguration(aleMac, msg, backhaulState)
  ...
  local wifiConfig = {}
  for index = 1, agentInfo.numberOfRadios do
    ...
    if modifiedRadio and radioType and radioType == modifiedRadio then
      local radioConfig = {}
      radioConfig.ruid = radioType == "radio_2G" and "000000000000" or "000000000010"
