...
local function validateAndAddAgentForUpgrade(mac, agentSoftwareImages, agentData, agentsToBeUpgraded, agentSoftwareImagesCache)
  ...
  if agentData.agentBasicInfo.FwInfo.subver == "" then
    agent_sw_ver = agentData.agentBasicInfo.FwInfo.mver and gsub(agentData.agentBasicInfo.FwInfo.mver, " ", "_")
  end
...
local function sendMessage(agentsToBeUpgraded, agentSoftwareImagesCache, agentInfoCache, checksum)
  ...
  for agent_mac, sw_image_section in pairs(agentsToBeUpgraded) do
    ...
    if checksum[agent_mac] then
      ...
      runtime.agent_ipc_handler.retry_software_upgrade("DEPLOYSOFTWARENOW_UNICAST", "DEPLOYSOFTWARENOW_COMPLETE", agent_mac, tlvData)
    end
  end
end
...
-- For all the agents which have to be upgraded, we form the IPC message and store it in a table.
-- Then we send a single message for all the agents.
local function sendDownloadRequest(agentsToBeUpgraded, agentSoftwareImagesCache, agentInfoCache)
  local cursor = runtime.uci.cursor()
  local totalTlvLength = 0
  local vendor_info = cursor:get_all("vendorextensions", "agent_software_deployment") or {}
  for agent_mac, sw_image_section in pairs(agentsToBeUpgraded) do
    local mver, subver = agentSoftwareImagesCache[sw_image_section].software_version:match("(%S+)_(%S+)")
      ...
      local numKey = tonumber(key, 16)
      local id = string.sub(tostring(numKey), 1, 8)
      local stripped_url = agentSoftwareImagesCache[sw_image_section].url:gsub("(%a+)://", "")
      local protocol_str = agentSoftwareImagesCache[sw_image_section].url:match("(%a+)://") or "http";
      local deploymentData = {
        url = stripped_url,
        user = agentSoftwareImagesCache[sw_image_section].username or "",
        pwd = agentSoftwareImagesCache[sw_image_section].password or "",
        proto = web_protocol[protocol_str],
        id = id,
        dwindow = vendor_info.deployment_window or 300,
        write = vendor_info.write or 1,
        woffset = vendor_info.write_offset or 60,
...
function M.deploySoftwareActionHandler(_, _, mac)
  ...
  --if download image locally
  if download_sw_locally == 1 then
      ...
      runtime.log:info("Agents1 to be upgraded successfully fetched")
      sendDownloadRequest(agentsToBeUpgraded, agentSoftwareImagesCache, agentInfoCache)
  end
...
function M.get_led_status(_, msg)
  ...
  if mac then
    runtime.agent_ipc_handler.retry_vendor_msg("GET_LED_STATUS", "GET_LED_STATUS_RESPONSE", mac, "")
  end
  return true
end
...
function M.set_led_status(_, msg)
  ...
  if mac and brightness then
    ...
    runtime.agent_ipc_handler.retry_vendor_msg("SET_LED_STATUS", nil, mac, json.encode({data = {{idx = "0", clr = "2", bri = brightness}}}))
  end
  return true
end
...
local function getWifiConfiguration(aleMac, msg, backhaulState)
  ...
  for index = 1, agentInfo.numberOfRadios or 0 do
    ...
    local radioType = agentInfo[key] and agentInfo[key].radiotype and M.radioTypeMap[agentInfo[key].radiotype]
...
function M.setWifiConfiguration(aleMac, msg)
  ...
  if aleMac then
    ...
    if configData then
      al_mac = string.gsub(aleMac, ":", "");
      runtime.agent_ipc_handler.retry_vendor_msg("SET_WIFI_CONFIG_REQUEST_UNICAST", "SET_WIFI_CONFIG_RESPONSE", al_mac, configData)
    end
  else
...
-- Handler for SET_ACCESS_CONTROL message
function M.setMacAcl(reg, msg)
  runtime.agent_ipc_handler.updateAclList(msg)
  return true
end
