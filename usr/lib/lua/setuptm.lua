...
local function check_hardware()
  ...
  if tmhelper.file_exists('/usr/bin/setuparcher') then
    tmhelper.dbg_printf("hardware info : Archer")
    hardware.archer = true
    return true
  end
  return false
end
...
local function apply_drop_profile_RED(devtype, devname, qid, name, qos_profile)
  ...
  if hardware.runner or hardware.archer then
    if qos_profile.tcp_only then
      --set TM queue WRED drop algorithm (priomask covers flow values 32-63 which corresponds with QOS_MARK_FLAG_TCP = 0x0400)
...
--    temporary workaround (CS3278284): configuring RED not possible, use WRED with priomask zero
      tmhelper.exec_tmctl("setqdropalgx --devtype %s --if %s --qid %d --dropalg %d --loredminthr %d --loredmaxthr %d --loredpct %d" ..
                 " --hiredminthr %d --hiredmaxthr %d --hiredpct %d --priomask0 0x0 --priomask1 0x0",
        tmhelper.TM_devtype[devtype], devname, qid, tmhelper.TM_dropalg["WRED"], qos_profile.min_thr, qos_profile.max_thr, qos_profile.drop_prob, qos_profile.min_thr, qos_profile.max_thr, qos_profile.drop_prob)
    end
    return
  end
end
...
local function load_QoS_profiles()  --currently RED only
  ...
  cursor_state:foreach("qos", "profile_RED",
    function(s)
      ...
      if hardware.runner or hardware.archer then -- runner only accepts drop_prob = 100
        p.drop_prob = 100
