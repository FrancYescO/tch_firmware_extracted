...
function M.start(global, l2config, l3config, wconfig)
    ...
    scripthelper.init({ubus = c, uci = uci, logger = logger, repeatedcheck=repeatedcheck, event_cb=event_cb})

    -- popultate the runtime
    myruntime={async = async,  ubus = c, uci = uci, logger = logger, scripth = scripthelper}
