...
function L2WasMachine:entry()
    self.entrys:entry(self.name, runtime)

    -- lets schedule a timeout event within 5 seconds
    runtime.async.timerstart(5000)
end
...
function L2WasMachine:exit(transition)
    ...
    -- lets persist the l2type if the exit script was running without problems
