...
local function unlock_pin(device, pin_type, pin)
	local t = "1"
	if pin_type == "pin2" then t = "2" end

	if device:send_command("--verify-pin" .. t .. " " .. pin) then return true end
	return nil
end
...
local function unblock_pin(device, pin_type, puk, newpin)
        local t = "1"
        if pin_type == "pin2" then t = "2" end
...
local function enable_pin(device, pin_type, pin)
	local t = "1"
	if pin_type == "pin2" then t = "2" end
...
local function disable_pin(device, pin_type, pin)
	local t = "1"
	if pin_type == "pin2" then t = "2" end
...
local function change_pin(device, pin_type, pin, newpin)
	local t = "1"
	if pin_type == "pin2" then t = "2" end
