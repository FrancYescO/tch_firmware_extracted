local table, unpack, string, pairs, os, tostring, type = table, unpack, string, pairs, os, tostring, type
...
function M.init_device(dev_idx)
	...
	device:send_command("--set-data-format 802.3")
	device:send_command("--wda-set-data-format 802.3")
...
local function get_network_info(device, info)
	...
	if type(ret) == "table" then
		...
		if ret.plmn_mcc then
			...
			info.plmn_info.mcc = string.format("%03d", ret.plmn_mcc)
			if ret.pcs_digit_include ~= 0 then
				info.plmn_info.mnc = string.format("%03d", ret.plmn_mnc)
			else
				info.plmn_info.mnc = string.format("%02d", ret.plmn_mnc)
			end
		end
	end
end
...
local function get_session_info(device, info, session_id)
	if not info.dhcp then info.dhcp = {} end
...
local function start_data_session(device, session_id, profile)
	...
	local cid
	if device.session_state[id] then
		if device.session_state[id].data_handle then
			return nil, "Data session " .. session_id .. "is already running"
		end
		if device.session_state[id].cid then
			cid = device.session_state[id].cid
		end
	end
	...
	local command = ' --set-client-id wds,' .. cid .. ' --start-network --apn "' .. escape_arg(apn) .. '"'
...
local function configure_device(device, config)
	...
	-- TODO Implement RAT selection preference list
	...
	if config.network.roaming == "international" or config.network.roaming == "national" then
	...
	if config.network.selection_pref == "manual" and config.network.mcc and config.network.mnc then
		device:send_command("--set-plmn --mcc " .. config.network.mcc .. " --mnc " .. config.network.mnc)
	else
		if device.reg_info.nas_state == "registered" then
...
function M.configure_device(dev_idx, config)
	return run_action(dev_idx, "configure_device", config)
end
...
function M.debug(dev_idx)
	...
	local ret = device:send_command("--get-current-settings")
	if type(ret) == "table" then
		if type(ret.ipv4) == "table" then
			data.sessions = ret
		end
	end
	return data
