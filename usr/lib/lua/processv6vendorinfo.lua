...
local l=logger.new('processv6vendorinfo',2)
local open=io.open
local M={}
...
local function match_cwmp_interface(interface)
  ...
  local cwmp_default_interface = uci_default_cursor:get(config,"cwmpd_config","interface6")
  local cwmp_interface = uci_read_cursor:get(config,"cwmpd_config","interface6")
...
local function set_provisioning_code(pcode)
  ...
  if not ret then
    l:error("Failed to set provisioning code from dhcpv6 option 17")
...
function M.process(interface,suboptions)
  ...
  if suboptions[suboptioncode["provisioningcode"]] then
    ...
    l:error("dhcpv6 option 17 suboption " .. suboptioncode["provisioningcode"] .. " not found")
  ...
  if suboptions[suboptioncode["acsurl"]] then
    ...
    if not ret then
      l:error("Failed to set acs url from dhcpv6 option 17")
    elseif not unchanged then
      os.execute('/etc/init.d/cwmpd reload')
    end
  else
    l:error("dhcpv6 option 17 suboption " .. suboptioncode["acsurl"] .. " not found")
  end
end
