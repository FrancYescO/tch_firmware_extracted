...
function Mapper:get_sim_info(device, info)
	local ret = device:send_singleline_command('AT+ZICCID?', '+ZICCID:')
	if ret then
		local iccid = string.match(ret, '^+ZICCID:%s?(%d+)')
...
function Mapper:get_device_info(device, info)
	if not device.buffer.device_info.hardware_version then
		ret = device:send_singleline_command('AT^HVER', "^HVER:")
		if ret then
			device.buffer.device_info.hardware_version = match(ret, '%^HVER:%s*(.*)')
		end
	end
end
...
function Mapper:get_radio_signal_info(device, info)
	...
	if ret then
		if info.radio_interface == "lte" then
			local rsrp, rsrq, rssi, sinr = match(ret, '+ZRSSI:%s?%-?(%d+),%-?(%d+),%-?(%d+),([%d-%.]+)')
			if rssi then info.rssi = (tonumber(rssi)*-1) end
			if rsrp then info.rsrp = (tonumber(rsrp)*-1) end
			if rsrq then info.rsrq = (tonumber(rsrq)*-1) end
			if sinr then info.sinr = sinr end
		elseif info.radio_interface == "umts" then
			local rscp, ecio0, ecio1 = match(ret, '+ZRSSI:%s?%-?(%d+),%-?(%d+).?(%d+)')
			if rscp then info.rscp = (tonumber(rscp)*-1) end
			if ecio0 then
				info.ecio = (tonumber(ecio0)*-1)
				if ecio1 then
					info.ecio = (info.ecio - (tonumber(ecio1)*0.1))
				end
			end
		elseif info.radio_interface == "wcdma" then
			local rscp = match(ret, '+ZRSSI:%s?%-?(%d+),%d+')
			if rscp then info.rscp = (tonumber(rscp)*-1) end
		end
...
function Mapper:get_pin_info(device, info, type)
	if type == "pin1" then
		...
		if ret then
			...
			ret = device:send_singleline_command('AT+ZRAP?', '+ZRAP:')
			if ret then
				info.unlock_retries_left, info.unblock_retries_left = match(ret, '+ZRAP:%s*(%d+),%d+,(%d+),%d+')
			end
		end
	end
end
...
function Mapper:configure_device(device, config)
	...
	for _, intf in pairs(device.interfaces) do
		if intf.interface then
			for _=1,3 do
				local ret = intf.interface:run('AT+CPMS?\r\n', '', '+CPMS:', 1)
	...
	local is_radiopref_list = #config.network.radio_pref > 1
	local mode = "0,0,0"
	if selected_radio.type == "gsm" then
		mode = is_radiopref_list and "0,0,1" or "1,0,0"
	elseif selected_radio.type == "umts" then
		mode = is_radiopref_list and "0,0,2" or "2,0,0"
	elseif selected_radio.type == "lte" then
		mode = is_radiopref_list and "0,0,3" or "6,0,0"
	end
...
function M.create(runtime, device) --luacheck: no unused args
	...
	elseif device.pid == "1709" then
		control_ports = attty.find_tty_interfaces(device.desc, { number = 0x0 })
		modem_ports = attty.find_tty_interfaces(device.desc, { number = 0x1 })
	else
