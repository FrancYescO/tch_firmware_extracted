...
function Mapper:configure_device(device, config)
	...
	-- In D-Link DWM-222 dongle, on config change it was observed that previous RMCALL session was still connected but data
	-- was not passing through. So, forcibly disconnecting existing RMCALL session for all data sessions during config change.
	for _, session in ipairs(device.sessions or {}) do
		if session.proto == "dhcp" then
			device:send_command(string.format("AT$QCRMCALL=0,1,%d",session.pdp_type or default_qcrmcall_pdp_type), 30000)
		end
	end
	...
	local mode = 2
	-- When radio_pref = lte, doing COPS=2 results in auto registration by DWM-222 dongle
	-- and so attach fails. Hence, forcing mode to auto for fixing attach issue.
	if selected_radio.type == "gsm" then
		mode = 3
	elseif selected_radio.type == "umts" then
		mode = 1
	end
	if device.bmmobapref_mode ~= mode then
		device:send_command(string.format("AT+BMMOBAPREF=%d,0", mode))
		device.bmmobapref_mode = mode
	end
...
function Mapper:set_attach_params(device, profile)
	...
	-- In D-Link DWM-222 dongle, on session config change it was observed that previous RMCALL session was still connected
	-- but data was not passing through. So, forcibly disconnecting existing RMCALL session for all device sessions.
	for _, session in ipairs(device.sessions or {}) do
		if session.proto == "dhcp" then
			local pdp_type = session.pdp_type or default_qcrmcall_pdp_type
			-- It was observed that disconnect of RMCALL was not happening. So below line was added as a work-around
			-- hack to start/refresh the RMCALL session and then disconnect then same.
			device:send_multiline_command(string.format("AT$QCRMCALL=1,1,%d", pdp_type), "$QCRMCALL:", 300000)
			device:send_command(string.format("AT$QCRMCALL=0,1,%d", pdp_type), 30000)
		end
	end
		...
		if device.network_interfaces and device.network_interfaces[cid] then
			...
			if type(status) == "table" and type(status.interface) == "table" then
				for _, interface in pairs(status.interface) do
					if interface.device == device.network_interfaces[cid].name and (interface.proto == "dhcp" or interface.proto == "dhcpv6") then
