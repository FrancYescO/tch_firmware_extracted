...
function Mapper:start_data_session(device, session_id, profile)
	...
	if ret then
		if profile.authentication and profile.password and profile.username then
			...
			return device:send_command(cmd, 5000, 1, true)
		else
...
function Mapper:stop_data_session(device, session_id)
	...
	local ret = device:send_command(string.format('AT^NDISDUP=%d,0', (session_id + 1)), 5000, 1, true)
...
function Mapper:unsolicited(device, data, sms_data)
	...
	elseif helper.startswith(data, "^CCALLSTATE:") then
		...
		if state then
			...
			if event then
				device:send_event("mobiled.voice", { event = "call_state_changed", call_id = call_id, dev_idx = device.dev_idx, call_state = device.calls[call_id].call_state })
			end
