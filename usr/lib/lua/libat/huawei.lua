...
local function match_ndisstat(data)
	...
	local stat4, stat6 = data:match('%^NDISSTATQRY:%s?(%d),%d*,,"IPV4",(%d),%d*,,"IPV6"')
	if not stat4 then
...
function Mapper:get_time_info(device, info)
	...
	if ret then
		...
		if info.localtime and info.timezone then
			info.utctime = info.localtime - info.timezone * 60
		end
	end
end
...
function Mapper:unsolicited(device, data, sms_data) --luacheck: no unused args
	...
	elseif helper.startswith(data, "^NWTIME:") then
		...
		if event_data.localtime and event_data.timezone then
			event_data.utctime = event_data.localtime - event_data.timezone * 60
		end
...
function Mapper:get_network_interface(device, session_id)
	if device.product == "K4305" then
		if device.network_interfaces and session_id == 1 and device.network_interfaces[1] then
			return device.network_interfaces[1].name
		end
	end
end
