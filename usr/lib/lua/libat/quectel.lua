...
function Mapper:get_network_info(device, info)
	...
	ret, err = device:send_singleline_command('AT+QSPN', "+QSPN:")
	if ret then
...
local function get_complete_revision(device)
	...
	if revision then
		local prefix, suffix = revision:match("^(.+%uAR%d+A%d+)(M4G.*)$")
	...
	if device.firmware_upload.exit_status == 0 then
		...
		if not device.calls or not next(device.calls) then
			if run_upgrade_script(device, "preflash_commands.lua") then
				...
				-- Detach and prevent the module ffrom reattaching for 10 seconds.
...
local function check_load_and_write_chunk(device)
	...
	local chunk_size = table.remove(device.firmware_upload.chunks, 1)
	if chunk_size then
		local write_timeout = 2 -- s
		local limited_source = atchannel.limited_source(device.firmware_upload.async_source, chunk_size)
		if limited_source then
			...
			if write_result and tonumber(write_result:match('%+QFWRITE:%s*(%d+)%s*,%s*%d+')) == chunk_size then
				-- Schedule the timer to expire immediately if there are still chunks waiting to
				-- be processed. This will give other processes in mobiled a chance to run.
				if device.firmware_upload.chunks[1] or device.firmware_upload.finished then
	...
	elseif device.firmware_upload.finished then
...
function Mapper:network_attach(device)
	...
	if device.cops_command and device:send_singleline_command("AT+COPS?", "+COPS:") == "+COPS: 2" then
		if not device:start_command(device.cops_command, 60000) then
...
local function set_timer_if_not_pending(timer, timeout)
...
function Mapper:handle_event(device, message)
	if message.event == "command_finished" then
		...
		if device.current_scan and device.scanresults.scanning and not device.current_scan.started then
			...
			local ret = device:start_singleline_command('AT+COPS=?', '+COPS:', (5*60*1000)) -- start the network scan
			if not ret then
				device.scanresults.scanning = false
				device.current_scan = nil
			else
				device.current_scan.started = true
			end
		end
	...
	elseif message.event == "async_chunk_received" then
		if device.firmware_upload then
			table.insert(device.firmware_upload.chunks, message.num_bytes)
			set_timer_if_not_pending(device.firmware_upload.write_timer, 0)
		end
	elseif message.event == "async_end_of_file" then
		if device.firmware_upload then
			device.firmware_upload.finished = true
			set_timer_if_not_pending(device.firmware_upload.write_timer, 0)
		end
	end
end
