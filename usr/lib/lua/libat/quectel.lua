...
local config_defaults = {
	...
	audio_volume_level = { default = 3, type = "number" },
	audio_side_tone_gain = { default = 0, type = "number" },
	audio_mode = { default = 0, type = "number" },
	data_audio_digital_tx_gain = { default = -1, type = "number" },
	data_audio_codec_tx_gain = { default = -1, type = "number" },
	data_audio_digital_rx_gain = { default = -1, type = "number" },
	data_audio_volume_level = { default = -1, type = "number" },
	data_audio_side_tone_gain = { default = -1, type = "number" },
	data_audio_mode = { default = -1, type = "number" },
...
local function set_call_state(device, call, new_call_state)
	if call.call_state == "dialing" or call.call_state == "delivered" then
		device.delivered_calls = device.delivered_calls - 1
	end
	if new_call_state == "dialing" or new_call_state == "delivered" then
		device.delivered_calls = device.delivered_calls + 1
	end

	device.poll_counter = 0
	if device.delivered_calls == 0 then
		device.poll_period = 3
	else
		device.poll_period = 1
	end

	call.call_state = new_call_state
end
		...
		if call_id and ( call_type == "voice" or call_type == "emergency" ) then
			...
			if current_dsci_call_state == "disconnected" then
				if device.calls[call_id].call_state ~= "disconnected" then
					set_call_state(device, device.calls[call_id], current_dsci_call_state)
...
function Mapper:destroy_device(device, force) --luacheck: no unused args
	 if device.calls then
		for _, call in pairs(device.calls) do
			...
			set_call_state(device, call, "disconnected")
			send_call_disconnect_event(device, call)
		end
		device.calls = nil
	end
			...
			for _, call in pairs(ret) do
				...
				if module_call_id then
					...
					if remote_party == "sip:mmtel" or remote_party == "mmtel" then
						...
						set_call_state(device, device.calls[module_call_id], voice.clcc_call_states[call_state].call_state)
...
function Mapper:periodic(device)
	if device.poll_counter == 0 then
		device:get_unsolicited_messages()
	end
	device.poll_counter = (device.poll_counter + 1) % device.poll_period
	return true
end
