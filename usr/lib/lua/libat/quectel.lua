...
local function check_load_and_write_chunk(device)
	...
	if not device.firmware_upload then
		device.runtime.log:error("check_load_and_write_chunk called but device.firmware_upload is NIL. SW Error")
		return
	end
	...
	if load then
		...
		if load_1 and load_1 > load_1_threshold and device.firmware_upload.max_write_backoff_attempts < 3 then
			...
			if device.firmware_upload.write_backoff_time == maximum_write_backoff_time then
				device.firmware_upload.max_write_backoff_attempts = device.firmware_upload.max_write_backoff_attempts + 1
			end
	...
	device.firmware_upload.max_write_backoff_attempts = 0
