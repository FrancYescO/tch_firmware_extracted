...
function Mapper:configure_device(device, config)
	local reset_required, ret = false
...
function Mapper:init_device(device)
	if device:get_mode() == "upgrade" then
		...
		-- Disable the network lock as it will prevent a lot of AT commands from working.
		device:send_command('AT+QNWLOCK="common/4G",0')
...
function Mapper:set_attach_params(device, profile) --luacheck: no unused args
	-- Deregister from the network before setting the attach parameters otherwise, this will not take effect.
