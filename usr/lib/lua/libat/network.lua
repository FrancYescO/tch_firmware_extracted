...
function M.parse_creg_state(data)
	...
	local stat, lac, cid, act = string.match(data, '+CG?REG:%s?%d,(%d),"(%x*)","(%x*)",(%d)')
	if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16), tonumber(act) end
	-- +CREG: <n>, <stat>, <lac>, <cid>
	stat, lac, cid = string.match(data, '+CG?REG:%s*%d,(%d),"(%x*)","(%x*)"')
	if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
	-- +CREG: <stat>, <lac>, <cid>, <radio type>
	stat, lac, cid, act = string.match(data, '+CG?REG:%s*(%d),"(%x*)","(%x*)",(%d)')
	if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16), tonumber(act) end
	-- +CREG: <n>, <stat>, <lac>, <cid>
	stat, lac, cid = string.match(data, '+CG?REG:%s*%d,%s?(%d),%s?(%x*),%s?(%x*)')
	if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
	-- +CREG: <stat>, <lac>, <cid>
	stat, lac, cid = string.match(data, "+CG?REG:%s*(%d),%s?(%x*),%s?(%x*)")
	if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
	-- +CREG: <n>, <stat>
	stat = string.match(data, "+CG?REG:%s*%d,%s?(%d)")
	if stat then return tonumber(stat) end
	-- +CREG: <stat>
	stat = string.match(data, "+CG?REG:%s?(%d)")
	if stat then return tonumber(stat) end
...
function M.get_plmn(device)
	...
	if ret then
		local oper = string.match(ret, '+COPS:%s?%d,%d,"(.+)"')
	...
	if ret then
		description = string.match(ret, '+COPS:%s?%d,%d,"(.+)"')
	end
...
function M.get_ps_state(device)
	local ret = device:send_singleline_command("AT+CGREG?", "+CGREG:")
	if ret then
		local state = string.match(ret, '^+CGREG:%s?%d,(%d+)')
...
function M.get_cs_state(device)
	...
	if ret then
		local state = string.match(ret, '^+CREG:%s?%d,(%d+)')
