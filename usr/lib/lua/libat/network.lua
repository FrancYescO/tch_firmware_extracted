...
function M.parse_creg_state(data)
    ...
    local stat, lac, cid, act = string.match(data, '+CREG:%s?%d,(%d),"(%x*)","(%x*)",(%d)')
    if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16), tonumber(act) end
    -- +CREG: <n>, <stat>, <lac>, <cid>
    stat, lac, cid = string.match(data, '+CREG:%s*%d,(%d),"(%x*)","(%x*)"')
    if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
    -- +CREG: <stat>, <lac>, <cid>, <radio type>
    stat, lac, cid, act = string.match(data, '+CREG:%s*(%d),"(%x*)","(%x*)",(%d)')
    if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16), tonumber(act) end
    -- +CREG: <n>, <stat>, <lac>, <cid>
    stat, lac, cid = string.match(data, '+CREG:%s*%d,%s?(%d),%s?(%x*),%s?(%x*)')
    if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
    -- +CREG: <stat>, <lac>, <cid>
    stat, lac, cid = string.match(data, "+CREG:%s*(%d),%s?(%x*),%s?(%x*)")
    if stat then return tonumber(stat), tonumber(lac, 16), tonumber(cid, 16) end
    -- +CREG: <n>, <stat>
