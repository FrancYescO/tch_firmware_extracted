...
function M.create(runtime, params, tracelevel)
	...
	if device.vid == "12d1" then
		...
		if #ctrl_ports == 0 then
			...
			-- Huawei K4203 needs to use the control channel to bring up the data sessions
			if device.pid ~= "155e" then
			...
			if #ctrl_ports == 0 then
				...
				-- and all interfaces have 0xff as protocol...
	...
	elseif device.vid == "19d2" and device.pid == "0031" then
		...
		device.sessions[1] = { proto = "ppp" }
	-- ZTE MF110
	elseif device.vid == "19d2" and device.pid == "0117" then
		ctrl_ports = attty.find_tty_interfaces(device.desc, { number = 0x1 })
		data_ports = attty.find_tty_interfaces(device.desc, { number = 0x2 })
		device.sessions[1] = { proto = "ppp" }
	...
	elseif device.vid == "0f3d" or device.vid == "1199" then
		ctrl_ports = attty.find_tty_interfaces(device.desc, { number = 0x3 })
	end
