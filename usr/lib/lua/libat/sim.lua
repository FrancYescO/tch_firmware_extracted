...
local function check_iccid(iccid)
    if iccid then
        if #iccid == 20 and iccid:sub(19,19) == 'F' then
            local result = {}
            local i = 1
            while i < #iccid do
                table.insert(result, iccid:sub(i+1,i+1))
                table.insert(result, iccid:sub(i,i))
                i = i + 2
            end
            iccid = table.concat(result, "")
        end
        if #iccid == 20 and iccid:sub(20,20) == 'F' then
