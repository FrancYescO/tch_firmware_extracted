...
function M.get_state(device)
	local ret, err = device:send_singleline_command("AT+CPIN?", "+CPIN:")
	local sim_state = ""
	if ret then
		if ret:match("SIM PIN") then
			sim_state = "locked"
		elseif ret:match("SIM PUK") then
			sim_state = "blocked"
		elseif ret:match("READY") then
			sim_state = "ready"
		end
	elseif err then
		if err:match("^cme error %d+: sim failure") or err:match("^cme error %d+: sim not inserted$") then
			sim_state = "not_present"
		elseif err:match("^cme error %d+: sim busy$") then
			sim_state = "busy"
		elseif err:match("^cme error %d+: sim failure") or err:match("^cme error %d+: sim wrong$") then
			sim_state = "error"
		end
	end
	return sim_state
...
function M.get_forbidden_plmn(device)
	...
	if ret then
		local output = ret:match('+CRSM:%s?%d+,%d+,"(.-)"')
		if output then
...
function M.get_imsi(device)
	for _ = 1, 5 do
		...
		if ret then
  			local imsistr = ret:match('+CIMI:%s?(%d+)')
			if imsistr and tonumber(imsistr) then return imsistr end
		end
	end
	return nil
end
...
function M.get_iccid(device)
	...
	if ret then
		return M.check_iccid(ret:match('+CRSM:%s?%d+,%d+,"(.-)"'))
	end
	ret = device:send_singleline_command("AT+CCID", "+CCID:")
	if ret then
		return M.check_iccid(ret:match('+CCID:%s?(%d+)'))
	end
	ret = device:send_singleline_command("AT+ICCID", "ICCID:")
	if ret then
		return M.check_iccid(ret:match('^.-ICCID:%s?(%d+)'))
	end
	return nil
end
...
function M.unlock(device, pin_type, pin) --luacheck: no unused args
	local ret, err = device:send_command('AT+CPIN="' .. pin .. '"', 2000)
	if not ret then
		if err:match("^cme error %d+: password wrong$") then
...
function M.unblock(device, pin_type, puk, newpin) --luacheck: no unused args
	local ret, err = device:send_command('AT+CPIN="' .. puk .. '","' .. newpin .. '"', 2000)
	if not ret then
		if err:match("^cme error %d+: password wrong$") then
...
function M.change_pin(device, pin_type, pin, newpin) --luacheck: no unused args
	local ret, err = device:send_command('AT+CPWD="SC","' .. pin .. '","' .. newpin .. '"', 2000)
	if not ret then
		if err:match("^cme error %d+: password wrong$") then
			return nil, "Wrong PIN code provided"
		elseif err == "command pending" then
...
function M.disable_pin(device, pin_type, pin) --luacheck: no unused args
	...
	local ret, err = device:send_command('AT+CLCK="SC",0,"' .. pin .. '"', 2000)
	if not ret then
		if err:match("^cme error %d+: password wrong$") then
			return nil, "Wrong PIN code provided"
		elseif err:match("^cme error %d+") and not enabled then
			return true
		elseif err == "command pending" then
			return nil, "Device busy, please try again later"
		end
...
function M.enable_pin(device, pin_type, pin) --luacheck: no unused args
	local enabled = M.get_locking_facility(device, "SC")
	local ret, err = device:send_command('AT+CLCK="SC",1,"' .. pin .. '"', 2000)
	if not ret then
		if err:match("^cme error %d+: password wrong$") then
			return nil, "Wrong PIN code provided"
		elseif err:match("^cme error %d+") and enabled then
			return true
		elseif err == "command pending" then
			return nil, "Device busy, please try again later"
		end
