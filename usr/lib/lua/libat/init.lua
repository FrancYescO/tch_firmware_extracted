...
local ubus_conn = nil
local M = {}
-- Pre-declare function start_network_scan to avoid error "attempt to call global 'start_network_scan' (a nil value)"
-- as it is called in run_attach_finished_action before it is declared and defined
local start_network_scan
		...
		for _ = 1, 5 do
			...
			if tonumber(ret) then
				...
				break
 				...
 				device.buffer.device_info.imei = ret and string.match(ret, "+CGSN:%s?(%d+)") or ""
				if device.buffer.device_info.imei ~= "" then
   					break
  				end
			end
		end
...
local function configure_device(device, config)
	...
	if config.network then
		...
		if device.cops_command ~= cops_command then
			-- Detach from the network before deregistration.
			-- In ZTE dongle, deregistration was failing when already attached to network
			device:send_command("AT+CGATT=0")
...
local function run_attach_finished_action(device)
	if not run_action(device.dev_idx, "attach_finished") then
		-- Check if network scan command was triggered (device.scanresults.scanning) and it was not actually
		-- started (device.current_scan.started) as AT channel was busy with cops or attach command.
		-- When these conditions are met, explicitly detach and manual deregister from network.
		-- Then, start network scan by sending AT+COPS=? command to the device.
		-- Instead of checking for the variable scope of device.current_scan and device.scanresults only
		-- device.current_scan is checked.
		if device.current_scan and device.scanresults.scanning and not device.current_scan.started then
			device:send_command("AT+CGATT=0", 150000) -- Detach from network
			device:send_command("AT+COPS=2", 60000)   -- Manual deregister from network

			start_network_scan(device)
		end
	end
end

function start_network_scan(device)
	local function handle_success(result)
		local network_scan_list = {}
		for section in string.gmatch(result, '%((%d,".-",".-",".-",.-,?)%)') do
			local entry = {}
		...
		device.scanresults.duration = os.difftime(os.time(), device.current_scan.start_time)
		device.scanresults.scanning = false
		device.current_scan = nil

		run_attach_finished_action(device)
	end

	local function handle_failure(message)
		if device.current_scan.tries_left == 0 then -- The scan has failed all tries
			...
			device:start_singleline_command('AT+COPS=?', '+COPS:', 300000, handle_success, handle_failure)
		end
	end

	if device:start_singleline_command('AT+COPS=?', '+COPS:', 300000, handle_success, handle_failure) then
...
local function network_scan(device, start)
	if start then
		...
		if device.cops_pending or device.attach_pending then
			device.runtime.log:info("AT channel is busy with COPS or CGATT command. NW scan will be attempted later")
		...
		for _, device in pairs(devices) do
			for _, interface in pairs(device.interfaces) do
				if interface.interface then
					interface.interface:enable_logging(config.tracelevel >= 6)
				end
			end
		end
	end
...
local function execute_command(device, command)
	...
	if command:match("^[Aa][Tt]") then
		local ret, errMsg = device:send_multiline_command(command, "", 10000)
		if not ret then
			return errMsg
		end
		return table.concat(ret, "\n")
	end
	return nil, string.format('Invalid command: "%s"', command)
end
...
local function network_attach(device)
	...
	if device.cops_command and device:send_singleline_command("AT+COPS?", "+COPS:") == "+COPS: 2" then
		local function handle_success(result)
		...
		if not device:start_command(device.cops_command, 60000, handle_success, handle_failure) then
		...
		local function handle_failure(message)
			device.attach_pending = nil
			local error_message = device:send_singleline_command('AT+CEER', '+CEER:')
			device.runtime.log:notice("Attach failed %s", error_message and "with error message: " .. error_message:gsub("^%+CEER:%s*", "") or "without error message")
			run_attach_finished_action(device)
		end

		if not device:start_command('AT+CGATT=1', 150000, handle_success, handle_failure) then
			return nil, "Failed to attach"
		end
