...
local function init_device(device)
	runtime.log:info("We have " .. #device.control_interfaces .. " AT control interface(s)")
...
function M.destroy_device(dev_idx, force)
	...
	for i=#device.control_interfaces,1,-1 do
		local intf = device.control_interfaces[i]
		if intf.channel then
			intf:close()
		end
		table.remove(device.control_interfaces, i)
	end
...
local function get_network_info(device, info)
	info.nas_state, info.location_area_code, info.cell_id = network.get_state(device)
...
function M.get_session_info(dev_idx, session_id)
	...
	if info.proto == "ppp" then
		if #device.data_interfaces > 0 then
			info.ppp = {
				device = device.data_interfaces[1].port
			}
		end
	end
	...
	if config and config.tracelevel then
		for _, device in pairs(devices) do
			for _, interface in pairs(device.control_interfaces) do
				interface:set_tracelevel(config.tracelevel)
			end
		end
	end
	return true
...
local function set_power_mode(device, mode)
	if mode == "lowpower" then
		...
		device.buffer.network_info = {}
		device.buffer.radio_signal_info = {}
		return device:send_command('AT+CFUN=0')
	end
...
local function execute_command(device, command)
	local parts = {}
	for word in command:gmatch("%S+") do table.insert(parts, word) end
	if #parts >= 2 then
		if parts[1] == "AT" then
			local ret = device:send_multiline_command(parts[2], parts[3] or "")
			if not ret then
				return "ERROR"
			end
