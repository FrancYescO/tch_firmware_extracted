...
function M.process(runtime, data, dev_idx)
	if data and data.mobiled_network_serving_system then
		...
		for _, param in pairs(monitor_params) do
			if last_params[dev_idx][param] and data.mobiled_radio_signal_quality and data.mobiled_radio_signal_quality[param] and data.mobiled_radio_signal_quality[param] ~= last_params[dev_idx][param] then
				local event_data = {
					dev_idx = dev_idx,
