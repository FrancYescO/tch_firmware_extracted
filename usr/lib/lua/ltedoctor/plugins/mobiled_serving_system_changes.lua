...
function M.process(runtime, data, dev_idx)
	if data and data.mobiled_network_serving_system then
		...
		for _, param in pairs(monitor_params) do
			if last_params[dev_idx][param] and data.mobiled_network_serving_system and data.mobiled_network_serving_system[param] and data.mobiled_network_serving_system[param] ~= last_params[dev_idx][param] then
				local event_data = {
					dev_idx = dev_idx,
