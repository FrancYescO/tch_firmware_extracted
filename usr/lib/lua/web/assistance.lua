...
local function checkUpdate(assistant, permanent, password)
    ...
    if type(pswcfg)=="table" and type(password)=="table" and
       (pswcfg.salt~=password.salt or pswcfg.verifier~=password.verifier) then
         return true
...
function M.enable()
    if not enabled then
        if not setup_done then
            ngx.log(ngx.ERR, "web.assistance.setup not called before enable")
            ngx.exit(ngx.HTTP_INTERNAL_SERVER_ERROR)
        end
