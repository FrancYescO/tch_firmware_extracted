...
-- check if there is any change for _mode, _pswcfg and WAN IP/Status change
-- @return [boolean] When there is any change for _mode and _pswcfg
-- @return [boolean, boolean] When there is change in WAN IP/Status and flag to ignore the unlock user
...
function Assistant:enable(bActive, bPermanent, password)
  if self._reload_config and not self:enabled() then
    ...
    local isCheckUpdate, WANCheck = checkUpdate(self, bPermanent, password)
    -- disable assistance if its cfg needs update
    if isCheckUpdate and self:enabled() then
        local ok, msg = assistant_disable(self, WANCheck)
        if not ok then
            return nil, msg
        end
        changed = true
...
function assistant_enable(self)
    ...
    if user then
        ...
        self:activity()
	if self._hardtimeout then
	    self._ignoreactivity = true
	end
...
function assistant_disable(self, WANCheck)
    local user = self._mgr.users[ self._user ]
    if user then
	if not WANCheck then
...
function Assistant:activity()
    ...
    -- Note: will not reset activity timer if ignoreactivity flag is set true
    if self._psw and not self._ignoreactivity then
...
local function newAssistant(config, sessionmgr)
    ...
    local assistant = {
        ...
        _permanent = false;
        _pswcfg = nil;
	_ignoreactivity = false; --when set true will disable assistant after timeout even if remote ui is actively used
    }
