...
local function createAttributesString(attributes)
    ...
    for k,v in pairs(attributes) do
        attr[#attr + 1] = format('%s="%s"', k, v:gsub('"', '&quot;'))
    end
    return concat(attr, " ")
end
...
local function createTableDataColumn(column, v)
    ...
    elseif column.type == "checkboxgroup" then
        content[#content + 1] = M.createSimpleCheckboxGroup(column.name, column.values, v, { input = { disabled = "disabled" } })
    ...
    for i,l in ipairs(data) do
        if editing == i then
            ...
            if canEdit == true and allowedindexes[i].canEdit then
                ...
                content[#content + 1] = M.createSimpleButton("", "icon-edit", attrEdit)
            end
            content[#content + 1] = " "
            if canDelete == true and allowedindexes[i].canDelete then
                ...
                content[#content + 1] = M.createSimpleButton("","icon-remove-sign icon-large", attrDelete)
            end
            content[#content + 1] = "</td></tr>"
        end
    end
...
function M.createSimpleLight(value, text, attributes)
    ...
    if value ~= nil then
        if value == true or value == "1" then
            defaults.light.class = defaults.light.class .. " green"
    elseif value == "2" then
        defaults.light.class = defaults.light.class .. " orange"
        else
            defaults.light.class = defaults.light.class .. " off"
        end
    end
...
-- @return #table for ngx.print
function M.createHeader(name, hasAdvanced, hasRefresh, autorefresh)
 ...
 <div class="modal-header" ]] .. htmlautorefresh .. [[>
  <div class="row-fluid">
    ...
    if hasRefresh == true then
        html[#html + 1] = format('<span class="modal-action modal-action-refresh"><i class="icon-refresh"></i> %s</span>', T"refresh data")
    end
    ...
    if hasAdvanced == true then
        html[#html + 1] = format([[
        <span class="modal-action">
            <span class="modal-action-advanced hide"><i class="icon-minus-sign"></i> %s</span>
            <span class="modal-action-advanced"><i class="icon-plus-sign"></i> %s</span>
        </span>
...
function M.createCardHeader(title, modalPath, switchName, switchValue, attributes)
    ...
    local html = {
      [[<div class="header">]],
      format("<div %s><p>%s</p></div>", header, title)
    }
