...
-- @param #string helpmsg
-- @return #string
function M.createLabel(desc, value, attributes, helpmsg, progressBar)
    local defaults = {
        group = {
            class="control-group"
        },
    ...
    local progressHtml = {}
    if progressBar then
      progressHtml = createProgressBar(progressBar)
    end
...
-- @function [parent=#ui_helper] createProgressBar
-- @param #string controller/agent
-- @return #table
function createProgressBar(mode)
    local progressHtml = {}
    if mode then
      local class = "flex-container"
      local classAgent = "flex-container-agent"
      local style = 'height:20px;position:absolute;left:330px'
      if find(mode, "controller") then
          progressHtml = {
              format("<div class=%s style=%s><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div>", class, style, "controller1", "controller2", "controller3", "controller4", "controller5", "controller6", "controller7"), "</div> <div style='position: relative;left: 93px;top: 18px;'><span id='status-text-box' class='status-text-box-class'></span></div>"
          }
      elseif find(mode, "agent") then
          progressHtml = {
              format("<div class=%s style=%s><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div><div id=%s></div>", classAgent, style,  "agent1", "agent2", "agent3", "agent4", "agent5", "agent6"), "</div> <div style='position: relative;left: 92px;top: 22px;'><span id='status-text-box-agent' class='status-text-box-class'></span></div>"
          }
      end
    end
return progressHtml
...
-- @param #string helpmsg
-- @return #table
function M.createSwitch(desc, name, value, attributes, helpmsg, progressBar)
    local defaults = {
        group = {
            class = "control-group"
        },
...
-- @param #string helpmsg
-- @return #table array for ngx.print
function M.createSimpleInputSelect(name, values, current, attributes, helpmsg, newList)
    local defaults = {
        select = {
            class = "span3",
            name = name,
    ...
    for i,v in ipairs(values) do
        ...
        if v[1] == "custom" and newList and current ~= "" then
          selected = 'selected="selected"'
          v[1] = current
          v[2] = current
        end
...
-- @param #string helpmsg
-- @return #table
function M.createInputSelect(desc, name, values, current, attributes, helpmsg, newList)
    local defaults = {
        group = {
            class="control-group"
        },
    ...
    local html = {
        format("<div %s><label %s>%s</label><div %s>",group, label, desc, controls),
        M.createSimpleInputSelect(name, values, current, attributes, helpmsg, newList),
        "</div></div>"
    }
    return html
end
...
--    @param #table attributes
--    @return #string
function M.createSimpleButton(buttontext, icon, attributes, customId)
    local buttonid = customId or buttontext
    local defaults = {
        button = {
            class = "btn",
            id = buttonid,
...
local function createTableDataEditColumn(v, data, helpmsg)
    ...
    elseif v.type == "select" then
        if v.default ~= "" then
          content[#content + 1] = M.createSimpleInputSelect(v.name, v.values, data or v.default or "", attributes, nil, v.default)
        else
...
local function createTableDataEdit(columns, data, add, helpmsg)
    ...
    for i,v in pairs(columns) do
        content[#content + 1] = '<td data-title="'..columns[i]["header"]..'">'
...
local function createTableButton(canAdd, editing, createMsg, newList, customId)
    local content = {}
    local buttonAttr = {
        button = {
            class = "btn-table-new"
    ...
    if (canAdd == true) then
        ...
        content[#content + 1] = M.createSimpleButton(createMsg,"icon-plus-sign", buttonAttr, customId)
        if newList ~= nil then
...
-- @function [parent=#ui_helper] createList
-- @param #table columns
-- @param #table data array
-- @param #table options
-- @param #table attributes
-- @param #table helpmsg
-- @return #table
local function createList(columns, data, options, attributes, helpmsg)
  local light_map = {
    ["0"] = "off",
    ["1"] = "green"
  }
  setmetatable(light_map, untaint_mt)
  local content = {}
  for row, value in ipairs(data) do
    local list = "Profile_" .. row
    content[#content + 1] = '<p>'
    content[#content + 1] = '<ul class="nav nav-list">'
    for index, info in ipairs(value) do
      if columns[index].name == "Profile" then
        local list = columns[index].name .. row
        content[#content + 1] = format('<li class="nav-header">%s</li>', list)
      end
      if columns[index].type == "light" then
        for k, name in ipairs(value) do
          if columns[k].name == "FriendlyName" then
            content[#content + 1] = '<ol>'
            content[#content + 1] = M.createLight("", nil, name, { light = { class = light_map[info] } })
            content[#content + 1] = '</ol>'
          end
        end
      elseif columns[index].name ~= "FriendlyName" then
        content[#content + 1] = format('<ol><strong>%s : </strong> %s</ol>', columns[index].name or columns[index].header, info)
      end
    end
      content[#content + 1] = '</ul>'
      content[#content + 1] = '</p>'
...
function M.createTable(columns, data, options, attributes, helpmsg)
    ...
    --createList function will be called in below scenarios.
    --when no of columns are greater than or equal to 7
    --when there is no option to add/edit and delete
    --when list view feature is supported.
    if #columns >= 7 and options.canEdit == false and options.canAdd == false and options.canDelete == false and options.tableid ~= "networkstats" and post_helper.isFeatureEnabled("listViewNotSupported", role) and post_helper.isFeatureEnabled("wifiDeviceslist", role) then
      local html = createList(columns, data, options, attributes, helpmsg)
      return html
    end
    ...
    local customId = options and options.customId
    ...
    content[#content + 1] = createTableButton(canAdd, editing, createMsg, newList, customId)
    content[#content + 1] = "</div>"
    return content
end
...
-- @return #table for ngx.print
function M.createModalTabs(tabs, uri)
    ...
    for _, tab in ipairs (tabs) do
      if uri then
        local active = ""
        if uri == ("/modals/" .. tab[1]) then
            active = "active"
        end
        html[#html+1] = string.format('<li class="%s"><a id ="%s" href="#" data-remote="/modals/%s">%s</a></li>', active, tab[3] or tab[2], tab[1], tab[2])
      else
        if tab.active then
          defaults["li"] = {
            ["class"] = tab.active,
...
function M.createCardHeader(title, modalPath, switchName, switchValue, attributes)
    ...
    local icon = createAttributesString(defaults["icon"])
    local title_id = string.gsub(title, " ", "_")

    local html = {
      [[<div class="header">]],
      format("<div %s><p id=%s_tab>%s</p></div>", header, title_id, title)
    }
...
-- @return #table for ngx.print
function M.createLanguageSelect(textdomain, current)
    local html = {}
    local languages = intl.listLanguages(textdomain)
    if languages and (#languages > 1) then
        html[#html + 1] = M.createSimpleInputSelect("webui_language", languages,current)
    end
    return html
end
