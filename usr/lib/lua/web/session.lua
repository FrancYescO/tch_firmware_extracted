...
local function generateNewIdentity(session, sessionID_req)
  ...
  session.XSRFtoken = generateRandom()
...
-- Retrieve the current user IP
-- @return remoteIP address
function Session:getCurrentUserIP()
...
function Session:checkXSRFtoken(token)
  if token ~= self.XSRFtoken then
    ngx.log(ngx.ERR, "POST without XSRF token")
    ngx.exit(ngx.HTTP_FORBIDDEN)
  end
  return true
end
...
local function createProxy(session)
  ...
  local checkXSRFtoken = function(_, token)
    return session:checkXSRFtoken(token)
  end
  ...
  local getCurrentUserIP = function()
    return session:getCurrentUserIP()
  end
  local proxy = {
    ...
    getCurrentUserIP = getCurrentUserIP
  }
  return setmetatable({}, {
    __index = proxy,
    __newindex = function()
