--NG-71847; NG-81787 implemented R&D generic Changes
...
function M.getMatchedContent (path, filter, num)
    ...
    if (type(res) == "table") then
        ...
        for _, v in pairs (res) do
			...
			local tmp_basic_path = v.path:match("^("..path.."[^%.]+)") .. "."
            if (basic_path == "" or tmp_basic_path ~= basic_path) then
                ...
                content[#content+1] = { path = basic_path, basepath = basic_path }
                match_count = 0
            end
