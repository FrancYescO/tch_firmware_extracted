...
function M.process(filename)
  filename = filename or ngx.var.request_filename
  local ext = filename:match("%.([^.]+)$")

  -- our response must not be cached because it's dynamically generated content
  ngx.header.cache_control = "no-cache"
  -- retrieve the content (as a function)
  local content
  if ext == "lp" then
    ngx.header.content_type = "text/html"
    content = lp.load(filename, ngx.var.uri)
  elseif ext == "lua" then
  ...
  -- set up gettext support for the content
...
do
  ...
  function M.get_cookies()
    ...
    local cookies = {}
    local text_cookie = ngx.var.http_cookie
