...
--   Note that the returned sessionID can be different from the given
--   one as the the sessionID may change on login or logout
local function verifySession(self, sessionID, remoteIP)
  local session = sessionID and self.sessions[sessionID]
  if session then
    ...
    if sessionExpired(self, session, sessionID) then
      return
    end
    local newid = sessionChanged(self, session, sessionID)
    if newid then
...
function SessionMgr:checkrequest()
  ...
  local sessionID_present = (sessionID ~= nil)
  ...
  session, sessionID = verifySession(self, sessionID, remoteIP)
  if not session then
    ...
    -- Set the cookie.
    setSessionCookie(self, sessionID)
  else
  ...
  if not rc then
    ...
    if cookie_header then
      if sessionID_present then
        cookie_header = gsub(cookie_header, "sessionID=%x+", new_session_cookie)
      else
