...
local function sessionExpired(self, session, sessionID, now, timeout)
  ...
  if timeLeft <=0 then
    if self.assistant and self._session_tracker.count == 1 then
      self.assistant:enable(false, false, nil)
    end
...
local function setSessionCookie(self, sessionID)
  local cookie = "sessionID="..sessionID.."; Path="..self["cookiepath"].."; HttpOnly; SameSite=Strict"
...
function SessionMgr:handleAuth()
  ...
  if M and session then
    ...
    local sessionKey, errmsg = verifier:session_key(M)
    if sessionKey then
      session:store("session_key", sessionKey)
    end
