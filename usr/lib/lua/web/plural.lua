...
local function div_op(op, left, right)
...
-- @param right the right operand
-- @returns an expression object
local function select_op(op, left, right)
...
-- @param right the right operand
-- @returns an expression object
local function cond_op(op, left, right)
...
-- @param right the right operand
-- @returns an expression object
local function not_op(op, right)
...
local function binop_reduce(binop)
    while true do
        ...
        if op.type=='unop' then
            ...
            table.remove(binop, idx+1) -- op/result still on index idx
            table.remove(binop, idx-1) -- so this is ok
        end 
    end
    while #binop>1 do
        -- still ?: ops left
        ...
        if idx>1 then
            ...
            table.remove(binop, idx+3)
            table.remove(binop, idx+2)
            table.remove(binop, idx+1)
            table.remove(binop, idx-1)
        else
...
local function expression(tokens)
    ...
    while true do
        ...
        elseif token.type=='(' then
            ...
            token = tokens:peek()
            if token.type ~= ')' then
                return nil, format('syntax error: ) expected, got %s', tostring(t))
            end
