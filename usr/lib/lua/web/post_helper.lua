...
local istainted, format, match, find, sub, lower, untaint = string.istainted, string.format, string.match, string.find, string.sub, string.lower, string.untaint
...
function M.secondsToTime(time)
  ...
  if (time_no and time_no >= 0) then
    local days =  floor(time_no / 86400)
    local hours =  floor(time_no / 3600) % 24
    local minutes = floor(time_no / 60) % 60
    local seconds = floor(time_no) % 60
    if (days > 0) then
      return format("%d%s %d%s %d%s %d%s", days, T"days", hours,  T"hours", minutes,  T"minutes", seconds, T"seconds")
    elseif (hours > 0) then
      return format("%d%s %d%s %d%s", hours, T"hours", minutes, T"minutes", seconds, T"seconds")
    elseif (minutes > 0) then
      return format("%d%s %d%s", minutes,  T"minutes", seconds, T"seconds")
    else
      return format("%d%s", seconds, T"seconds")
    end
  else
...
function M.validateStringIsMAC(value)
    ...
    if not value:match(mac_pattern) then
...
function M.validateQTN(value)
  ...
  elseif inet.isValidIPv4(value) == true then
    ...
    if qtnIP and qtnIP[1].ip_address == value then
      return nil, format(T"Cannot assign, %s in use by system.", value)
    end
    return true
  end
