...
function M.validateCount(value, object, key)
  ...
  local result = (result and result["value"]) or (result and result[1] and result[1].value or "")
  if result and result ~= "" then
    if saveonusb then
       max = 2*tonumber(untaint(result))
    else
      max = math.min(2*tonumber(untaint(result)), tonumber(max))
    end
  end
