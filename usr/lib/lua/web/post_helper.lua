...
-- Checks whether the received 'inputTime' is in HH:MM format
-- @function [parent=#post_helper] validTimeString
-- @param inputTime The inputTime will be checked in HH:MM format
-- @treturn #boolean true if valid nil+msg if invalid
-- @error Error message
function M.validTimeString(inputTime)
    if not inputTime then
        return nil, T"Invalid input"
    end
    local time_pattern = "^(%d+):(%d+)$"
    local hour, min = inputTime:match(time_pattern)
