...
function M.validateStringIsDomainName(value)
    ...
    repeat
        ...
        if strippedLabel ~= nil then
            ...
            local correctLabel = match(strippedLabel, "[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9]")

            if #strippedLabel == 1 then
                if not match(strippedLabel, "[a-zA-Z]") then
...
function M.validateStringIsLeaseTime(value)
    ...
    local leasetime_pattern = "^(%d+)[hmswd]?$"
    local number = value:match(leasetime_pattern)
    number = number and tonumber(number)
    if not number or number < 1 then
        return nil, T"Invalid value; enter a number greater than 0, followed by 's' for seconds or 'w' for week or 'd' for days or 'm' for minutes or 'h' for hours. No spaces."
    end
    return true
end
