...
function M.getActiveInterface()
  local activeIntf = proxy.get("uci.env.var.active_intf")
  activeIntf = activeIntf and activeIntf[1].value or "wan"
  return activeIntf
end

function M.getActiveInterface_v6()
  local activeIntf = proxy.get("uci.env.var.active_intf")
  return activeIntf and activeIntf[1].value == "titan" and "titan_v6" or "wan6"
end
...
function M.handleQuery(mapParams, mapValidation, doNotApply)
    setlanguage()
    ...
    if ngx.var.request_method == "POST" and ngx.var.content_type and match(ngx.var.content_type, "application/x%-www%-form%-urlencoded") then
        ...
        if (post_data["action"] == "SAVE") then
            ...
            for k,v in pairs(post_data) do
                ...
                -- Encrypted values will be decrypted and updated in content to save values in config
                if match(k, "passWord") or match(k, "wpa_psk") or match(k, "passwordcred") or match(k, "wep_key") or match(k, "ddns_password") or match(k, "ddnsv6_password") or match(k, "cwmpAcsPass") or match(k, "connectionrequestPass") or  match(k, "wpa3") or  match(k, "password@cred") then
            ...
            if validated and not errorsFound() then
                ...
                if ok then
                    if next(content) and not doNotApply then
                        ok, msg = proxy.apply()
                    end
...
function M.handleTableQuery(columns, options, filter, defaultObject, mapValidation)
    ...
    if ngx.var.request_method == "POST" and ngx.var.content_type and match(ngx.var.content_type, "application/x%-www%-form%-urlencoded") then
        ...
        -- Encrypted password in table will be decrypted and updated in content which will save in config
        if action == "TABLE-MODIFY" or action == "TABLE-ADD" then
            if cid == "lte_profiles" or cid == "vpnuser" and content.password and content.password ~= "" then
                content.password = decryptData(untaint(content.password))
            end
        end
...
-- @param value
-- @return #boolean, #string
function M.validateStringIsIP(ipTypeV4orV6)
    return function (value)
      if not ipTypeV4orV6 then
        local iptype = GetIPType(value)
        if iptype == 4 or iptype == 6 then
          return true
        end
        return nil, T"Invalid IP address."
      elseif ipTypeV4orV6 == 4 then
        if not value then
      ...
      elseif ipTypeV4orV6 == 6 then
        local k = {}
        for ip in value:gmatch("[^%s,]+") do
          k[#k+1] = ip
        end
        if #k > 1 then
          for ip in value:gmatch("[^%s,]+") do
            if not (ip and inet.isValidIPv6(untaint(ip))) then
              return nil, "Invalid IPv6 Address, address group is invalid."
            end
          end
        elseif not (value and inet.isValidIPv6(untaint(value))) then
          return nil, "Invalid IPv6 Address, address group is invalid."
        end
        return true
      end
...
function M.validateStringIsDomainName(value)
    ...
    if not string.match(value, "^[a-zA-Z0-9%.%-]+$") then
        return nil, T"Received invalid Domain Name"
    end
    ...
    repeat
        ...
        if strippedLabel ~= nil then
            if #strippedLabel == 0 then
                return nil, T"Dot at start or end of domain name is not allowed. Successive dots are not allowed in domain name."
            end
            ...
            elseif strippedLabel ~= correctLabel then
                for chr in string.gmatch(value, "[-.][-.]") do
                    if not match(chr, "[.][.]") and not match(chr, "[-][-]") then
                        return nil, T"Hyphen and dot should not follow eachother"
                    end
                end
                return nil, T"Domain name should start and end only with alphanumeric characters. Domain name cannot contain white space or special characters other than hyphen or dot."
            end
        end
...
function M.validateWEP(value)
    if value == nil or (#value ~= 5 and #value ~= 10 and #value ~= 13 and #value ~= 26) then
        return nil, T"Invalid length, a WEP key must be 5, 10, 13 or 26 characters long."
    end

    if (#value == 10 or #value == 26) and (not value:match("^[%x]+$")) then
        return nil, T"A WEP key of length 10 or 26 can only contain the hexadecimal digits."
    end
    return true
end
...
function M.validateDestinationIP(ipAddr, object)
  local validateStringIsIP = M.validateStringIsIP()
  local resIP, errMsg = validateStringIsIP(ipAddr)
  if not resIP then
    return nil, errMsg
  end
...
function M.validateURL(url,proto)
  ...
  if url then
    ...
    if addr and port then
      if M.validateStringIsPort(port) and (validateStringIsIP(addr) or M.validateStringIsDomainName(addr)) then
        return true
      end
    else
      if (validateStringIsIP(domain) or M.validateStringIsDomainName(domain)) then
        return true
      end
    end
  end
...
-- @param userRole Role of the user
-- @return true if feature is enabled and role is enabled for that feature
...
function M.isFeatureEnabled(feature, userRole)
  ...
  if allowedRoles and type(allowedRoles) == "table" then
    for _, role in ipairs(allowedRoles) do
      if role.value == userRole then
         return true
      end
  ...
  if allowedRoles and not next(allowedRoles) then
     return true
  end
end

--- Returns a table of options that holds variant specific values
-- Each section has multiple pages and each page has multiple options
-- @function [parent=#post_helper] getVariant
-- @param #table variants Table that contains the sections, pages and its options
-- @param #string section Represents the section name of the variant
-- @param #string page Represents the page name of the corresponding Section
-- @return #table or nil Returns the options for a given page for a given Section within a given table,
-- returns nil if there is no such variants, page or section.
...
--- Return a value that is specific to the variant
-- @function [parent=#post_helper] getVariantValue
-- @param #table variant Table that contains set of options for a given page in a given section
-- @param #string option Represents the options/elements of the corresponding page
-- @return #boolean, #string, #table or nil Returns the value that the option holds
-- for the corresponding page and section, returns nil if no such page, section or option occurs.
...
--- Check to see if the variant has a role that has access to a particular option
-- @function [parent=#post_helper] variantHasAccess
-- @param #table variant Table that contains set of options for a given page in a given section
-- @param #string option Represents the options/elements of the corresponding page
-- @param #string userRole Represents the role
-- @return #boolean Returns true if the option does not have any role or if there exists a userRole
-- that has access to the given option.
-- Returns false if the userRole has no access to the option or there exists no such userRole within role or
-- if there is no such page, section, option.
function M.variantHasAccess(variant, option, userRole)
    if variant and variant[option] and not variant[option].role then
        return true
    end
    return variant[option].role[userRole] or false
end

--- Check to see if the variant has an option that is visible in the given view
-- Each section name has multiple pages and each page
-- @function [parent=#post_helper] variantHasPageView
-- @param #table variant Table that contains set of options for a given page in a given section
-- @param #string option Represents the options/elements of the corresponding page
-- @param #string pageView Represents the view or mode of the GUI
-- @return #boolean Returns true if the option does not have any pageView key or if there exists a pageView that the option has access to.
-- Return false if the pageView is not available in the given option or if the pageView has no access to the option or
-- if there is no such page, section, option.
function M.variantHasPageView(variant, option, pageView)
    if variant and variant[option] and not variant[option].view then
        return true
    end
    return variant[option].view[pageView] or false
end
...
function M.validateUciName(value)
  if not match(value, "^[%w_]+$") then
    ...
    local success, errmsg = M.getValidateStringLengthInRange(1,63)(value)
    if not success then
...
-- Function to validate a DeviceName
-- value must be between 1 an 63 characters long
-- valid name must be non-empty string that consists of ASCII characters
-- @return true or nil+error message
function M.validateDeviceName(value)
  if not match(value, "^[!-~]+$") or string.match(value,"[^%w+%-?%.]") or (match(value, "^[-.]") or match(value, "[-.]$") or value:match("%-%.") or value:match("%.%-"))  then
    return nil, T"Invalid input."
  else
    local success, errmsg = M.getValidateStringLengthInRange(1,63)(value)
    if not success then
...
--- Is feature enabled or not based on view
-- @param feature Specific feature
-- @param currentView view of the user
-- @return true if feature is enabled
-- @return nil if feature is disabled
function M.featureEnabledInView(feature, currentView)
  local allowedViews = proxy.get("uci.web.feature.@"..feature..".")
  if not allowedViews then
     return true
  end
  for _, view in ipairs(allowedViews) do
    if view.value == currentView then
      return true
    end
  end
end

-- Compare tod rule for duplicate or overlap entries
function M.overlapCheck(oldDay, oldStart, oldStop, newDay, newStart, newStop)
  for _, dayNew in pairs(newDay) do
    for _, dayOld in pairs(oldDay) do
      if dayOld == dayNew then
        if (oldStart == newStart and oldStop == newStop) then
          return false, T"Duplicate contents are not allowed"
        end
        if (newStart < oldStop and newStop > oldStart) then
          return false, T"Overlap contents are not allowed"
        end
      end
    end
  end
...
function M.validateTODTime(value, object, key)
...
-- Function to validate the NAT
-- value must be 0 or 1(boolean)
-- @return true or nil+error message
function M.validateNat(value, object, key, forwdnum, zonenum)
  ...
  local valid, msg = M.validateBoolean(value, object, key)
  if valid and zonenum ~= "" and value ~= proxy.get("uci.firewall.zone."..zonenum..".masq")[1].value then
    ...
    local keyValue = "forwarding"..M.getRandomKey()
    if forwdnum == "" then
      forwdnum = proxy.add("uci.firewall.forwarding.", keyValue)
      proxy.set(firewall_value)
    end
    proxy.set("uci.firewall.forwarding.@"..forwdnum..".enabled", value == "1" and "0" or "1")
    proxy.apply()
  end
  return valid, msg
end
...
-- valid name must be non-empty string that consists of letters, digits or underscores
-- @return true or nil+error message
function M.validateUsername(value, object, key)
  local err,msg = M.validateNonEmptyString(value)
  if not err then
    return err,msg
  end
  ...
  if content_uci["wan_domain"] ~= "" then
    ...
    object["uci_wan_username"] = format('%s@%s', value, content_uci["wan_domain"])
  end

  return true
end

-- Function to validate a NTP
-- valid name must be domain name or IPv4 or IPv6
-- @return true or nil+error message
function M.validateNTP(value, object, key)
-- Adding localdevicemask in post to find the input is a network or broadcast IPv4 address in advancedIPValidation
  local wholeNumber = string.gsub(value, "%.", "")
  local validateStringIsIPv4 = M.validateStringIsIP(4)
  local validateStringIsIPv6 = M.validateStringIsIP(6)
  local mapParam = {
  ...
  object["localdevmask"] = mapParam.localdevmask

  if M.getValidateWholeNumber(wholeNumber) then
    local ok, errmsg = validateStringIsIPv4(value)
    if not ok then
      return nil, errmsg
    end
    ok, errmsg = M.validateLXC(value)
    if not ok then
      return nil, errmsg
    end
    if M.validateIPv4Netmask(value) then
      return nil, T"Cannot use IPv4 subnet mask."
    end
    if mapParam.ipaddr and mapParam.ipaddr == value then
      return nil, T"Cannot use the gateway IP address."
    elseif mapParam.wan_ipaddr and mapParam.wan_ipaddr == value then
      return nil, T"Cannot use the gateway WAN IP address."
    else
      return M.getOrValidation(M.getAndValidation(M.reservedIPValidation, M.advancedIPValidation), validateStringIsIPv6)(value, object, key)
    end
  else
    if not validateStringIsIPv6(value) then
      return M.validateStringIsDomainName(value)
    else
      return true
    end
  end
...
local function valueInList(object, inputList, macAddr)
  local fieldType = "acl_accept_list"
  ...
  for _, v in ipairs(inputList) do
    ...
    if pathIndex then
      local listType = find(v.path, "accept") and "acl_accept_list" or "acl_deny_list"
      if M.compareMACAddresses(v.value, macAddr) and (listType ~= fieldType or tonumber(object.index) ~= tonumber(pathIndex)) then
        return true
      end
    end
  end
...
-- To check whether given ACL Mac is valid or not
-- To check whether any overlap Mac address is given
-- @return true or nil+error message
function M.validateAclMac(macAddress, object, key, curap)
  local retval, errmsg = M.validateStringIsMAC(macAddress, object, key)
  if not retval then
    return retval, errmsg
  end
  local path = "uci.wireless.wifi-ap.@" .. curap
  local acceptDenyList = proxy.get(path .. ".acl_accept_list.", path .. ".acl_deny_list.")
  if valueInList(object, acceptDenyList, macAddress) then
    return nil, T"Mac address duplicate with either current whitelist or current blacklist"
  end
  return true
end

-- To check whether Radio and AP value is valid or not
-- @return true or nil+error message
function M.validateRadioandAp(value, object, key, pathap)
  local valid, msg = M.validateBoolean(value, object, key)
  if valid then
    local _, errmsg = proxy.set(pathap .. "admin_state", value)
    if errmsg then
      return nil, errmsg
    end
    proxy.apply()
...
-- To check whether given SSID is valid or not
-- To check whether any overlap SSID value is given
-- @return true or nil+error message
function M.validateSSIDWireless(value, wirelessList)
  local success, msg = M.getValidateStringLengthInRange(1,32)(value)
  if success then
    for _, v in ipairs(wirelessList) do
...
-- To validate device name
-- @return true if value matches or nil
function M.validateUnknownHostname(value)
  local pattern = "^Unknown%-%x%x:%x%x:%x%x:%x%x:%x%x:%x%x$"
  return value:match(pattern)
end

-- To validate the port given
-- To check if port is overlap with UPnP port range
-- @return true or nil+error message
function M.validPorts(ports)
  local curend = -1
  ...
  for _, v in ipairs(ports) do
    ...
    for _, l in ipairs(upnp_params) do
      if v.portProtocol == string.lower(l.proto) or v.portProtocol =="tcpudp" then
        ...
        elseif lanCheck then
          return nil, {lanport = "An existing UPNP mapping overlaps with the entered ports range"}
...
-- To check if source and destination has same IP or not
-- @return true or nil+error message
function M.validateNotSameSourceAndDestinationIP(value, object, key, ipType)
  local errmsg = T"Source IP cannot be same as Destination IP"
  if ipType == "ipv4" and object.destinationip == object.sourceip then
    return nil, errmsg
  elseif ipType == "ipv6" and object.dest_ip_v6 == object.src_ip_v6 then
    return nil, errmsg
  end
  return true
end

-- To validate ALG value
-- @return true or nil+error message
function M.validateAlgSip(value, object, key, alg_sip_paths)
  local success, msg = M.validateNonEmptyString(value)
  if success then
    for sipValue in ipairs(alg_sip_paths) do
      object["fw_alg_sip_" .. sipValue] = value
    end
    return true
  end
  return success, msg
...
-- To check destination and source IP based on subnet
-- @return true or nil+error message
function M.validateWANIP(value, object, key)
  local validateIPv4_Subnet = M.validateIPAndSubnet(4)
  if value == "" then
    return true
  end
  local ipAddress, netMask = value:match("^([^/]+)/?(%d*)$")
  if netMask == "" then
    --To add default subnet mask to the IPv4 Network Address if not explicitly mentioned.
    netMask = M.getDefaultSubnetMask(ipAddress)
    if netMask and M.isNetworkAddress(ipAddress, M.cidr2mask(netMask)) then
      object[key] = value .. "/" .. netMask
    end
  end
  object["localdevmask"] = netMask and M.cidr2mask(netMask)
  local retVal, msg = validateIPv4_Subnet(value, object, key)
  if retVal then
    retVal, msg = M.advancedIPValidation(ipAddress, object, key)
    if retVal then
      retVal, msg = M.reservedIPValidation(ipAddress)
    end
  end
  return retVal, msg
end

-- To check destination and source LAN IP based on subnet
-- @return true or nil+error message
function M.validateLANIP(value, object, key)
  local validateIPv4_Subnet = M.validateIPAndSubnet(4)
  if value == "" then
    return true
  end
  local retVal, msg = validateIPv4_Subnet(value, object, key)
  local ipAddress = value:match("^([^/]+)/?(%d*)$")
  local ipdetails = {
  ...
  local validateDeviceIPv4 = M.getValidateStringIsDeviceIPv4
  local validateIPDetails = validateDeviceIPv4(ipdetails.gw, ipdetails.netmask)
  if retVal then
    retVal, msg = validateIPDetails(ipAddress)
    if retVal then
      retVal, msg = M.reservedIPValidation(ipAddress)
    end
  end
...
-- To check whether given IPAddress is IPv4 or not
-- @return true or nil+error message
function M.validateIPv4(value, object, key)
  if string.find(value, ":") then
    return nil, T"Invalid IPv4 Address"
  end
  return M.validateIPAndSubnet(4)(value)
end

-- To check whether given IPAddress is IPv6 or not
-- @return true or nil+error message
function M.validateIPv6(value, object, key)
  if value == "" then
    return true
  end
  return M.validateIPAndSubnet(6)(value)
end

-- To check whether given Port is valid or not
-- @return nil+error message
function M.validateSyslogPort(value, data, key)
  value = value and match(value, "^[%s0]*(%d+)%s*$")
  local port = value and tonumber(value)
  if port and (floor(port) == port) and port >= 1 and port < 65536 then
    if data and key then
  ...
  return nil, T"Port is invalid. It should be between 1 and 65535."
end

-- To check whether given size is valid or not
-- @return nil+error message
function M.validateSyslogSize(value, data, key)
  value = value and match(value, "^[%s0]*(%d+)%s*$")
  local size = value and tonumber(value)
  if size and (floor(size) == size) and size >= 4 then
    if data and key then
      data[key] = value
    end
    return true
  end
  return nil, T"Size is invalid. Syslog size should be greater than 4 (KB)."
end

--Function to validate Count
--Checking the count is max of 60000 and min of 1
function M.validateCount(value, object, key)
  local max = 60000
  local saveonusb = object.saveonusb and object.saveonusb == "1" and true or false
  local refcountparam = saveonusb and "refcountusb" or "refcount"
  local result = proxy.get("rpc.system.tcpdump."..refcountparam) or {}
  if result[1] and result[1].value then
     max = saveonusb and 2*tonumber(untaint(result[1].value)) or math.min(2*tonumber(untaint(result[1].value)), tonumber(max))
  end
  return M.getValidateNumberInRange(1, tonumber(max))(value, object, key)
end
...
function M.validateConnReqNone(value, object)
...
function M.validateFilename(filename)
  if filename and #filename > 0 and #filename < 64 and match(filename, "^.*%.0$") then
    return true
  end
end

-- To filter the given IP is valid or not
-- @return true or nil+error message
function M.filterIPValidation(value, object)
  if not value then
    return nil, T"Invalid IP Address."
  end
  local ip = M.ipv42num(value)
 ...
 if startLoopback <= ip and ip <= endLoopback  then
   ...
   local ok, errmsg = M.validateLXC(value)
    if not ok then
      return nil, errmsg
    end
  if startMulticastRange <= ip and endMulticastRange >= ip then
  ...
  if object.localdevIP == value then
    return nil, T"Cannot use the GW IP."
  end
...
--@return nil+error message
function M.validateRelay(value, object)
  local validateStringIsIP = M.validateStringIsIP()
  if validateStringIsIP(value) then
    ...
    local success, errmsg = M.reservedIPValidation(value)
    if not success then
      return nil, errmsg
    end
    --Adding localdevicemask in post to find the input is a network or broadcast IPv4 address in advancedIPValidation
...
function M.validateUserUrl(value, object)
  if object.cwmpState ~= "1" then
    return false
  end
  local validateStringIsIP = M.validateStringIsIP() or ""
  local mapParam = {
    ipaddr = "rpc.network.interface.@lan.ipaddr",
  }
  content_helper.getExactContent(mapParam)
  if (validateStringIsIP(value) and validateStringIsIP(value) ~= nil) then
    if mapParam.ipaddr and mapParam.ipaddr == value then
      return nil, T"Cannot use the gateway IP address."
    end
  end
  return M.getValidateStringLengthInRange(1,255)(value)
end

-- validate the password
function M.validatePass(value, object, key)
  if object.cwmpState ~= "1" then
    return false
  end
  return M.getValidationPassword(M.getValidateStringLengthInRange(1, 255))(value, object, key)
end

function M.validateExactLength(len)
  return function(value, object, key)
    if type(value) ~= "string" and not istainted(value) then
...
function M.validatehostName(value, object)
  local iptype = M.GetIPType(value)
  local domainName  = value and string.match(value, '^[a-zA-Z0-9%.%-]+$') and M.validateStringIsDomainName(value)
  local wanStatus = proxy.get("rpc.network.interface.@wan6.up")
  ...
  if object["ProtocolVersion"] == "IPv6" or object["ProtocolVersionTrace"] == "IPv6" then
    -- check for wan6 interface is up/down
    if wanStatus and wanStatus[1].value == "1" then
      if iptype == 4 or (iptype ~= 6 and not domainName) then
        return nil, T"Please enter valid ipv6/domain address"
      end
    else
      return nil, T"Wan6 interface is down"
    end
  end
  return true
end

--function to convert tx/rx rate into GB,MB,KB and B respectively
function M.convertData(val)
  val = tonumber(val)
  if val and val ~= "" then
    if (val > 1000*1000*1000) then
      val = val/(1000*1000*1000)
      return format("%.2f", val) .. " GB"
    elseif (val > 1000*1000) then
      val = val/(1000*1000)
...
--function to convert phyrate into gbps,mbps,kbps and bps respectively
function M.convertPhyRate(val, extender)
  val = tonumber(val)
  if val and val ~= "" then
    if extender then
      if (val > 1000) then
        val = val/(1000)
        return format("%.2f", val) .. " Gbps"
      elseif (val < 1) then
        if ((val*1000) < 1000) and ((val*1000) > 1) then
          val = (val*1000)
          return format("%.2f", val) .. " Kbps"
        else
          val = (val*1000*1000)
          return format("%.2f", val) .. " bps"
        end
      else
        return val .. " Mbps"
      end
      ...
      elseif (val > 1000) then
        val = val/(1000)
        return format("%.2f", val) .. " Mbps"
      elseif (val < 1) then
        return format("%.2f", val*1000) .. " bps"
      else
        return val .. " Kbps"
      end
    end
