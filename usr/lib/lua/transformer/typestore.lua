...
local function collectMappings(self, typepath)
  -- TODO optimization: Make this an iterator, so the mapping is only fetched when really needed.
  -- build the maplist.
  local maplist = {}
...
function TypeStore:synchronize(mapping, parent_keys, parent_ireferences)
  ...
  return self.persistency:sync(mapping.objectType.name, entries, parent_ireferences)
end
...
function TypeStore:getkeys(typepath, irefs)
  ...
  for _,mapping in ipairs(collectMappings(self,typepath)) do
    if self:isMultiInstanceMapping(mapping) then
      ...
      -- For optional single instances we should call an exists() function in the mapping API
      -- to determine whether it's actually present. For mandatory single instances we can
      -- assume it always exists.
      -- TODO: Validate optional single instance exists.
    end
  end
  return keys
end
