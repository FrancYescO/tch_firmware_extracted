...
function M.set_firewall_mode(paramvalue, commitapply)
    ...
    -- to line up IPv4 and IPv6 wan to lan connection behavior, set firewall.@defaults[0].forward
    uci_helper.foreach_on_uci({config="firewall", sectionname="defaults"}, function(s)
        uci_helper.set_on_uci({config="firewall", sectionname=s[".name"], option="forward"}, policy, commitapply)
      return false
    end)
