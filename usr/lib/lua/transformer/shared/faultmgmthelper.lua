...
--- judge if need send the faultmgmt notifications to ACS
function M.NeedNotification()
  local ret = true
  local binding = { config = "faultmgmt", sectionname = "global" , option = "enable_notification"}
  local enable_notif = uci_helper.get_from_uci(binding)
  if enable_notif == "0" then
    ret = false
  end
  return ret
end
