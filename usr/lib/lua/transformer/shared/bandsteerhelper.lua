  ...
  if binding_multiap then
    ...
    uci_helper.foreach_on_uci(binding_multiap, function(s)
      if s.fronthaul == '1' then
        ...
        elseif s.frequency_bands == 'radio_5Gu,radio_5Gl' then
...
function M.isBandSteerEnabledByAp(ap)
  if M.isMultiapEnabled(ap) then
    ...
    if primaryControllerCredSection and primaryControllerCredSection.frequency_bands == 'radio_2G,radio_5Gu,radio_5Gl' then
      return true
    end
  else
    local bandsteerid = M.getApBandSteerId(ap)
    if bandsteerid and strmatch(bandsteerid, "bs%d") then
      return true
    end
  end
  return false
...
local function setControllerCredOption(value, option, iface, commitapply, transactions)
  ...
  if M.isBaseIface(iface) then
    ...
    if primaryControllerCredSection and primaryControllerCredSection.frequency_bands == 'radio_2G' and secondaryControllerCredSection then
...
function M.setSSID(iface, ap, value, commitapply, transactions)
  if M.isMultiapEnabled(nil, iface) then
...
local function setBandSteerPeerApAuthentication(baseap, needsetap)
  ...
  setWirelessUciValue(value, needsetap, "wpa_psk_key", commitapply, transactions)
end
...
function M.setSecurityOption(ap, option, value, commitapply, transactions)
  if value then
    local isMultiapEnabled = M.isMultiapEnabled(ap)
    if isMultiapEnabled then
      local iface = getWirelessUciValue(ap, 'iface')
      return setControllerCredOption(value, option, iface, commitapply, transactions)
    else
      if M.isBandSteerEnabledByAp(ap) then
        return setBandsteerApOption(ap, option, value, commitapply, transactions)
      else
        setWirelessUciValue(value, ap, option, commitapply, transactions)
      end
    end
...
local function disableBandSteer(key, commitapply)
  ...
  if primaryControllerCredSection and secondaryControllerCredSection then
    ...
    setBandSteerID(baseap, needsetap, "off", commitapply, transactions)
    --to reset the ssid
