...
local transactions = {}
envBinding.option = "smartSteering"
local smartSteering = uci_helper.get_from_uci(envBinding)
...
function M.setSSID(iface, ap, value, commitapply, transactions)
  if not M.isMultiapEnabled(nil, iface) then
    ...
    if ap and M.isBandSteerEnabledByAp(ap) or (smartSteering == "1" and isSplitssid == "0") then
      if smartSteering == "1" or  M.isBaseIface(iface) then --if the band steer enabled, set both 2.4G/5G
        local peerIface, errmsg = M.getBandSteerPeerIface(iface)
        if not peerIface then
          return nil, errmsg
        else
...
local function setBandsteerApOption(ap, option, value, commitapply, transactions)
  ...
  if smartSteering == "1" or M.isBaseIface(iface) then
...
function M.setSecurityOption(ap, option, value, commitapply, transactions)
  if value then
    ...
    if not M.isMultiapEnabled(ap) then
      isSplitssid = uci_helper.get_from_uci(webBinding)
      if M.isBandSteerEnabledByAp(ap) or (smartSteering == "1" and isSplitssid == "0") then
