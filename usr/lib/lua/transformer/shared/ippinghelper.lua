...
local pairs, remove, execute = pairs, os.remove, os.execute
...
function M.startup(user, binding)
  ...
  if not f then
    ...
    if user == "device2" then
      uci.set_on_uci(uci_binding[user]["ProtocolVersion"], "Any")
      uci.set_on_uci({config = "ipping", sectionname = "device2" , option = "protocol" }, "Any")
    else
  ...
  local pingBinding = { config = "ipping" }
  uci.foreach_on_uci(pingBinding, function(s)
    local sectionName = s['.name']
    if sectionName:match("network_rpc_") then
      execute("start-stop-daemon -K -p /var/run/ipping_" .. sectionName ..".pid")
      uci.delete_on_uci({config = "ipping", sectionname = sectionName }, commitapply)
      remove("/tmp/ipping_".. sectionName)
      remove("/tmp/ipping_full_".. sectionName)
    end
  end)
...
function M.uci_ipping_get(user, pname)
  ...
  if not uci_binding[user] then
     uci_binding[user] = {
       ...
       Interval = { config = config, sectionname = user, option = "interval" },
