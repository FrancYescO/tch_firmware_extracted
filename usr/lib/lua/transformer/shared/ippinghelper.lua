...
function M.uci_ipping_set(user, pname, pvalue, commitapply)
  if pname == "DiagnosticsState" then
    ...
    elseif pvalue == "Requested" and user == "device2" then
      local intf = M.uci_ipping_get(user, "Interface")
      if intf ~= "" then
        local intfStatus = helper.get_ubus_interface_status(intf)
        M.ipAddress = intfStatus and intfStatus['ipv4-address'] and intfStatus['ipv4-address'][1] and intfStatus['ipv4-address'][1]['address']
      end
    end
