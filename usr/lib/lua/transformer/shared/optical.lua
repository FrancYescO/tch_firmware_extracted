...
local match, find, floor, len, sub, format, lower = string.match, string.find, math.floor, string.len, string.sub, string.format, string.lower
...
function M.getLevel(param)
  ...
  if entry then
    ...
    elseif param == "TransmitOpticalLevel" then
      return value <= -50 and "-50" or value >= 4500 and "4500" or tostring(value)
    end
  end
  return "0"
end
...
function M.getSfpState()
  ...
  elseif output == "" and phyState == "linkup" then
    local wanType = M.getWantype()
    local bType = M.getBoardtype()
    if wanType == "xepon_ae" and bType == "gpon_sfp" then
      status = M.getP2pState()
    end
  end
  return status
end
...
function M.getPonAeStats(param)
  ...
  local bType = M.getBoardtype()
  local PhyState = M.getLinkStatus()
  if wanType == "xepon_ae" and bType == "gpon_sfp" and output == "" and PhyState == "linkup" then
    local intf = getFromUci({config = "network", sectionname = "wan", option = "ifname"})
    return intf and M.getIntfstats(intf, param) or "0"
  elseif output == "" then
    return "0"
  end
...
function M.getPonAeAllStats()
  ...
  for param in pairs(statsEntries) do
    if wanType == "xepon_ae" and bType == "gpon_sfp" and output == "" and PhyState == "linkup" then
      local intf = getFromUci({config = "network", sectionname = "wan", option = "ifname"})
      StatsValues[param] = intf and M.getIntfstats(intf, param) or "0"
    elseif output == "" then
      StatsValues[param] = "0"
    else
...
function M.getPortStatus(port)
  ...
  ctl:close()
  if output then
    if match(lower(output), "link is up") then
      link = "Up"
    else
      link = "Down"
    end
