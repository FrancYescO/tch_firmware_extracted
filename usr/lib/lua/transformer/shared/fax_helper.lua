...
local function getFaxT38Enable(mapping, param, key)
    ...
    foreachOnUci(fxsBinding, function(s)
        ...
        local devFax = getFromUci(fxsBinding)
        local faxTransportType = (devFax == "" or devFax == "t38") or false
        if (not faxTransportType) or (globalFaxTransportValue == "" and not faxTransportType) or (devFax == "" and globalFaxTransportValue ~= "" and globalFaxTransportValue ~= "t38") then
            isT38faxTransport = false
            return
        end
    end)
    return (isT38faxTransport == true) and "1" or "0"
end
