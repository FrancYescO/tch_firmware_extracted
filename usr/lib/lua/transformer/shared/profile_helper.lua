...
function M.authPassword(sipnet_binding, value, commitapply)
    local errMsg
    sipnet_binding.option = "password"
    local oldValue = getFromUci(sipnet_binding)
    local newValue = value
    if M.isEncrypted(oldValue) then
        oldValue = crypto.decrypt(oldValue)
    end
    if M.isEncrypted(newValue) then
        newValue = crypto.decrypt(newValue)
    end
    if oldValue ~= newValue or not oldValue and not newValue then
        if not M.isEncrypted(value) then
            value, errMsg = crypto.encrypt_keysel(value, crypto.AES_256_CBC, crypto.RIP_RANDOM_A)
            if not value then
                return nil, errMsg
            end
        end
        set_on_uci(sipnet_binding, value, commitapply)
    end
    return true
end

function M.registerURI(sipnet_binding, value, commitapply)
    local registeredURI = {}
    local profileBinding = { config = "mmpbxrvsipnet", sectionname = "profile" }
    uci_helper.foreach_on_uci(profileBinding, function(s)
        if s.uri ~= nil then
            registeredURI[s.uri] = true
        end
    end)
    sipnet_binding.option = "uri"
        if getFromUci(sipnet_binding) ~= value then
            if registeredURI[value] then
