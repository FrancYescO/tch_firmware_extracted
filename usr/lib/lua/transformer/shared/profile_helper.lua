...
local function delete_profile_group_entry(profile, transactions, commitapply)
    mmpbx_binding.sectionname = "profile_group"
    local binding = {config = "mmpbx"}
    uci_helper.foreach_on_uci(mmpbx_binding, function(s)
         if type(s.profile) == "table" then
             local newProfiles = {}
             for _, value in pairs(s.profile) do
             ...
             binding.sectionname = s[".name"]
             binding.option = "profile"
             uci_helper.set_on_uci(binding, newProfiles, commitapply)
             transactions[binding.config] = true
         end
    end)
end
...
function M.profile_delete(profile, transactions, commitapply)
    ...
    delete_profile_group_entry(profile, transactions, commitapply)
    if dial_plan_entry_default then
