...
-- @param #string key contains the specified interface in dhcp config
-- @return #table dhcpTable contains all the values to be set on uci for dhcp_option of dhcp config in specified interface
-- @return #table dhcpDnsTable contains only the input DNS Server values to be set on uci for dhcp_option_force of dhcp config in specified interface

function M.parseDNSServersFromDatamodel(value, key)
  local dnsServers = {}
  local dhcpTable = {}
  local dhcpForceTable = {}
  local dhcpValue = "option:dns-server"
  local oldForceDns = false
  dhcpBinding.sectionname = key
  dhcpBinding.option = "dhcp_option"
  local dhcpList = getFromUci(dhcpBinding)
  -- populate list of other dhcp_option items except dns
  for _, val in ipairs(dhcpList) do
    if not ( (val:find("^6,") == 1) or (val:find("^option:dns%-server") == 1) ) then
      dhcpTable[#dhcpTable + 1] = val
    end
  end
  -- populate list of other dhcp_option_force items except dns
  dhcpBinding.option = "dhcp_option_force"
  dhcpList = getFromUci(dhcpBinding)
  for _, val in ipairs(dhcpList) do
    if not ( (val:find("^6,") == 1) or (val:find("^option:dns%-server") == 1) ) then
      dhcpForceTable[#dhcpForceTable + 1] = val
    else
      oldForceDns = true
    end
  end
  ...
  if value and value ~= "" then
    ...
    dhcpForceTable[#dhcpForceTable + 1] = dhcpValue
    return dhcpTable, dhcpForceTable
  else
    -- if value empty, retain other dhcp_option items
    -- dhcp_option_force section to be updated only if dns-server was present earlier, retain other dhcp_option_force items.
    return dhcpTable, oldForceDns and dhcpForceTable or nil
  end
end
