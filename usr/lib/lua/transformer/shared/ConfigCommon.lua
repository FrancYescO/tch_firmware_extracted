...
local function get_cipher_key()
  ...
  if not random_B or #random_B < 32 then throw_error() end
...
local function get_signature_key()
  ...
  if not random_B or #random_B < 64 then throw_error() end
...
local function export_compose_header(export_data)
  ...
  if not check_export_plaintext() then
    header[#header+1] = { key = "CIPHERKEY", value = "GW" }
  end
  if not check_export_unsigned() then
    header[#header+1] = { key = "SIGNATUREKEY", value = "GW" }
  end
