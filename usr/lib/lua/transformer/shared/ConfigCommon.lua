...
local uci_path_header = {
  ...
  SERIALNUMBER = "env.rip.serial",
  MAC = "env.rip.eth_mac",
  BUILDVERSION = "version.@version[0].version",
}
...
local function get_rip_random_B()
  local f_rip_random_B = io.open(rip_path .. rip_random_B, "rb")
  if not f_rip_random_B then return end

  local random_B = f_rip_random_B:read("*all")
  f_rip_random_B:close()
  return random_B
end

local function get_cipher_key()
  local random_B = get_rip_random_B()

  if not random_B or random_B:len() < 32 then throw_error() end
  return random_B:sub(1,32)
end

local function get_signature_key()
  local random_B = get_rip_random_B()

  if not random_B or random_B:len() < 64 then throw_error() end
  return random_B:sub(1,64)
end
...
local function export_compose_header(export_data)
  ...
  local function add_header_uci(key)
    ...
    table.insert(header, { key = key, value = value })
  end

  table.insert(header, { key = "PREAMBLE", value = "THENC" })
  table.insert(header, { key = "BACKUPVERSION", value = export_version })
  ...
  if not check_export_plaintext() then
    table.insert(header, { key = "CIPHERKEY", value = "GW" })
  end
  if not check_export_unsigned() then
    table.insert(header, { key = "SIGNATUREKEY", value = "GW" })
  end
  ...
  for _,entry in ipairs(export_data.header) do
    table.insert(header, entry.key .. "=" .. entry.value)
  end
...
local function export_config_start_package(data, package)
  export_config_add(data, string.format("[%s]", package))
end
...
local function export_config_start_section(data, package, section)
  export_config_add(data, string.format("[%s.%s]", package, section))
end
...
local function export_config_set_section(data, package, section, sectiontype)
  export_config_add(data, string.format("%s.%s=%s", package, section, sectiontype))
end

local function export_config_set_param(data, package, section, key, value)
  if type(value) == "string" then
    if string.match(value, "'") then throw_error() end
    export_config_add(data, string.format("%s.%s.%s='%s'", package, section, key, value))
    return
  end
  if type(value) == "table" then
    for i,elem in ipairs(value) do
      if string.match(elem, "'") then throw_error() end
      value[i] = "'" .. elem .. "'"
    end
    export_config_add(data, string.format("%s.%s.%s={%s}", package, section, key, table.concat(value, ',')))
    return
  end
end
...
local function export_section(data, package, section, sn)
  ...
  if section:sub(1,1) == "@" then
    ...
    uci_cursor:foreach(package, sectiontype, function(uci_section)
      local instancename = string.format("%s[%d]", section, uci_section['.index'])
      export_config_set_section(data, package, instancename, sectiontype)

      for key,value in pairs(uci_section) do
        if not string.match(key, "^[._]") and ((not sn.pm[key] and sn.value) or (sn.pm[key] and sn.pm[key].value)) then
  ...
  if pkg.value then --export undefined sections fully
    ...
    uci_cursor:foreach(package, function(uci_section)
      ...
      if uci_section['.anonymous'] == false then
        ...
        sectionname = string.format('@%s[%d]', sectiontype, types_count[sectiontype])
      end
...
local function export_encrypt_data(export_data)
  ...
  export_data.ciphertext = crypto.encrypt(cipher_scheme, get_cipher_key(), export_data.iv, export_data.plaintext)
...
local function export_add_signature(export_data)
  ...
  export_data.signature = crypto.sign(signature_scheme, get_signature_key(), table.concat(data))
...
local function export_execute(export_mapdata)
  ...
  if not rv then
    export_set_error(export_mapdata, string.format("compose header failed (%s)", get_error_info(err) or "?"))
    return
  end
  ...
  if not rv then
    export_set_error(export_mapdata, string.format("read export list failed (%s)", get_error_info(err) or "?"))
  ...
  if not rv then
    export_set_error(export_mapdata, string.format("collect export data failed (%s)", get_error_info(err) or "?"))
    return
  end
  ...
  if not check_export_plaintext() then
    ...
    if not rv then
      export_set_error(export_mapdata, string.format("encrypt export data failed (%s)", get_error_info(err) or "?"))
      return
    end
  end
  ...
  if not check_export_unsigned() then
    ...
    if not rv then
      export_set_error(export_mapdata, string.format("sign export data failed (%s)", get_error_info(err) or "?"))
      return
    end
  end
  ...
  if not rv then
    export_set_error(export_mapdata, string.format("write export data failed (%s)", get_error_info(err) or "?"))
    return
  end
...
local function import_check_signature(import_data)
  if import_data.header["SIGNATUREKEY"] ~= "GW"
     or import_data.rawcontent:len() < 20 then
    throw_error()
  end
  ...
  if true ~= crypto.validate(signature_scheme, get_signature_key(), import_data.rawheader .. import_data.rawcontent,
...
local function import_decrypt_data(import_data)
  if import_data.header["CIPHERKEY"] ~= "GW"
     or import_data.rawcontent:len() < 16 then
    throw_error()
  end
  ...
  local plaintext = crypto.decrypt(cipher_scheme, get_cipher_key(), iv, import_data.rawcontent)
...
local function import_apply_configdata(data)
  ...
  for name,package in pairs(packages) do
    if package.commit then
      if not uci_cursor:commit(name) then print("commit: " .. name) throw_error() end
    end
  end
end
...
local function import_execute(import_mapdata)
  ...
  if not rv then
    import_set_error(import_mapdata, string.format("read import list failed (%s)", get_error_info(err) or "?"))
  ...
  if not rv then
    import_set_error(import_mapdata, string.format("read import data failed (%s)", get_error_info(err) or "?"))
    return
  end
  ...
  if import_data.header["SIGNATUREKEY"] then
    ...
    if not rv then
      import_set_error(import_mapdata, string.format("signature check failed (%s)", get_error_info(err) or "?"))
      return
    end
  ...
  if import_data.header["CIPHERKEY"] then
    ...
    if not rv then
      import_set_error(import_mapdata, string.format("decrypt import data failed (%s)", get_error_info(err) or "?"))
      return
    end
  ...
  if not rv then
    import_set_error(import_mapdata, string.format("parse import content failed (%s)", get_error_info(err) or "?"))
    return
  end
  ...
  if not rv then
    import_set_error(import_mapdata, string.format("filter config data failed (%s)", get_error_info(err) or "?"))
    return
  end
  -- cleanup
  import_config_list = nil
  ...
  if not rv then
    import_set_error(import_mapdata, string.format("apply config data failed (%s)", get_error_info(err) or "?"))
    return
  end
