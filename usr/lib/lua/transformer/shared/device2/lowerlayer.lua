...
set_handler["Device.IP.Interface.{i}."] = function(model, upper, lowerkey, typepath)
  local ucikey = model:getUciKey(lowerkey)
  local upperuci = model:getUciKey(upper)
  if ucikey:match("wan") or ucikey:match("hotspot") then
    uci_set(upperuci, "tunlink", ucikey)
    return true
  else
