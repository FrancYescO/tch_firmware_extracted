...
local function real_getKeys(self, devname)
	...
	foreach_interface(function(s)
		if match_proto(self, s.proto) and not activedevice.isActiveInterface(s['.name']) and isWanInterface(s['.name'], wanInterfaces) then
                        cursor:load("network")
                        local device = cursor:get("network", s['.name'], "ifname")
			local ref_intf = s.ifname and s.ifname:match("^@(.*)") or s.device and s.device:match("^@(.*)") or device and device:match("^@(.*)")
                        cursor:unload("network")
                        ref_intf = not ref_intf and s.proto == "6rd" and "wan" or ref_intf
