...
function M.tr143_get(config, user, pname)
  ...
  if pname == "X_0876FF_Throughput" then
    ...
    if config == "UploadDiagnostics" then
      if diagOptions.PeriodOfFullLoading and diagOptions.PeriodOfFullLoading ~= "0" and diagOptions.TotalBytesSentUnderFullLoading then
        return tostring(8 * (tonumber(diagOptions.TotalBytesSentUnderFullLoading)/tonumber(diagOptions.PeriodOfFullLoading)))
      end
    end
    return "0"
  end
