...
--- Helper function to execute the actual password update; fork and execv update_passw.sh to do the actual work.
-- Using execv protects against potential commands injections through user-provided password.
...
local function execUpdate(user, password, password_is_crypted)
  ...
  if not pid then
    return nil, "Could not execute password update"
  elseif pid == 0 then -- pid 0 is the child process
    -- If password is provided in encrypted form, pass `-e` to script
    local crypted = (password_is_crypted and "-e") or ""
    -- execv never returns, unless errors
    local _, err = posix.execv("/usr/share/transformer/scripts/update_passw.sh", { user, password, crypted })
    local logger = require("transformer.logger")
    logger:critical("setClashPassword: execUpdate() failed: %s", err)
