...
function M.uci_traceroute_set(user, pname, pvalue, commitapply)
  if pname == "DiagnosticsState" then
    ...
    elseif pvalue == "Requested" and user == "device2" then
      ...
      if interface ~= "" then
        local intfStatus = helper.get_ubus_interface_status(interface)
        if intfStatus and intfStatus['ipv4-address'] and intfStatus['ipv4-address'][1] then
          M.ipAddress = intfStatus['ipv4-address'][1]['address']
        elseif intfStatus and intfStatus['ipv6-address'] and intfStatus['ipv6-address'][1] then
          M.ipAddress = intfStatus['ipv6-address'][1]['address']
        end
      end
    end
