...
function M.startup(user, binding)
  ...
  if not f then
    ...
    if architecture == "aarch64" then
     uci.set_on_uci(uci_binding[user]["DataBlockSize"], 46)
    else
...
function M.uci_traceroute_get(user, pname)
  ...
  if uci_binding[user] == nil then
     uci_binding[user]= {
          ...
          IPAddress = { config = config, sectionname = user, option = "ipaddress" },
        }
  end
...
function M.uci_traceroute_set(user, pname, pvalue, commitapply)
  if pname == "DiagnosticsState" then
    ...
    if pvalue == "Canceled" then
      return transaction_set(uci_binding[user]["DiagnosticsState"], "None", commitapply)
    else
      return transaction_set(uci_binding[user]["DiagnosticsState"], pvalue, commitapply)
    end
  elseif (pname == "ResponseTime") then
    return nil, "invalid parameter"
  elseif pname == "DataBlockSize" then
    local proto = uci.get_from_uci(uci_binding[user]["ProtocolVersion"])
    if architecture == "aarch64" then
      if proto == "IPv4" and tonumber(pvalue) < 46 then
        return nil, "DataBlockSize for IPv4 should be more than 46"
      elseif proto == "IPv6" and tonumber(pvalue) < 72 then
        return nil, "DataBlockSize for IPv6 should be more than 72"
      end
    else
      if proto == "IPv4" and tonumber(pvalue) < 38 then
        return nil, "DataBlockSize for IPv4 should be more than 38"
      elseif proto == "IPv6" and tonumber(pvalue) < 64 then
        return nil, "DataBlockSize for IPv6 should be more than 64"
      end
    end
  elseif pname == "ProtocolVersion" then
    local size = uci.get_from_uci(uci_binding[user]["DataBlockSize"])
    if architecture == "aarch64" then
      if pvalue == "IPv6" and tonumber(size)<72 then
        transaction_set(uci_binding[user]["DataBlockSize"], "72", commitapply)
      end
    else
      if pvalue == "IPv6" and tonumber(size)<64 then
        transaction_set(uci_binding[user]["DataBlockSize"], "64", commitapply)
      end
    end
  end
