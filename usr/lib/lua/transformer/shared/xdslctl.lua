...
local function createTableWithEmptyValues()
  ...
  addEmptyAdslMib( "QLNGds", 0, "QLNGus", 0)
 -- g.fast parameters
...
local function getInfoValuesFromAdslMib( lineid )
  ...
  local channel = valueFromCmd(xdslctlinfo[lineid], "channel")
  if not channel then channel = 0 end
  allvalues.currentrate.channel = channel
  return allvalues
end
...
local function getInfoValueFromAdslMib( lineid, key, subkey )
  local getValue = {}
  getAdslMibInfo(lineid)
  if key == "status" or key == "linit" or key == "channel" then
