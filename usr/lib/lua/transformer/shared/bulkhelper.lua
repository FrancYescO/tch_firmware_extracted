...
local function getUciValue(binding, option)
...
function ProfileMap:getProfileKeys(parentkey)
  ...
  foreach_profile(function(s)
    if s['.name']:match(parentkey) then
...
local function getIndexID(parameter_instances, indexid)
  for i, v in pairs(parameter_instances) do
    if tonumber(v) == indexid then
      indexid = i
      return indexid
    end
  end
  ...
  for p in pairs(parameters) do
    if (map[p]) then
      if type(map[p]) == "function" then
        data[p] = map[p](object, indexid)
      elseif type(map[p]) == "table" then
        -- for parameters or http_uri
        if not data.Name or not data.Reference then
          indexid = getIndexID(object["parameter_instances"], indexid)
          local ref, name = match((object[map[p][1]] and object[map[p][1]][indexid]) or "", "(.*)|(.*)")
          if p == "Enable" then
            data.Enable = name ~= "" and name or "1"
          elseif p == "SampleMode" or p == "CalculationMode" then
            data.CalculationMode = ref ~= "" and ref or "Latest"
            data.SampleMode =  name ~= "" and name or "Current"
          else
            data.Name = name or ""
...
--- get all http_uri's values for the given mapping
-- @param mapping [table] a mapping
-- @param key [string] the key of the current instance
-- @returns a list (table) of parameter(http_uri) values
function M.http_uri_getall(mapping, key)
  local data = {}
  local object, indexid = getObject(key)
  local map = mapping._profile.map.get
  local parameters = mapping.objectType.parameters
  for p in pairs(parameters) do
    if (map[p]) then
      ...
      elseif type(map[p]) == "table" then
        -- for parameters or http_uri
        if not data.Name or not data.Reference then
          indexid = getIndexID(object["http_uri_instances"], indexid)
          local ref, name = match((object[map[p][1]] and object[map[p][1]][indexid]) or "", "(.*)|(.*)")
          if p == "Enable" then
            data.Enable = name ~= "" and name or "1"
          elseif p == "SampleMode" or p == "CalculationMode" then
...
--- get one http_uri's value for the given mapping and given parameter name
-- @param mapping [table] a mapping
-- @param param [string] the parameter name to be gotten
-- @param key [string] the key of the current instance
-- @returns a value (string) of parameter
function M.http_uri_get(mapping, param, key)
  local object, indexid = getObject(key)
  local map = mapping._profile.map.get
  local parameters = mapping.objectType.parameters
  if (map[param]) then
...
function M.set(mapping, param, value, key, instance)
  local object, indexid = getObject(key)
  ...
  if map[param] then
    ...
    elseif type(map[param]) == "table" then
      ...
      for i, v in pairs(instance) do
        if (tonumber(v) == tonumber(indexid)) then
          indexid = i
        end
      end
...
local function get_profile_name(parentkey, max)
  local all = {}
  local count = 0
  foreach_profile(function(s)
    local id = tonumber(s['.name']:match(parentkey.."_(%d+)$"))
    if id then
      count = count + 1
      all[id] = id
    end
  ...
  return count < max and format("%s_%d", parentkey, #all+1) or nil
end
...
function M.add_profile(mapping, parentkey)
  ...
  local name = get_profile_name(parentkey, tonumber(max))
  if name == nil then
  ...
  for option, value in pairs(profile_values) do
    setUciValue(binding, option, value)
  end
...
function M.add_param_elements(mapping, parentkey, option)
  local commitapply = mapping._profile.commitapply
  local transactions = mapping._profile.transactions
  local binding = {config = "bulkdata"}
