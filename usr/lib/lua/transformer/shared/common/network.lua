...
-- set value to the parameter in DHCP Pool config
local function setDHCPConfig(poolname, param, value, commitapply)
  dhcpBinding.sectionname = poolname
  dhcpBinding.option = param
  uciHelper.set_on_uci(dhcpBinding, value, commitapply)
end

-- Validates whether Min/Max address are valid and not overlapping with other pool
-- return true if the range is valid and not overlapping, (nil, err) on overlapping, nil if invalid
local function validateDHCPAddr(poolname)
  local data = dhcp.parseDHCPData(poolname)
  if data and data.ipEnd >= data.ipStart and data.ipStart >= data.ipMin and data.ipEnd <= data.ipMax then
     return overlapCheck(data.network, poolname, data.ipStart - data.network, data.ipEnd - data.ipStart + 1, data.ipMax - data.network)
  end
end

-- Disable the specified DHCP pool
local function disable_DHCPPool(poolname, commitapply)
  setDHCPConfig(poolname, "dhcpv4", "disabled", commitapply)
end
...
--- Set the minaddress for the given DHCP Pool.
-- @string poolname The DHCP Pool for which the start address to be modified.
-- @string address The IP Address to be set as start address.
-- @treturn boolean True if the given address is successfully set for the DHCP pool else nil + error message.
-- @string commitapply is to apply all changes it will execute all queued actions asynchronously in the background.

function M.setDHCPMinAddress(poolname, address, commitapply)
  ...
  local start_value = newStart - data.network
  local limit_value = data.ipEnd - newStart + 1
  local result, err = overlapCheck(data.network, poolname, start_value, limit_value, data.ipMax)
  if result then
    setDHCPConfig(poolname, "start", start_value, commitapply)
    setDHCPConfig(poolname, "limit", limit_value, commitapply)
    return true
  end
  return result, err
end

--- Set the maxaddress for the given DHCP Pool.
-- @string poolname The DHCP Pool for which the end address to be modified.
-- @string address The IP Address to be set as end address.
-- @treturn boolean True if the given address is successfully set for the DHCP pool else nil + error message.
-- @string commitapply is to apply all changes it will execute all queued actions asynchronously in the background.

function M.setDHCPMaxAddress(poolname, address, commitapply)
  ...
  local limit_value = newEnd - data.ipStart + 1
  local start_value = data.ipStart - data.network
  local result , err = overlapCheck(data.network, poolname, start_value, limit_value, data.ipMax)
  if result then
    setDHCPConfig(poolname, "limit", limit_value, commitapply)
    return true
...
--- Set the Min/Max address for the given DHCP Pool and defer validation to commit.
-- @string poolname The DHCP pool for which the end address to be modified.
-- @string address The IP Address to be set as end address.
-- @string parameter Specifies the parameter MinAddress or MaxAddress
-- @string commitapply is to apply all changes it will execute all queued actions asynchronously in the background.
-- @treturn boolean True if the given address is successfully set for the DHCP pool else nil + error message.

function M.setDHCPAddr_defervalidation(poolname, address, parameter, commitapply)
  local newValue = ipv4ToNum(address)
  if not newValue then
    return nil, "Invalid IP Address"
  end
  local data = dhcp.parseDHCPData(poolname)
  if parameter == "MinAddress" then
    setDHCPConfig(poolname, "start", newValue - data.network, commitapply)
    setDHCPConfig(poolname, "limit", data.ipEnd - newValue + 1, commitapply)
  else
    setDHCPConfig(poolname, "limit", newValue - data.ipStart + 1, commitapply)
  end
  return true
end

--- Disable DHCP Pool if the Min/Max address provisioned are invalid
-- @string poolname The DHCP pool for which the min/max address range to be validated
-- @treturn string return back error string if the min/max address range is invalid or overlapped
function M.disableDHCPPool_ifInvalid(poolname)
  local dhcp_valid, err = validateDHCPAddr(poolname)
  if not dhcp_valid then
     disable_DHCPPool(poolname)
     err = err or "Min Max address provisioned is not valid".." for DHCP Pool "..poolname
     return err
  end
end
