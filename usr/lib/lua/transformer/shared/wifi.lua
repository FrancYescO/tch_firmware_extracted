...
local function checkDifference(baseIntf, peerIntf, baseAp, peerAp)
  local base_ssid = getFromUci({ config = "wireless", sectionname = baseIntf, option = "ssid"})
  local base_key = getFromUci({ config = "wireless", sectionname = baseAp, option = "wpa_psk_key"})
  local base_securitymode = getFromUci({ config = "wireless", sectionname = baseAp, option = "security_mode"})
  local base_state = getFromUci({ config = "wireless", sectionname = baseAp, option = "state"})
  local base_acl_mode = getFromUci({ config = "wireless", sectionname = baseAp, option = "acl_mode"})
  local peerAp_ssid = getFromUci({ config = "wireless", sectionname = peerIntf, option = "ssid"})
  local peerAp_key = getFromUci({ config = "wireless", sectionname = peerAp, option = "wpa_psk_key"})
  local peerAp_securitymode = getFromUci({ config = "wireless", sectionname = peerAp, option = "security_mode"})
  local peerAp_state = getFromUci({ config = "wireless", sectionname = peerAp, option = "state"})
  local peerAp_acl_mode = getFromUci({ config = "wireless", sectionname = peerAp, option = "acl_mode"})
  ...
  if base_state == peerAp_state and base_ssid == peerAp_ssid and base_securitymode == peerAp_securitymode and base_key == peerAp_key then
   if base_acl_mode ~= peerAp_acl_mode or acl_list and ( #base_acl_table ~= #peerAp_acl_table or mac_difference(base_acl_table, peerAp_acl_table)) then
     return false
   end
   return true
  end
...
function M.splitSSID(param, aps)
  ...
  if network and network == "lan" and main_splitssid then
    ...
    multiap_ap0 = "cred0"
    multiap_ap1 = "cred1"
  elseif network and string.match(network,"wlnet_b") and guest_splitssid then
  ...
  if bandSteerHelper.isMultiapEnabled(aps) and multiap_ap0 and multiap_ap1 and getFromUci(multiapBinding) == "0" then
