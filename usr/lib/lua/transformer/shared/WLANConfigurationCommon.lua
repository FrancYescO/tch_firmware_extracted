...
local function getApBandSteerId(ap)
    return get_from_uci(ap, "bandsteer_id")
end
...
local function getSecurityMode(ap)
    return get_from_uci(ap, "security_mode")
end
...
local function enableBandSteer(key, commitapply)
    ...
    if "wep" == getSecurityMode(ap) then
    ...
    setBandSteerID(ap, bspeerap, bsid, "1", commitapply)
...
local function disableBandSteer(key, commitapply)
    ...
    if not bandsteerid or "" == bandsteerid or "off" == bandsteerid then
        return nil, "Band steering has already been disabled"
    end
    ...
    setBandSteerID(ap, bspeerap, "off", "0", commitapply)
...
local function isBandSteerEnabled(iface)
...
M.getMappings = function(commitapply)
    ...
    local setWLANDevice = {
        ...
        SSID = function(mapping, param, value, key)
            ...
            if not isBaseIface(binding_wireless.sectionname) and isBandSteerEnabled(binding_wireless.sectionname) then
        ...
        -- WEPEncryptionLevel 40-bit,104-bit
            ...
            if not isBaseIface(key) and isBandSteerEnabled(key) then
                ...
                toModifyBSPeerNodeAuthentication("wpa_psk_key", value, string.match(key, "(.*)_psk_"), commitapply)
        ...
        AuthenticationServiceMode = function(mapping, param, value, key)
            if not isBaseIface(key) and isBandSteerEnabled(key) then
                ...
                set_on_uci(ap, "security_mode", mode, commitapply)
        ...
        X_000E50_ChannelMode = function(mapping, param, value, key)
            ...
            set_on_uci(radio, "channel", p, commitapply)
        end,
