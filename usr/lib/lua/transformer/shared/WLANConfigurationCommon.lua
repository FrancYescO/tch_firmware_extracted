...
function M.setBandSteerID(ap, relatedAP, bsid, enable, commitapply)
  ...
  -- set the "bandsteer_id" option in both the AP and related AP and set the state of the bandsteer configuration
  M.setOnWireless(ap, "bandsteer_id", bsid, commitapply)
  M.setOnWireless(relatedAP, "bandsteer_id", bsid, commitapply)
  M.setOnWireless(bsid, "state", enable, commitapply)
end
...
--to set pmf value
local function setPmfValue(baseAP, relatedAP)
  local value = M.getFromWireless(baseAP, "security_mode")
  if value == "wpa3-psk" then
    M.setOnWireless(relatedAP, "pmf", "required", commitapply)
  elseif value == "wpa2-wpa3-psk" then
    M.setOnWireless(relatedAP, "pmf", "enabled", commitapply)
  else
    M.setOnWireless(relatedAP, "pmf", "disabled", commitapply)
  end
end

--to set the authentication related content
local function setBandSteerPeerApAuthentication(baseAP, relatedAP)
  local value = M.getFromWireless(baseAP, "security_mode")
  M.setOnWireless(relatedAP, "security_mode", value, commitapply)
  local wpaKey = M.getFromWireless(baseAP, "wpa_psk_key")
  M.setOnWireless(relatedAP, "wpa_psk_key", wpaKey, commitapply)
  local passPhrase = M.getFromWireless(baseAP.."_credential0", "passphrase")
  local state = M.getFromWireless(baseAP.."_credential0", "state")
  M.setOnWireless(relatedAP.."_credential0", "state", state, commitapply)
  if value == "wpa3-psk" or value == "wpa2-wpa3-psk" then
     M.setOnWireless(relatedAP.."_credential0", "passphrase", passPhrase, commitapply)
  end
end
...
function M.enableBandSteer(key, commitapply)
  ...
  setBandSteerPeerApAuthentication(baseAP,relatedAP)
  setPmfValue(baseAP,relatedAP)
end
...
function M.disableBandSteer(key, commitapply)
  ...
  M.setBandSteerID(baseAP, relatedAP, bsid, nil, commitapply)
...
function M.modifyBSPeerNodeAuthentication(option, value, iface, commitapply)
  ...
  local bandSteerstate = bandSteerid and bandSteerid:match("bs%d") and M.getFromWireless(bandSteerid, "state", "1") or "0"
...
--Function Description: Hostmanager has device's ethernet address(which is fixed) and wireless.accesspoint.station / multiap.controller.station
...
--Input  : macaddr <MAC address of connected device in hostmanager.device>,
--         wifimac <MAC address in wireless.accesspoint.station / multiap.controller.station>
--Output : return true or false

function M.isExtMac(macaddr,wifimac)
...
function M.getStationDataFromIface(iface, macAddress, option, default)
  local ssid, ap
  if iface:match("wds") then
    local host_data = conn:call("hostmanager.device", "get", { ["mac-address"] = macAddress }) or {}
    for _, wifi_data in pairs(host_data) do
        ssid = wifi_data["wireless"]["SSID"]
        ap = wifi_data["wireless"]["accesspoint"]
    end
  else
    ssid = M.getFromWireless(iface, "ssid")
    ap = M.getAPFromIface(iface)
  end
  ...
  if stationInfo[ap] and option then
    for mac, data in pairs(stationInfo[ap]) do
      if (mac == macAddress or M.isExtMac(macAddress, mac)) and data.state:match("Associated") and data.last_ssid == ssid then
