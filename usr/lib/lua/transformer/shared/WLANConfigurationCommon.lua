...
M.getMappings = function(commitapply)
  ...
  local setWLANDevice = {
    ...
    KeyPassphrase = function(mapping, param, value, key)
      local len = value:len()
      if (len ~= 10 and len ~=26) or value:match("[^%x]") then
    ...
    WPAEncryptionModes = function(mapping, param, value, key)
      ...
      local authMode = wpaAuthenticationModeMap[secMode]
      local val
      if authMode == "PSKAuthentication" then
        val = "wpa-wpa2-psk"
      elseif authMode == "EAPAuthentication" then
        val = "wpa-wpa2"
      end
      if val then
  ...
  local getStaDataFromIface = function(iface, macAddress, option)
    ...
    if stationInfo[ap] and option then
      for mac, data in pairs(stationInfo[ap]) do
         if mac == macAddress and data.state:match("Associated") and data.last_ssid == ssid then
           return tostring(data[option]) or ""
         end
      end
    end
  ...
  local getAssociatedDevice = {
    ...
    AssociatedDeviceAuthenticationState = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      local state = getStaDataFromIface(parentKey, macAddress, "state")
      return state:match("Authenticated") and "1" or "0"
    end,
    LastRequestedUnicastCipher = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "encryption") or ""
    end,
    LastRequestedMulticastCipher = "",
    LastPMKId = "",
    LastDataTransmitRate = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      local txRate = getStaDataFromIface(parentKey, macAddress, "tx_data_rate_history")
      return txRate:match("%d+") or ""
    end,
    X_Status = function(mapping, param, key)
    ...
    X_LastDataUplinkRate = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "tx_phy_rate") or "0"
    end,
    X_LastDataDownlinkRate = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "rx_phy_rate") or "0"
    end,
    X_000E50_AssociatedDeviceRSSI  = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "rssi") or "0"
    end,
    X_000E50_LastDisconnectBy = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "last_disconnect_by") or "0"
    end,
    X_000E50_LastDisconnectReason = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "last_disconnect_reason") or "0"
    end,
    X_000E50_TxNoAckFailures = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "tx_noack_failures") or "0"
    end,
    X_000E50_TxPhyRate = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "tx_phy_rate") or "0"
    end,
    X_000E50_RxPhyRate = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "rx_phy_rate") or "0"
    end,
    X_000E50_RSSIHistory = function(mapping, param, key, parentKey)
      local macAddress = key:match("_sta_([%da-fA-F:]+)$") or ""
      return getStaDataFromIface(parentKey, macAddress, "rssi_history") or "0"
    end
  }
