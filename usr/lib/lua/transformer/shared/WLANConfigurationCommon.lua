...
local function getDataFromWDS(key, option, default)
...
M.getWLANMappings = function(commitapply)
  local getWLANDevice = {
    ...
    X_000E50_SetDefAcAgg = function(mapping, param, key)
      local radio = getRadioFromIface(key)
      local radioType = getFromUci(radio, "type")
      return radioType == "quantenna" and getFromUci(radio, "ac_agg_vi") ~= "" and "0" or "1"
    end,
   ...
   X_000E50_MuMimo = function(mapping, param, key)
    local radio = getRadioFromIface(key)
    return getFromUci(radio, "mumimo") or ""
  end,
  }
  ...
  local function getallWLANDevice(mapping, key)
    ...
    return {
	  ...
	  X_000E50_MuMimo = getFromUci(radio, "mumimo") or ""
   }
   end
...
M.associatedDeviceMapping = function(commitapply)
  ...
  local function getStationInfo(_, param, key, parentKey)
    ...
    elseif key:match("_wds") then
      return getDataFromWDS(key, paramMap[param], defaultValueMap[param])
    end
  end
  ...
  local getAssociatedDevice = {
    ...
    AssociatedDeviceIPAddress = function(mapping, param, key)
      ...
      if key:match("_wds") then
        macAddress = modifyMAC(macAddress)
      end
    ...
    X_OperatingStandard = function(mapping, param, key, parentKey)
      ...
      elseif key:match("_wds") then
        operatingStandard = getDataFromWDS(key, "capabilities", "")
      end
...
local function stationStats(key, parentKey, option)
  ...
  elseif key:match("_wds") then
    return getDataFromWDS(key, option, "0")
  end
end
