...
local function getKeyPassPhrase(key, optionsec)
    ...
    if show_password == "1" then
        ...
        if type(ap) == 'string' then
            ...
            binding_wireless.option = optionsec
...
local function getAuthenticationServiceModeFromApSecurity(key)
    ...
    if iface then
        ...
        mode = uci_helper.get_from_uci(binding_wireless)
    end
...
local function getBandSteerPeerIface(key)
    ...
    return nil, "There is no band steer peer SSID exist."
end

local function isBaseIface(iface)
    return "0" == string.match(iface, "%d+")
end
...
local function setBandSteerPeerApAuthentication(baseap, needsetap)
    ...
    binding_wireless.sectionname = baseap
    binding_wireless.option = "wep_key"
    value = uci_helper.get_from_uci(binding_wireless)
    binding_wireless.sectionname = needsetap
    uci_helper.set_on_uci(binding_wireless, value, commitapply)

    binding_wireless.sectionname = baseap
    binding_wireless.option = "wpa_psk_passphrase"
    value = uci_helper.get_from_uci(binding_wireless)
    binding_wireless.sectionname = needsetap
    uci_helper.set_on_uci(binding_wireless, value, commitapply)
...
local function enableBandSteer(key)
    ...
    if "" ~= bandsteerid and "off" ~= bandsteerid then
        return nil, "Band steering has already beed enabled."
    end

    local adminstate = tostring(getDataFromAP(ap).admin_state)
    if "1" ~= adminstate then
        return nil, "Please enable network firstly."
    end

    local bspeeriface = getBandSteerPeerIface(key)
    ...
    adminstate = tostring(getDataFromAP(bspeerap).admin_state)
    if "1" ~= adminstate then
        return nil, "Please enable network for band steering switching node firstly"
    end

    local bsid, errmsg = getBandSteerId(key)
    if not bsid then
        return nil, errmsg
    end

    setBandSteerID(ap, bspeerap, bsid, "1")

    --No need to check the return value now
    local baseap, needsetap, baseiface, needsetiface = getBandSteerRelatedNode(ap, bspeerap, key, bspeeriface)
...
local function disableBandSteer(key)
    ...
    local bandsteerid = getApBandSteerId(ap)
    if ("" == bandsteerid or "off" == bandsteerid) then
        return nil, "Band steering has already beed disabled"
    end
    ...
    setBandSteerID(ap, bspeerap, "off", "0")

    --to reset the ssid, no need to check the return value now
    local baseiface = isBaseIface(key) and key or bspeeriface
    local needsetiface = isBaseIface(key) and bspeeriface or key
...
local function isOptionCanModifiedChecking(iface)
...
local function toModifyBSPeerNodeAuthentication(option, value, iface)
    local ap = getAPForIface(iface)
    if type(ap) ~= 'string' then
        return
    end
    ...
    if "" == bandsteerid or "off" == bandsteerid then
        ...
        if isBaseIface(iface) then
            if "ssid" == option then
                binding_wireless.sectionname = bspeeriface
            else
                local bspeerap = getAPForIface(bspeeriface)
                if type(bspeerap) == "string" then
                    binding_wireless.sectionname = bspeerap
...
M.getMappings = function(commitapply)
    ...
    local getWLANDevice = {
        ...
        AutoChannelEnable  = function(mapping, param, key)
            local autoCHEnable
            local radio = getRadioForIface(key)
            binding_wireless.sectionname = radio
            binding_wireless.option = "channel"
            local channel = uci_helper.get_from_uci(binding_wireless)
        ...
        X_000E50_Power = function(mapping, param , key)
            local radio = getRadioForIface(key)
            binding_wireless.sectionname = radio
            binding_wireless.option = "tx_power_adjust"
            local p = uci_helper.get_from_uci(binding_wireless)
            if p=='' then
                return '4'
            else
    ...
    local setWLANDevice = {
        Enable = function(mapping, param, value, key)
            local ap = getAPForIface(key)
            binding_wireless.sectionname = ap
            binding_wireless.option = "state"
            local state = uci_helper.set_on_uci(binding_wireless, value, commitapply)
        end,
        ...
        SSID = function(mapping, param, value, key)
            local iface = string.match(key, "(.*)_remote")
            if iface then
                binding_wireless.sectionname = iface
            else
                binding_wireless.sectionname = key
            end

            if not isOptionCanModifiedChecking(binding_wireless.sectionname) then
                return nil, "Can not modify the value when band steer enabled"
                  ...
                  toModifyBSPeerNodeAuthentication("ssid", value, key)
            end
        end,
        BeaconType = function(mapping, param, value, key)
            if not isOptionCanModifiedChecking(key) then
                return nil, "Can not modify the value when band steer enabled"
            else
                local ap = getAPForIface(key)
                local apsecurity = getDataFromAPSecurity(ap)
                ...
                binding_wireless.sectionname = ap
                binding_wireless.option = "security_mode"
                uci_helper.set_on_uci(binding_wireless, bt, commitapply)

                --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                toModifyBSPeerNodeAuthentication("security_mode", bt, key)
            end
        end,
        ...
        --  and update the local key index
            ...
            if WEPKeyIndex ~= indexnumber then
                if not isOptionCanModifiedChecking(key) then
                    ...
                    binding_wireless.sectionname = ap
                    binding_wireless.option = "wep_key"
                    uci_helper.set_on_uci(binding_wireless, wepkeys[indexnumber], commitapply)

                    --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                    toModifyBSPeerNodeAuthentication("wep_key", wepkeys[indexnumber], key)
                end
            end
        end,
        ...
        -- WEPEncryptionLevel 40-bit,104-bit
            ...
            if not isOptionCanModifiedChecking(key) then
                ...
                --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                toModifyBSPeerNodeAuthentication("wep_key", value, key)

                binding_wireless.option = "wpa_psk_key"
                uci_helper.set_on_uci(binding_wireless, value, commitapply)

                --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                toModifyBSPeerNodeAuthentication("wpa_psk_key", value, key)
        ...
        BasicEncryptionModes = function(mapping, param, value, key)
            if not isOptionCanModifiedChecking(key) then
                ...
                uci_helper.set_on_uci(binding_wireless, modevalue, commitapply)

                --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                toModifyBSPeerNodeAuthentication("security_mode", modevalue, key)
            end
        end,
        ...
        WPAEncryptionModes = function(mapping, param, value, key)
            if not isOptionCanModifiedChecking(key) then
                ...
                if not optionvalue then
                    uci_helper.set_on_uci(binding_wireless, optionvalue, commitapply)

                    --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                    toModifyBSPeerNodeAuthentication("security_mode", optionvalue, key)
                end
            end
        end,
        ...
        SSIDAdvertisementEnabled = function(mapping, param, value, key)
            local ap = getAPForIface(key)
            binding_wireless.sectionname = ap
            binding_wireless.option = "public"
            uci_helper.set_on_uci(binding_wireless, value, commitapply)
        end,
        ...
        RegulatoryDomain = function(mapping, param, value, key)
            ...
            binding_wireless.sectionname = radio
            binding_wireless.option = "country"
            uci_helper.set_on_uci(binding_wireless, value, commitapply)
        end,
        ...
        AuthenticationServiceMode = function(mapping, param, value, key)
            if not isOptionCanModifiedChecking(key) then
                ...
                binding_wireless.sectionname = ap
                binding_wireless.option = "security_mode"
                uci_helper.set_on_uci(binding_wireless, mode, commitapply)

                --Bandsteer processing: if bs is enabled reset the security_mode for bs peer node
                toModifyBSPeerNodeAuthentication("security_mode", mode, key)
            end
        end,
        ...
        AutoChannelEnable  = function(mapping, param, value, key)
            ...
            if flag then
                uci_helper.set_on_uci(binding_wireless, channel, commitapply)
            end
        end,
        ...
        X_000E50_ChannelMode = function(mapping, param, value, key)
            ...
            binding_wireless.sectionname = radio
            binding_wireless.option = "channel"
            uci_helper.set_on_uci(binding_wireless, p, commitapply)
        end,
        ...
        X_000E50_UpgradeURL = function(mapping, param, value, key)
            local radio = getRadioForIface(key)
            binding_wireless.sectionname = radio
            binding_wireless.option = "remote_upgrade_url"
            uci_helper.set_on_uci(binding_wireless, value, commitapply)
        end,
        X_000E50_UpgradeCheckPeriod = function(mapping, param, value, key)
            local radio = getRadioForIface(key)
            binding_wireless.sectionname = radio
            binding_wireless.option = "remote_upgrade_check_period"
            uci_helper.set_on_uci(binding_wireless, value, commitapply)
        end,
    ...
    local entriesAssociatedDevice = function(mapping, parentkey)
        binding_wireless.sectionname = parentkey
        binding_wireless.option = "ssid"
        local ssid = uci_helper.get_from_uci(binding_wireless)
        local ap = getAPForIface(parentkey)
    ...
    local getWLANStats = {
        DiscardPacketsReceived = function(mapping, param, key)
           ...
           return tostring(ssidstats["rx_discards"])
        end,
        DiscardPacketsSent = function(mapping, param, key)
           local ssidstats = getDataFromSsidStats(key)
           return tostring(ssidstats["tx_discards"])
        end,
        ErrorsReceived = function(mapping, param, key)
           local ssidstats = getDataFromSsidStats(key)
           return tostring(ssidstats["rx_errors"])
        end,
        ErrorsSent = function(mapping, param, key)
           local ssidstats = getDataFromSsidStats(key)
           return tostring(ssidstats["tx_errors"])
        end,
   }
   ...
   local function getallWLANStats(mapping, key)
        local ssidstats = getDataFromSsidStats(key)
        return {
            DiscardPacketsReceived = tostring(ssidstats["rx_discards"]),
            DiscardPacketsSent = tostring(ssidstats["tx_discards"]),
            ErrorsReceived = tostring(ssidstats["rx_errors"]),
            ErrorsSent = tostring(ssidstats["tx_errors"]),
        }
  end
    return {
        wlan = {
