...
local function mapRequestTags(tags)
  local r = {}
  for _, tagname in ipairs(requestTagList) do
    r[tags[tagname]] = true
  end
  return r
end

local function requestTags(self)
  local r = self._requestTags
  if not r then
    r = mapRequestTags(self.tags)
    self._requestTags = r
  end
  return r
end
...
local function isRequest(self, tag)
  local request = requestTags(self)
  return request[tag]
end

local function more_data(self)
  return self.index <= #self.message
end
...
function Decoder:RESOLVE_REQ()
  local path = decode_string(self)
  local key = decode_string(self)
  local sync = true
  if more_data(self) then
    sync = decode_byte(self)~=0
  end
  return { path = path, key = key, sync=sync }
end
