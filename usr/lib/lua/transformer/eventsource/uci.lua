...
local function check_watches(action, config, sectionname, option, value)
  ...
  for cb, mappings in pairs(callbacks) do
    for mapping in pairs(mappings) do
      local rc, ret = pcall(cb, mapping, action, config, "?", sectionname, option, value)
      if not rc then
      ...
      elseif ret then
        for _, info in ipairs(ret) do
          ...
          if info.paramname and not mapping.objectType.parameters[info.paramname] then
            ...
            local path = resolve(store, mapping.objectType.name, info.key, false)
            if path then
              path = path .. "."
...
--   - The new UCI value if we are performing a "set" action.
--
