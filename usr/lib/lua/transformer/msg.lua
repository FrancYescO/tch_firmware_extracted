...
Msg.tags = {
  ...
  -- * optionally a byte to encode the sync flag. nonzero for sync
  --   and zero for no sync.
...
-- The maximum size of a socket buffer in normal usage
-- The size of the buffer can only be bigger if a parameter
-- value of about this size is encoded. 
...
-- except in exceptional case like a 32K parameter value.
-- proper usage is:
--     local max_size = msg.softLimit(uds.MAX_DGRAM_SIZE)
--     msg:init_encode(tag, max_size, uuid)
