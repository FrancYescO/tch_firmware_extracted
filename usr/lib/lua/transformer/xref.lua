...
local function resolve_impl(store, typepath, key, sync)
  sync = (sync==nil) or sync
  ...
  if last_multi then
    ...
    store:getkeys(maplist, empty, empty, not sync)
    path = typepath
  end

  if not path and not inumbers and not sync then
  ...
  if inumbers then
    ...
    store:getkeys(maplist, inumbers, empty, not sync)
    -- generate full path
...
-- @param key the key
-- @param sync Boolean indicating whether to synchronize with the mapping(s)
--                when resolving fails. (defaults to true)
...
-- result.
function M.resolve(store, typepath, key, sync)
  local rc, path = pcall(resolve_impl, store, typepath, key, sync)
  if not rc then
    return nil
  end
  return path
