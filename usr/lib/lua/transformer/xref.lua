...
function M.resolve(store, typepath, key, no_sync)
  ...
  if #maplist>0 then
    -- get the typepath ID of the longest multi instance subpath from typepath
    local lastMI = maplist[#maplist].tp_id
...
function M.tokey(store, objectpath, typepath, ...)
  ...
  return store.persistency:getKey(mapping.tp_id, inumbers), tp
end
