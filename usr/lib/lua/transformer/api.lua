...
-- (do_transaction_pcall requires an extra first parameter)
local function xref_resolve(_, uuid, ...)
  return xref.resolve(...)
end
...
function Transformer:resolve(uuid, typePath, key, sync)
    self.store:setClientUUID(uuid)
    local rc, errcode, errmsg = do_transaction_pcall(xref_resolve, self, uuid, self.store, typePath, key, sync)
