...
local function validate_mapping(mapping, ignore_patterns)
  local objtype = mapping.objectType
  ...
  if is_ignored(name, ignore_patterns) then
  ...
  for pname, ptype in pairs(objtype.parameters) do
    ...
    if is_ignored(name..pname, ignore_patterns) then
...
local function create_map_env(store, commitapply, ignore_patterns)
  -- The environment available to a mapping.
  -- All these functions can throw an error.
  local function register(mapping)
    local ok, reason = validate_mapping(mapping, ignore_patterns)
    if ok then
...
function M.load_all_maps(store, commitapply, mappath, ignore_patterns)
  local map_env = create_map_env(store, commitapply, ignore_patterns)
