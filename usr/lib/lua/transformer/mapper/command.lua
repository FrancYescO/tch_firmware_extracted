...
local function create_setter(command_mapping)
  command_mapping.set = function(mapping, paramname, paramvalue, parentkey)
    ...
    if paramname == "launch_command" then
      ...
      local command_socket = session_cache[parentkey]["command_socket"]
      session_cache[parentkey] = nil
      return fork_and_exec(mapping._command.full_path, exec_args, command_socket, parentkey)
    else
