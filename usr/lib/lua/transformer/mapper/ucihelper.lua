...
function M.foreach_on_uci(binding,func)
  --trace_binding(binding, "foreach_on_uci")
  local config = binding.config
  local section = binding.sectionname
  if not config then
    error("No config could be found in the given binding", 2)
  end
  -- Use a separate read-only cursor for the loop. Another ucihelper function can be
  -- passed as second argument which can corrupt the cursor. Never use a writable cursor for this!
  local cursor = binding.state ~= false and state_cursor or foreach_cursor --doc.ucidoc#cursor
  local result = refresh_cursor(binding, cursor)
  -- Stop the cursor that is being used in a foreach loop from being unloaded by
  -- locking it.
  if cursors_in_foreach[cursor] then
    cursors_in_foreach[cursor] = cursors_in_foreach[cursor] + 1
  else
    cursors_in_foreach[cursor] = 1
  end
  if result then
    result = cursor:foreach(config, section, func)
  end
  if cursors_in_foreach[cursor] == 1 then
    cursors_in_foreach[cursor] = nil
  else
    cursors_in_foreach[cursor] = cursors_in_foreach[cursor] - 1
  end
  return result
