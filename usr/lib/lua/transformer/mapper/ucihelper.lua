...
local function commit_keys(binding)
  refresh_cursor(binding, keycursor)
  return keycursor:commit(binding.config)
end
...
local function revert_keys(binding)
  refresh_cursor(binding, keycursor)
  keycursor:revert(binding.config)
  keycursor:save(binding.config)
...
--                config
function M.revert_on_uci(binding)
  local config = binding.config
  if not config then
    error("No config could be found in the given binding", 2)
  end
...
--- Function which generates a unique key for the given section in the
-- given config in uci.
...
function M.generate_key_on_uci(binding)
  local config = binding.config
  local section = binding.sectionname
  if not config then
    error("No config could be found in the given binding", 2)
  ...
  local key = M.generate_key()
  local result

  result = keycursor:set(config, section, "_key", key)
  if result then
    result = keycursor:save(config)
  end
  keycursor:unload(config)
  return key
end
