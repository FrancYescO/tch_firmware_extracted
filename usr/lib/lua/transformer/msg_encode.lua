...
local function can_use_hard_limit(self)
  return self.data_len>self.soft_size or --hard limit already in use
         self.first_encoding
end

--- Does the new temp data fit into the maximum buffer
-- @param #table self A reference to the message encoder
...
-- @param #table self A reference to the message encoder
-- @return true if the new data was added, false if the new data
...
function Encoder:RESOLVE_REQ(path, key, sync)
  encode_string(self, path)
  encode_string(self, key)
  if sync~=nil then
    encode_byte(self, sync and 1 or 0)
  end
  return confirm_encoding(self)
end
...
-- @param max_size The maximum size of a single message. This is either a single number
