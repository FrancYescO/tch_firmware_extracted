...
function Param:get()
  ...
  if all_values then
    ...
    -- parameter definition forces us not to.
    if not param.single then
  ...
  if not pvalue then
    ...
    if type_get == "function" then
      local rc, errmsg
      rc, pvalue, errmsg = xpcall(getter, traceback, mapping, paramname, unpack(keys))
      if not rc then  -- getter threw an error, which is returned in 'pvalue'
        logger:error("getter threw an error: %s", pvalue)
        fault.InternalError("get() failed: %s", pvalue)
      elseif not pvalue then  -- getter returned nil + an error (or returned false)
        fault.InternalError("get() failed: %s", (errmsg or "invalid value"))
      end
...
function Object:delete()
  ...
  self.store.persistency:delKey(objtype.name, self.irefs)
...
function Objtype:add(name)
  ...
  local instance = self.store.persistency:addKey(objtype.name, self.irefs, svalue)
...
output_instance = function(it_state)
  ...
  for childmapping in it_state.store:children(it_state.mapping) do
    ...
    -- single instance objtype?
    if objtype.minEntries == 1 and objtype.maxEntries == 1 then
...
local function navigate(store, path, action, level)
  ...
  local keys = store:getkeys(typepath, irefs)
  assert(#irefs == #keys)
