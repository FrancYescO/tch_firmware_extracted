...
function Role:authorize_command(command)
  if self.allowed_commands[command] then
    return true
  end
  return false
end
...
function Role:authorize_path(path)
  ...
  if self:is_whitelist() then
    ...
    paths = self.disallowed_paths
    default = true
  end
  for pattern in pairs(paths) do
...
local function read_config()
  ...
  for role, params in pairs(config.roles) do
    if not params.allowed_commands then
      ngx.log(ngx.ERR, "no allowed_commands for role ", role)
      config.roles[role] = nil
    elseif params.allowed_paths and params.disallowed_paths then
      ngx.log(ngx.ERR, "allowed_paths and disallowed_paths can not be mixed for role ", role)
      config.roles[role] = nil
    elseif not params.allowed_paths and not params.disallowed_paths then
      ngx.log(ngx.ERR, "no allowed_paths or disallowed_paths for role ", role)
      config.roles[role] = nil
    end
  end
