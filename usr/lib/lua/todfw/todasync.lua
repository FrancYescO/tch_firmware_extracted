...
function M.start()
    ...
    events['time.changed'] = function(msg)
        if msg ~= nil then
            ...
            local nd = os.date("%a", newtime)
            local nt = os.date("%X", newtime)
            local newtout, starttime, stoptime, startday, stopday
            ...
            for actname,action in pairs(actionlist) do
                local oldtlfound = false -- per action variable
                local newtlfound = false -- per action variable
                            ...
                            if stopday ~= nil and stoptime ~= nil and action.timer[stopday] ~= nil and action.timer[stopday][stoptime] ~= nil and action.timer[stopday][stoptime].cbtype == "stop" then
                                ...
                                if action.timer[stopday][stoptime].tout - action.timer[day][time].tout < 0 then
                                    if action.timer[day][time].active == true then
                                        ...
                                        oldtlfound = true
                                        runtime.logger:notice("update time, still in the same time slot: "..actname.." "..day.."_"..time.."-"..stopday.."_"..stoptime)
                                    else
                                        ...
                                        newtlfound = true
                                        runtime.logger:notice("update time, into the time slot: "..actname.." "..day.."_"..time.."-"..stopday.."_"..stoptime.." start timeout="..tostring(newtout/1000).."s")
                                    end
                                elseif action.timer[day][time].active == true and action.timer[stopday][stoptime].tout - action.timer[day][time].tout > 0 then
                                    ...
                                    oldtlfound = true
                                    runtime.logger:notice("update time, out from the time slot: "..actname.." "..day.."_"..time.."-"..stopday.."_"..stoptime.." sotp timeout="..tostring(newtout/1000).."s")
                                end
                            else
                                runtime.logger:notice("no stop pair of start time "..actname.." "..day.."_"..time)
                            end
                        end
