...
function M.start(actionname, actiontb)
    ...
    local reapply_after_boot = actiontb.reapply_after_boot	
    if reapply_after_boot == nil then
    else
	if reapply_after_boot=='1' then
		force_restart=1
	end	
    end 
    ...
    if next(action.timerlist) ~= nil then
        for _,timer in pairs(action.timerlist) do
            ...
            obsolete_active_ts_nb = todtimer.checktimeslot(action[timer], obsolete_active_ts_nb, valid_activedaytime,force_restart)
        end
    end
