...
function M.GetDnsServers()
  ...
  local ifname = "@wan"
  if param:match("^@") and param:match("vlan_wan_eth4") == nil then
    ifname = param
  end
...
function M.GetSystemInfo()
  ...
  local ifname = untaint(dm.get("uci.network.interface.@wan.ifname")[1].value)
  ifname = ifname:match("^@(.+)")
  if ifname and ifname:match("vlan_wan_eth4") == nil  then
    sysinfo.wangw = intfobj[ifname].nexthop
  else
