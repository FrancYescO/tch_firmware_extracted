...
-- Handles the received wps button event.
-- If wps button press event received, authentication response will be sent.
...
local function triggerAuthMessage(payload)
  ...
  elseif payload.physicalAuthentication.type == "wps" and payload.physicalAuthentication.timeout then
    ...
    mqtt_lookup.uloop.timer(function()
      sendAuthResponse = false
      log:debug("Physical authentication timed out")
...
-- Sends authentication message to the backend for verification
-- @function auth
...
function M.auth(lookup, payload, handler)
  ...
  if payload and registerStatus == 1 then
    ...
    elseif not payload.requestId then
      log:error("Authentication Request without a 'requestId' value")
    end
  else
