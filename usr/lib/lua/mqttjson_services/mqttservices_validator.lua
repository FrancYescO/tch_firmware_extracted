...
--  local radioData = M.mqtt_lookup.ubusConn:call("wireless.radio", "get", { name = radio }) or {}
  local radioData = ubus:call("wireless.radio", "get", { name = radio }) or {}
...
--  local apData = M.mqtt_lookup.ubusConn:call("wireless.accesspoint.security", "get", { name = ap }) or {}
  local apData = ubus:call("wireless.accesspoint.security", "get", { name = ap }) or {}
...
--- Validates the wifitod rule timer name.
-- @function validateTimerName
-- @param value wifitod timer name
-- @return true / false
function M.validateTimerName(value)
  return value == 'wifitod' and true or false
end

--- Validates the wifitod rule timer start and stop time.
-- @function validateWifitodTime
-- @param value Timer start/ stop time
-- @return true / false
function M.validateWifitodTime(value)
  local days, hours, mins = value:match("^(%S+):(%d+):(%d+)$")
  if days and hours and mins then
    hours, mins = tonumber(hours), tonumber(mins)
    for day in gmatch(days, "%a+") do
...
--- Validates the wifitod rule action script.
-- @function validateTodActScript
-- @param value wifitod action script
-- @return true / false
function M.validateTodActScript(value)
  return value == 'wifitodscript' and true or false
end

--- Validates the wifitod rule action object.
-- @function validateTodActObject
-- @param value wifitod action object
-- @return true / false
function M.validateTodActObject(value)
  return match(value, "wifitod%.wifitod%S+") and true or false
end

--- Validates the wifitod rule action timer value.
-- @function validateTodActTimer
-- @param value wifitod action timer value
-- @return true / false
function M.validateTodActTimer(value)
  return match(value, "timer%S+") and true or false
end

--- Validates the wifitod rule SSID or Accesspoint.
-- @function validateTodSsidAP
-- @param value SSID / AP in wifitod rule.
-- @return true / false
function M.validateTodSsidAP(value, key, param)
  if type(value) == "string"  then
    ...
    local wifi_data
    if param == "ssid" then
      wifi_data = ubus:call("wireless.ssid", "get", {}) or {}
    elseif param == "ap" then
      wifi_data = ubus:call("wireless.accesspoint", "get", {}) or {}
    end
    for key, data in pairs(wifi_data) do
      if param == "ssid" and match(value, data.ssid) then
        return true
      elseif param == "ap" and value == key then
        return true
      end
    end
  end
