...
create_ipset() {
    # flush the ipsets before populating them
...
flush_ipset() {
    ipset flush "${ipset_table4}"
    ipset flush "${ipset_table6}"
    logger -t intercept "flush nointercept[46] ipsets"
}
...
intercept_set_state() {
    ...
    [ $active == 1 ] && create_ipset || flush_ipset
    firewall_update
    restart_dnsmasq
...
case "$1" in
    ...
    init_state
    state_set_active 0 #initialize to off, then switch to $active
    logger -t intercept "boot in state $active"
