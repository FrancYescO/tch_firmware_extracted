...
datamodel_check() {
    ...
    logger "intercept: confirm $intf via datamodel"
    local STATUS="$(transformer-cli get ${dmstatus} | awk -F' = *'  '{print $2}')"
    if [ "${STATUS##Error_}" != "${STATUS}" ]
    then
	logger "intercept: $intf shows ${STATUS} via $dmstatus"
	return 1
    fi
}
...
check_all_wan_down() {
    ...
    logger "intercept: WAN connectivity $wan_is_up"
    return $wan_is_up
}
...
case "$1" in
	...
	# when all wan interfaces are down, do not stop intercepting
	check_all_wan_down || intercept_set_state 0
	exit 0
	;;
    ifdown)
	# don't handle hotplug events if intercept is not started yet (see boot)
