...
ip_flush() {
    ...
    ip -4 route del table intercept
    # IPv6
    ip -6 rule del table intercept
    ip -6 route del table intercept
}
...
ip_setup() {
    ...
    # IPv4
    ip -4 rule add iif $lan_dev fwmark $INTERCEPT_MARK pref 1 table intercept
    ip -4 route add local 0/0 dev lo table intercept
    # IPv6
    ip -6 rule add iif $lan_dev fwmark $INTERCEPT_MARK pref 1 table intercept
