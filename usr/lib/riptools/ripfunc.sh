...
findrip_mtd() {
	local part=$(grep -m1 $RIPNAME /proc/mtd | cut -d ' ' -f1 | tr -d ': ')
	if [ -z "$part" ]; then
		return
	fi
	RIPDEV=/dev/$part
	RIPTYPE=MTD
	RIPSIZE=$((0x$(grep -m1 eripv2 /proc/mtd | cut -d ' ' -f2)))
}

findrip_emmc() {
	local part="mmcblk0boot1"
	RIPDEV=/dev/$part
	RIPTYPE=EMMC
	RIPSIZE=131072
}
...
if cat /proc/partitions | grep -q mmcblk; then
...
update_rip_partition() {
	...
	elif [ "$RIPTYPE" = "EMMC" ]; then
		cmd="dd if=$img of=$RIPDEV"
	else
