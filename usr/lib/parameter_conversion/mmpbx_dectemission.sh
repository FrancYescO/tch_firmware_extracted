...
if [ "$emission" = 0 ] && [ -f "/etc/config/mmpbxbrcmdectdev" ];
then
...
#  emission is 1 and BRCM DECT configuration is not available but BRCM DECT library is: config has to be restored from back-up
#  TODO: DSPG DECT configuration back-up and restore
elif [ "$emission" = 1 ] && [ ! -f "/etc/config/mmpbxbrcmdectdev" ] && [ -f "/rom/usr/lib/mmpbx/libmmpbxbrcmdect.so" ];
then
