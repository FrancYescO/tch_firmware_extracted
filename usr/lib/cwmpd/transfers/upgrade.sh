...
do_upgrade()
{
  ...
  if [ -x /usr/bin/sysupgrade-safe ]; then
    /usr/bin/lua /usr/lib/cwmpd/transfers/checkVoiceCalls.lua
    if [ $SWITCHOVER_TYPE = "1" ]; then
...
remember_bank()
{
  ...
  if is_dual_bank && [ -f /proc/banktable/notbooted ] ; then
    uci set $CONFIG.$id.bank=$(cat /proc/banktable/notbooted)
  fi
}
...
if [ "$TRANSFER_ACTION" = "start" ]; then
  ...
  if [ "$STARTED" != "yes" ]; then
    ...
    if [ ! -f $WAIT_FOR_SWITCHOVER_FILE ]; then
      do_upgrade "$URL"
      ...
      #the file cwmp_waitforswitchover exists, need to switchover before next upgrade.
      E="1,wait for switch over"
      set_error "$TRANSFER_ID" "$E"
    fi
...
if is_dual_bank && [ "$TRANSFER_ACTION" = "rollback" ]; then
    echo "Failed connect to ACS URL , start rollback to old bank"
    if [ -f /proc/banktable/notbooted ] ; then
        cat /proc/banktable/notbooted > /proc/banktable/active; reboot
    else
        echo "Rollback error, /proc/banktable/notbooted not exist, exit"
        exit 1
    fi
    exit 0
fi
