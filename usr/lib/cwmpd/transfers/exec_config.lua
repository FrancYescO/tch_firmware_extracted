...
local uci = require('uci').cursor(CONF_DIR)
...
local function do_add_list(args)
    ...
    if (arg.value~='') and not table_index(v, arg.value) then
        v[#v+1] = arg.value
        uci:set(arg.config, arg.section, arg.option, v)
        changed_config(arg.config)
    else
        printf("*warning: value %s already in list", tostring(value))
    end
end

-- delete an item from a list option
