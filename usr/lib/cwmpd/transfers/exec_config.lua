...
local check_forbidden = require('f_commands').scan_for_forbidden
...
local function do_set(args)
    ...
    -- Ignore certain 'set config.section.option=value' commands that could be applied via sts file sent from the ACS.
    if check_forbidden(arg) then
       error(errmsg("Config- %s Section- %s Option- %s is not supposed to be modified via ACS", arg.config, arg.section, arg.option))
    end
    ...
    if arg.option then
        if not section[arg.value] then
            ...
            if section[arg.option] then
                -- Ignore certain 'rename config.section.option=value' commands that could be applied via sts file sent from the ACS.
                if check_forbidden(arg) then
                    error(errmsg("Config- %s Section- %s Option- %s is not supposed to be modified via ACS", arg.config, arg.section, arg.option))
                end
        ...
        if not find_section_named(arg.config, section['.type'], arg.value) then
            -- Ignore certain 'rename config.section=value' commands that could be applied via sts file sent from the ACS.
            if check_forbidden(arg) then
                error(errmsg("Config- %s Section- %s is not supposed to be renamed via ACS", arg.config, arg.section))
            end
...
local function do_add(args)
    ...
    local name = args[3]
    if section then
        -- Ignore certain 'add config section [name]' commands that could be applied via sts file sent from the ACS.
        if check_forbidden(args) then
            error(errmsg("Section of Config- %s is not supposed to be added via ACS", args[1]))
        end
...
local function do_add_list(args)
    ...
    if (arg.value~='') then
        v[#v+1] = arg.value
        -- Ignore certain 'add_list config.section.option=value' commands that could be applied via sts file sent from the ACS.
        if check_forbidden(arg) then
            error(errmsg("Config- %s Section- %s Option- %s is not supposed to be modified via ACS", arg.config, arg.section, arg.option))
        end
...
local function do_del_list(args)
    ...
    if v_index then
        ...
        until not v_index
        -- Ignore certain 'del_list config.section.option=value' commands that could be applied via sts file sent from the ACS.
        if check_forbidden(arg) then
            error(errmsg("Config- %s Section- %s Option- %s is not supposed to be deleted via ACS", arg.config, arg.section, arg.option))
        end
...
local function do_delete(args)
    ...
    if arg.option then
        -- Ignore certain 'delete config.section.option' commands that could be applied via sts file sent from the ACS.
        if check_forbidden(arg) then
            error(errmsg("Config- %s Section- %s Option- %s is not supposed to be deleted via ACS", arg.config, arg.section, arg.option))
        end
        uci:delete(arg.config, arg.section, arg.option)
    else
        -- Ignore certain 'delete config.section' commands that could be applied via sts file sent from the ACS.
        if check_forbidden(arg) then
            error(errmsg("Config- %s Section- %s is not supposed to be deleted via ACS", arg.config, arg.section))
        end
