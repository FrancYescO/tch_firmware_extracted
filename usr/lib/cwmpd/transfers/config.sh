  ...
  if [ "$STARTED" = "no" ]; then
    if [ ! -f $DLFILE ]; then
      ...
      if [ -n "$user_passwd" ]; then
         curl -u "$user_passwd" "$TRANSFER_URL"  -S -s --insecure --capath /etc/ssl/cwmp -o "$DLFILE" $client_auth_arguments
      else
         curl "$TRANSFER_URL" -S -s --insecure --capath /etc/ssl/cwmp -o "$DLFILE" $client_auth_arguments
      fi
