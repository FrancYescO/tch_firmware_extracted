...
sfpport=$(uci -q get fastweb.port_shaping.sfpport)
[ -z $sfpport ] && sfpport="eth4"
...
if [ "$interface" = "$sfpport" ]; then
  if [ "$ds_disable" = "0" ] && [ -n $ds_rate ] && ([ -z $swshaping ] || [ "$swshaping" = "1" ]); then
    tc qdisc del dev $interface ingress
    del_skiplog_rules

    tc qdisc add dev $interface handle ffff: ingress
    tc filter add dev $interface parent ffff: protocol ip prio 10 u32 match ip $tcrule police rate ${ds_rate}kbit burst ${ds_burst}k drop flowid :1
    add_skiplog_rules
    fcctl flush
