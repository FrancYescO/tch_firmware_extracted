...
if mapControlEnabled and mapControlEnabled  == "1" then
	...
	for i, v in pairs(ap_intf) do
		...
		if backhaul_state and backhaul_state[1].value == "1" or v == "wl1_2" then
	...
	for _, bsVal in pairs(bs_list) do
		local bs_last_state = proxy.get("uci.system.wifi-bandsteer.@" ..bsVal.. ".last_state")
        ...
        for i, v in pairs(ap_intf) do
                local backhaul_state = proxy.get(intfPath.."@"..v..".backhaul")
                local fronthaul_state = proxy.get(intfPath.."@"..v..".fronthaul")
