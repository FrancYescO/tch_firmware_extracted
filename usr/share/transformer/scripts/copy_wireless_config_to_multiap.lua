...
local controllerSelectionEnabledPath = "uci.multiap.global.@controller_selection.enabled"
				...
				for kcred, vcred in pairs(v) do
					...
					if wpa_psk_key_val and isWirelessCredsChanged(contentControllerCreds[vcred]["wpa_psk_key"], wpa_psk_key_val) then
						...
						changeApplied = true
                                        ...
                                        local state = proxy.get(baseapPath .. ".state")
                                        state = state and state[1].value or ""
                                        if state and isWirelessCredsChanged(contentControllerCreds[vcred]["operational_state"], state) then
                                          credConfig[vcred]["state"] = state
                                          changeApplied = true
                                        end
                                        local publicstate = proxy.get(baseapPath .. ".public")
                                        publicstate = publicstate and publicstate[1].value or ""
                                        if publicstate and isWirelessCredsChanged(contentControllerCreds[vcred]["public_state"], publicstate) then
                                          credConfig[vcred]["public"] = publicstate
                                          changeApplied = true
                                        end
...
local backhaul_state, fronthaul_state, bs_state, bs_last_state
...
local controllerSelectionEnabled = proxy.get(controllerSelectionEnabledPath)
controllerSelectionEnabled = controllerSelectionEnabled and controllerSelectionEnabled[1].value
...
if mapControlEnabled and mapControlEnabled  == "1" then
	...
	local _, err
	if isControllerCredUpdated and (controllerSelectionEnabled == "" or controllerSelectionEnabled == "0") then
	...
	-- Disable BS when Easymesh is enabled
		...
		bs_state = proxy.get(bsStatePath.. "@" ..bsVal.. ".state")
		bs_state = bs_state and bs_state[1] and bs_state[1].value or "0"
		proxy.set("uci.system.wifi-bandsteer.@" ..bsVal.. ".last_state", bs_state)
                proxy.set(bsStatePath.."@"..bsVal..".state", "0")
        end
        local main_guest_ap, wps_state
        for network, values in pairs(webData) do
          for _, iface in pairs(webData[network]) do
            for _, ap in pairs(aps) do
              main_guest_ap = proxy.get(apPath.."@"..ap..".iface")
              if main_guest_ap and main_guest_ap[1].value == iface then
                wps_state = network == "main" and "1" or "0"
                proxy.set(apPath.."@"..ap..".wps_state",wps_state)
	...
	for i, v in pairs(ap_intf) do
		backhaul_state = proxy.get(intfPath.."@"..v..".backhaul")
		if backhaul_state and backhaul_state[1].value and backhaul_state[1].value ~= "" then
	...
	for _, bsVal in pairs(bs_list) do
		bs_last_state = proxy.get("uci.system.wifi-bandsteer.@" ..bsVal.. ".last_state")
	...
	if smartWifiStatus == "" then
	  local multiAPData = convertResultToObject(mapCredPath, proxy.get(mapCredPath))
          ...
          for i, v in pairs(ap_intf) do
                  backhaul_state = proxy.get(intfPath.."@"..v..".backhaul")
                  fronthaul_state = proxy.get(intfPath.."@"..v..".fronthaul")
                  backhaul_state = backhaul_state[1] and backhaul_state[1].value == "1"
                  fronthaul_state = fronthaul_state[1] and fronthaul_state[1].value == "1"
                  if backhaul_state then
                    proxy.set(intfPath.."@"..v..".backhaul","0")
	            proxy.set(intfPath.."@"..v..".state","0")
                  end
                  if fronthaul_state then
                    proxy.set(intfPath.."@"..v..".fronthaul","0")
                  end
          end
	end
	proxy.apply()
