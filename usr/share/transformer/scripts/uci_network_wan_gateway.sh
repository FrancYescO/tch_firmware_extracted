...
if [ "$wan_ip" == "_SET_BY_CWMP_SCRIPT_" -o "$wan_ip" == "" -o "$wan_ip" == "0.0.0.0" ]; then
...
uci set firewall.MARK_DATA_SRCIP.enabled='0'
uci set firewall.MARK_SPEEDTEST_DATA_SRCIP.enabled='0'
uci set firewall.MARK_CPE_DNSQUERY_SRCIP.enabled='0'
uci set firewall.MARK_BULKDATA_SRCIP.enabled='0'
uci set firewall.MARK_NGWFDD_SRCIP.enabled='0'
uci set firewall.@zone[2].output='DROP'
uci set firewall.PSandXBoxLive1.src_dip='_SET_BY_CWMP_SCRIPT_'
uci set firewall.PSandXBoxLive2.src_dip='_SET_BY_CWMP_SCRIPT_'
uci set firewall.PSandXBoxLive3.src_dip='_SET_BY_CWMP_SCRIPT_'
...
uci set firewall.MARK_DATA_SRCIP.enabled='1'
uci set firewall.MARK_SPEEDTEST_DATA_SRCIP.enabled='1'
uci set firewall.MARK_CPE_DNSQUERY_SRCIP.enabled='1'
uci set firewall.MARK_BULKDATA_SRCIP.enabled='1'
uci set firewall.MARK_NGWFDD_SRCIP.enabled='1'
uci set firewall.MARK_LAN_TO_WAN.enabled='1'
uci set firewall.@zone[2].output='ACCEPT'

uci set firewall.PSandXBoxLive1.src_dip=$wan_ip
uci set firewall.PSandXBoxLive2.src_dip=$wan_ip
uci set firewall.PSandXBoxLive3.src_dip=$wan_ip
