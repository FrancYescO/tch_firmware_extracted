...
local find, match, gmatch, untaint = string.find, string.match, string.gmatch, string.untaint
...
local function isReservedPort(port, portMapValues, wanServ)
  local portStart, portEnd = parsePortRange(untaint(port))
  for resPort, resProto in pairs(wanServ) do
    resPort = tonumber(untaint(resPort))
    if tonumber(portStart) and tonumber(portEnd) then
      if (tonumber(portStart) <= resPort and resPort <= tonumber(portEnd)) then
