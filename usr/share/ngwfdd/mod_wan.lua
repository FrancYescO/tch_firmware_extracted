...
local function handle_interface_event(event)
 if next(event) then
   ...
   deviceName = gwfd.get_uci_param("network." .. event["interface"] .. ".ifname")
   if deviceName == nil then
     return
   end
   if is_wan_device(deviceName) then
     msg["name"] = deviceName
...
uloop.init()
log, ubus_conn = gwfd.init("gwfd_wan", 6, { return_ubus_conn = true })
