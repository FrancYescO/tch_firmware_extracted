...
function valid_nonempty_string(param)
...
function valid_msg_file_to_write(msg, fifo_file_path)
...
function common.get_transformer_param(path, msg)
  ...
  for _, param in ipairs(results) do
    local number = tonumber(param.value)
    if number then
...
function common.get_transformer_params(paths, msg)
