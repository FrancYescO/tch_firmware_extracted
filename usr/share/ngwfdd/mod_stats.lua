...
local timer
local interval = 600 * 1000 -- Every 10 Minutes
...
local function parse_proc_stat_file()
  ...
  while line do
    local cpu = string.match(line, cpu_exp)
    if cpu then
...
local function send_stats_data()
  local msg = {}
  local new_cpu_stats = get_cpu_params(cpu_stats, msg)
  cpu_stats = new_cpu_stats
  local rv, errmsg = gwfd.get_transformer_param("sys.mem.", msg)
  if not rv then
    log:error(errmsg)
    uloop.cancel()
  end
...
log = gwfd.init("gwfd_stats", 6, { init_transformer = true })

cpu_stats = parse_proc_stat_file()

timer = uloop.timer(send_stats_data)
timer:set(interval)
