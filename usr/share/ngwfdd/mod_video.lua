...
local igmp_snooping_keys = {
  "bridge", "device", "src-dev", "#tags", "lan-tci", "wan-tci", "group",
  "mode", "RxGroup", "source", "reporter", "timeout", "Index", "ExcludPt"
}
...
local timer
local interval = 30 * 60 * 1000 -- Every 30 minutes
    ...
    while line do
      ...
      if wan_dev_match == wan_dev then
        ...
        while i > 0 do
          ...
          local subscription_tokens = string.gmatch(line, "%S+")
...
local function get_igmp_snooping()
  ...
  local t = {}
  while line do
    local first = string.match(line, "([^%s]+)") -- First field is the bridge name
    if bridges[first] then
      local entry = {}
      local tokens = string.gmatch(line, "%S+")

      for _, v in pairs(igmp_snooping_keys) do
        entry[v:lower()] = tokens()
      end
...
local function get_hw_sw_acc_data()
  ...
  local lines = string.gmatch(all_lines, "([^\n]+)") -- break lines
  lines() --skip header
  lines() --skip another header

  for line in lines do
    local fo, swhits, swhittb, hwhits = string.match(line, "(%S+)%s+%d+:+%s+%d+%s+(%d+):%s+(%d+)%s+%S+%s+(%d+)")
    local entry = {}
    entry["FlowObject"] = fo
...
local function send_video_data()
  ...
  for _, intf in pairs(up_intfs) do
    ...
    local msg_stats = {}
    msg_stats = get_dev_stats(wan_dev)
...
uloop.init()
log, ubus_conn = gwfd.init("gwfd_video", 6, { return_ubus_conn = true, init_transformer = true })
