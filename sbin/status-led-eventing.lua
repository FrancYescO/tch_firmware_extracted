...
local statusled_enabled,err=lcur:get(ledcfg,'status_led','enable')
--cursor:close()
if statusled_enabled == nil then
   statusled_enabled=false
elseif statusled_enabled == '1' then
   statusled_enabled = true
elseif statusled_enabled == '0' then
   statusled_enabled = false
end
...
local function is_service_ok()
    ...
    if services.voip == false then
        ...
        if (voiceEnabled == nil) or (voiceEnabled == "0") or (profileEnabled == nil) or (profileEnabled == "0") then
            ignorevoip = true
        end
    end
    lcur:load(ledcfg)
    ...
    for k, v in pairs(services) do
        if v == false then
            if (tostring(k) ~= "voip" or not ignorevoip) and (ledfw == nil or ledfw[k]==nil or ledfw[k]['check']==nil or ledfw[k]['check']=='1') then
...
local function ledaction()
    if is_service_ok() == false then
        ...
        local packet = {}
        if infobutton_pressed == false and info_service_led_timeout > 0 and statusled_enabled then
            packet["state"] = "active"
            conn:send("statusled", packet)
        end
...
events['infobutton'] = function(msg)
    if msg ~= nil and msg.state == "active" and infobutton_pressed == false then
        ...
        conn:send("statusled", packet)
        -- Setup a timer
        if info_service_led_timeout > 0 then
...
local mobile_device_plugged=false
local mobile_session_connected=false
if services.mobile ~= nil then
    events['mobiled'] = function(msg)
        local new_value = false
        if msg ~= nil then
           if msg.event == "device_connected" then
               mobile_device_plugged = true
               mobile_session_connected = false
           elseif msg.event == "device_disconnected" then
               mobile_device_plugged = false
           elseif msg.event == "session_state_changed" and msg.session_state ~= nil then
               if msg.session_state == "disconnected" then
                   mobile_session_connected = false
               elseif (match(msg.session_state, "_connected_")) then
                   mobile_session_connected = true
               end
           end
        end
        if not mobile_device_plugged or mobile_session_connected  then
            new_value = true
        else
            new_value = false
        end
        if new_value ~= services.mobile then
            services.mobile = new_value
            ledaction()
        end
    end
end
