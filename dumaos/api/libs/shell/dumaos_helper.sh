...
if [ -f "/dumaossystem/model" ];then
  SYSROOT_PREFIX="/"
else
  SYSROOT_PREFIX="/media/dumaos_output"
fi

if [ -f "/lib/functions.sh" ] && [ -f "/lib/config/uci.sh" ];then
  . /lib/functions.sh
else
  . $SYSROOT_PREFIX/dumaos/api/libs/shell/uci.sh
fi

# All global variables to go here
export_global_variables(){
  export dumaos_status_lfile="/var/run/dumaos-status"
  export MODEL="$(cat $SYSROOT_PREFIX/dumaossystem/model 2>/dev/null)"
  export VENDOR="$(cat $SYSROOT_PREFIX/dumaossystem/vendor 2>/dev/null)"
  export ODM="$(cat $SYSROOT_PREFIX/dumaossystem/odm 2>/dev/null)"
  export SDK="$(cat $SYSROOT_PREFIX/dumaossystem/sdk 2>/dev/null)"
  export CHIPSET="$(cat $SYSROOT_PREFIX/dumaossystem/chipset 2>/dev/null)"
  export DUMAOS_VERSION="$(cat $SYSROOT_PREFIX/dumaossystem/version 2>/dev/null)"
  export FLAVOUR="$(cat $SYSROOT_PREFIX/dumaossystem/flavour 2>/dev/null)"
}
export_global_variables

# Generate DumaOS uci parameters
generate_dumaos_config(){
if [ ! -e "$SYSROOT_PREFIX/etc/config/dumaos" ];then
  touch /etc/config/dumaos
fi
uci set dumaos.tr69=tr69
uci set dumaos.tr69.dumaos_enabled=0
if [ "$(cat $SYSROOT_PREFIX/dumaossystem/model)" = "LH1000" ];then
