...
var dragElem = $("#firewall-drag",context)[0];
...
function create_rule_rows(rows){
  var table = $("#firewall-rules");
  var tableBody = table.find("tbody")[0];
  if(!tableBody){
    tableBody = document.createElement("tbody");
    $(tableBody).toggleClass("drag_container")
    Polymer.dom(table[0]).appendChild(tableBody);
  }else{
    $(tableBody).empty();
  }
  ...
  {
    var row = rows[i];
    var rowhtml = $('<tr class="row"></tr>')[0];
    $(rowhtml).attr("row_id",row.id)
    for(var t = 0; t < row.data.length; t++){
      var datahtml = $("<td></td>").html(row.data[t][0])[0];
      Polymer.dom(rowhtml).appendChild(datahtml);
    }
    Polymer.dom(tableBody).appendChild(rowhtml);
  }
}
...
function format_rule(index,rule_id, enabled, name, deviceNames, targets, schedule, allow, notifications,canReject = true) {
  var row = [
    ...
    r(format_schedule(schedule,allow).addClass('non-overflow')),
...
function r(content,asText=false,classes=undefined){
  if(asText){
    return $("<span class='rule-name'></span>").text(content);
  }
  else{
    return $(content);
  }
...
function format_priority(priority,rule_id){
  ...
  div.append(gap(5).text(priority))
    .append('<iron-icon icon="duma-icons:drag-indicator" class="drag_handle"></iron-icon>');
  return div;
}
...
function format_drop_down(action, allowed_values, rule_id=undefined,id=undefined){
  ...
  for(var i = 0; i < allowed_values.length; i ++){
    options.push('<paper-item value="' + allowed_values[i].toLowerCase() + '">' + (allowTypes_translations[allowed_values[i]] || allowed_values[i]) + '</paper-item>');
  }
  var drop = ['<paper-dropdown-menu no-label-float allow-outside-scroll="true" class="dropdown-small"']
  ...
  drop.push('</paper-listbox></paper-dropdown-menu>');
  return drop.join('');
}
...
function format_schedule(schedule,action){
  var cron = new Cron(schedule);
  var am = format_timewheel(cron.am(),false,action);
  var pm = format_timewheel(cron.pm(),true,action);
  var main = $('<div></div>').addClass("timewheels");
  var timewheels = $('<div></div>')
    .append(am)
    .append(pm);
  var days = $(document.createElement("duma-days"));
  days[0].parseCron(cron);
  days.attr("allow",action);
  main.append(timewheels);
  main.append(days);
  return main;
}
function format_timewheel(boolArr,pm,action){
  var timewheel = $("<duma-timewheel small " + (pm ? "pm" : "") +"></duma-timewheel>");
  timewheel.attr("starting",JSON.stringify(boolArr));
  timewheel.attr("allow",action);
  return timewheel;
}
...
function on_start_delete_rule(target){
  ...
  $("duma-alert", context)[0].open("<%= i18n.deleteConfirm %>".format(name),[
    {text:"<%= i18n.yes %>",default:true,action:"confirm",callback:rule_delete_callback},
...
function refreshRules(){
  ...
  Q.spread([
    ...
    load_devices(ids).then(function(devices) {
      ...
      create_rule_rows(rows);
      on_refresh();
      refresh_main_disabled();
      dragElem.set("dragEnabled",true);
    });
...
function switchRules(rule_id,to){
  ...
  to = parseInt(to);
  var drag_rows = $("#firewall-rules #drag_container > tr:not(.gu-mirror) > td:first-child span");
  ...
  dragElem.set("dragEnabled",false);
}

function disable_all(target){
  var boolVal = target.checked;
  if(boolVal) set_disabled(boolVal);
...
function set_disabled(boolVal=undefined,main=false){
  var disabled = $("#disable-all-rules",context);
...
function filter_devices(deviceList,idList){
  if(idList.length == 0) { return ["<%= i18n.allDevices %>"]; }
  var ret = [];
...
function format_services(_services,serviceList){
  if (is_all_traffic(serviceList)) { return {targets: ["<%= i18n.allTraffic %>"], reject: true}; }

  var ret = [];
  var reject = false;
  for(var i = 0;i<serviceList.length;++i){
    var s = serviceList[i];
    var proto = s.hasOwnProperty('proto') ? (s.proto == 6 ? "<%= i18n.TCP %>" : "<%= i18n.UDP %>") : "<%= i18n.port %>";
    ...
    if(s.hasOwnProperty('appid') || (sport && dport)){
      ...
      if(sport){
        ret.push("<%= i18n.source %> " + proto + " " + s.sport[0] + "-" + s.sport[1]);
      }else if(dport){
        ret.push("<%= i18n.dest %> " + proto + " " + s.dport[0] + "-" + s.dport[1]);
      }else{
        ret.push("<%= i18n.unknown %>")
      }
    }
  }
...
function init(){
  $("#firewall-drag",context)[0].drop = function(el, target, source, sibling){
    var jt = $(el);
    var id = parseInt(jt.attr("row_id"));
    switchRules(id,jt.index()+1)
  }
  $("#firewall-drag",context).on("drag-enabled-changed",function(e){
    var dragEnabled = e.detail.value;
    if(dragEnabled){
      $("#firewall-drag .drag_disabled",context).removeClass("drag_disabled");
    }else{
      $("#firewall-drag .drag_handle",context).addClass("drag_disabled");
    }
  });
  refresh_main_disabled();
  ...
  $("#refresh-rules", context).on("tap",function () {
    refreshRules();
  });
  refreshRules();
}
