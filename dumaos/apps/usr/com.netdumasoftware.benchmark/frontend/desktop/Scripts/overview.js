...
(function (context)
{
  ...
  class TestProcessor
  {
    ...
    StartTest()
    {
      $("#runTestButton", context)[0].disabled = true;
      ...
      newFetchedWebSocket(packageId, "benchmark").then((socket) => {
        ...
        $("#runTestButton", context)[0].disabled = false;
    ...
    SendResult(message)
    {
      ...
      switch (data[0])
      {
        ...
        case 0:
          switch (data[1])
          {
            ...
            case 0:
              ...
              this.graphMan.PushResult(data[3], this.uploadSpeed * 1000, graphTargets.SetUpload, 0);
              return [0, false];
            ...
            case 2:
              ...
              this.graphMan.PushResult(data[3], this.downloadSpeed * 1000, graphTargets.SetDownload, 0);
              return [0, false];
        ...
        case 8:
          var info = JSON.parse(stringData[1]);
          if (info.version != "2.0.0")
            return [-1, false];
        
          $(graphConfigs[0].htmlTag, context)[0].chart.config.options.scales.xAxes[0].ticks.max = info.speed_length * 1000;
          $(graphConfigs[1].htmlTag, context)[0].chart.config.options.scales.xAxes[0].ticks.max = info.ping_length * 1000;
          $(graphConfigs[2].htmlTag, context)[0].chart.config.options.scales.xAxes[0].ticks.max = info.bbloat_length * 1000;
          this.testStartTime = info.timestamp;
    ...
    SetLoading(value)
    {
      $("#runTestButton", context)[0].disabled = value;
