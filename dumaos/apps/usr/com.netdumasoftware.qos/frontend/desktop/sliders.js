...
function bindBandwidthChange() {
  ...
  <% if platform.vendor == "TELSTRA" then %>
  var upSlider = $("#upload-slider",context);
  var downSlider = $("#download-slider",context);
  upSlider[0].filterMarkers = function(index) {
    return index === this.secondaryProgress;
  };
  downSlider[0].filterMarkers = function(index) {
    return index === this.secondaryProgress;
  };
  setTimeout(function(){
    Polymer.dom(upSlider.find(".slider-marker[display]")[0]).appendChild($('<span class="secondary-label">Minimum Upload Limit</span>')[0]);
    Polymer.dom(downSlider.find(".slider-marker[display]")[0]).appendChild($('<span class="secondary-label">Minimum Download Limit</span>')[0]);
  },100);
  <% end %>
}
