...
function generateToggle(device, enabled, id, allowFiltering) {
  return $("<paper-toggle-button><%= i18n.filteringMode %></paper-toggle-button>")
...
function do_add_service( device, id, name, enabled, allowFiltering ) {
  var geodevice = $("<div></div>")
    ...
    //   .append($("<div class='service'></div>").text(name))
      .append($("<div class='delete-box'></div>").append(
        // $("<div class='buttons'></div>").append(
          $('<paper-button><%= i18n.delete %></paper-button>').click(
          (function( device, id ){
            return function () {
              $("#geofilter-duma-alert", context)[0].show(
                ...
                [
                  ...
                  { text: "<%= i18n.delete %>", action: "confirm", callback: function(){
                    delete_device(device,id,function(){ $(geodevice).remove(); });
                  }}
                ]
              )
            }
...
function add_services( device, service, filter_mode, devices){
  ...
  if(filter_mode === true || filter_mode === false){
    promise.done(function(id){
      long_rpc_promise(geoFilter.getPackageId(), "capture_toggle", [1, device.id, id[0], JSON.stringify(filter_mode)]).done(function(){
        after(id);
      });
    });
  }else{
...
function on_add_device( ){
  get_devices().done(function ( d ){
    function add_device_callback( device, service, filter){
      add_services( device, service, filter, d);
    }
