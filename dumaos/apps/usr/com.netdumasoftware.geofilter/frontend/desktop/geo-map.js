...
function attemptStartPolygonMode(){
  ...
  var doneOrigTour = duma.storage(geoFilter.getPackageId(),"newtour");
  if(doneTour !== "true" && doneOrigTour === "true"){
...
function doOpenTourStart(){
  ...
  tourStartDiag[0].open();
  tourStartDiag.on("iron-overlay-closed",function(){
    duma.storage(geoFilter.getPackageId(),"newtour","true");
  });
  tourStartDiag.find("#start-tour").on("click",function(){
    hopscotch.startTour(duma.tour.getTour()(0), 0);
    tourStartDiag[0].close();
  });
}

function openTourStarter(){
  var tourState = duma.storage(geoFilter.getPackageId(), "newtour");
  if(tourState !== "true"){
    doOpenTourStart();
  }
}
