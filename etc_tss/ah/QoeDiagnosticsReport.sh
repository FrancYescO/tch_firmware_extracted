...
Device.X_ADB_QoE.MPDUDiagnostics.Report)
	...
	MULTIPLE_REPORT=0
	;;
Device.X_ADB_QoE.WebDiagnostics.Report)
	TIME_OBJ="Report_QoE_Web"
	BASE_OBJ="Device.X_ADB_QoE.WebDiagnostics"
	DIAG_OBJ="Device.IP.Diagnostics.X_ADB_WebDiagnostics"
	FILE_REPORT="/tmp/QoE_Web_report.xml"
	DIAG_TYPE="Web"
	MULTIPLE_REPORT=0
	;;
...
Device.IP.Diagnostics.UDPEchoDiagnostics Device.DNS.Diagnostics.NSLookupDiagnostics Device.IP.Diagnostics.X_ADB_YoutubeDiagnostics.[Alias=Immediate] \
Device.IP.Diagnostics.X_ADB_WebDiagnostics.[Alias=Immediate]"
	DIAG_OBJ=""
	FILE_REPORT="/tmp/QoE_ipdiagnostics_report.xml"
	DIAG_TYPE="IPPing TraceRoute Download Upload"
	MULTIPLE_REPORT=1
	;;
Device.X_ADB_QoE.Scheduler.Report)
	TIME_OBJ=""
	BASE_OBJ="Device.X_ADB_QoE.IPPingDiagnostics Device.X_ADB_QoE.TraceRouteDiagnostics Device.X_ADB_QoE.DownloadDiagnostics Device.X_ADB_QoE.UploadDiagnostics \
Device.X_ADB_QoE.NSLookupDiagnostics Device.X_ADB_QoE.UDPEchoDiagnostics Device.X_ADB_QoE.SamplingDiagnostics Device.X_ADB_QoE.AssociatedDevicesDiagnostics \
Device.X_ADB_QoE.YoutubeDiagnostics Device.X_ADB_QoE.WebDiagnostics"
	DIAG_OBJ="Device.IP.Diagnostics.X_ADB_IPPing Device.IP.Diagnostics.X_ADB_TraceRoute Device.IP.Diagnostics.X_ADB_DownloadDiagnostics Device.IP.Diagnostics.X_ADB_UploadDiagnostics \
Device.DNS.Diagnostics.X_ADB_NSLookup Device.IP.Diagnostics.X_ADB_UDPEchoDiagnostics Device.X_ADB_QoE.Sampling Device.WiFi.X_ADB_AssociatedDevicesDiagnostics \
Device.IP.Diagnostics.X_ADB_YoutubeDiagnostics Device.IP.Diagnostics.X_ADB_WebDiagnostics"
	FILE_REPORT="/tmp/QoE_SchduledDiagnostics_report.xml"
	DIAG_TYPE="IPPing TraceRoute Download Upload"
	MULTIPLE_REPORT=2
	;;
*)
...
print_xml_device_info()
{
	...
	print_xml_param AgentInfo AgentSoftwareVersion 2 xml
...
print_xml_test_info()
{
	...
	*MPDUDiagnostics)
		...
		;;
	*YoutubeDiagnostics*)
		params="$params URL ProtocolVersion BufferSize \
			MinTime MaxTime BitRate OneBitRate"
		;;
	*WebDiagnostics*)
		params="$params URL ProtocolVersion"
		;;
	esac
	for p in $params
	do
...
print_xml_test_item()
{
	...
	*WiFiChannelMeasurementsDiagnostics*)
		...
		dtype="WiFiChannelMeasurements"
		;;
	*AssociatedDevicesDiagnostics*)
		starttime=$(cmclient GETV "$dobj.LastScanTime")
		dtype="AssociationDevices"
		;;
	*MPDUDiagnostics*)
		starttime=$(cmclient GETV "$dobj.LastScanTime")
		dtype="MPDU"
	...
	*MPDUDiagnostics*)
		...
		;;
	*TraceRoute*)
		hops=$(cmclient GETO "$dobj.RouteHops")
		for hop in $hops; do
			hop=${hop##${dobj}.}
			print_xml_param "$dobj" "$hop.ErrorCode" 5
			print_xml_param "$dobj" "$hop.Host" 5
			print_xml_param "$dobj" "$hop.HostAddress" 5
			print_xml_param "$dobj" "$hop.RTTimes" 5
		done
		;;
	*Youtube*)
		print_xml_param "$dobj" Stalls 5
		print_xml_param "$dobj" AverageStallDuration 5
		print_xml_param "$dobj" TotalStallDuration 5
		print_xml_param "$dobj" MaxBitRate 5
		print_xml_param "$dobj" MinBitRate 5
		print_xml_param "$dobj" AdvertisedBitRate 5
		print_xml_param "$dobj" PlaybackTime 5
		print_xml_param "$dobj" ITag 5
		;;
	*Web*)
		print_xml_param "$dobj" LoadTime 5
		print_xml_param "$dobj" TCPOpenTime 5
		print_xml_param "$dobj" DnsTime 5
		;;
	esac
	echo "        </ParamList>" >> $FILE_REPORT
	echo "      </ItemData>" >> $FILE_REPORT
...
qoe_tests_execute()
{
	local test diagState testPath schedulerOngoing
	#wait for current single scheduled test finish
	while true; do
		schedulerOngoing=$(cmclient GETV Device.X_ADB_QoE.Scheduler.TestOngoing)
		[ "$schedulerOngoing" = "false" ] && break
		sleep 1;
	done
	IFS=","
	for test in $newTestList; do
		...
		case $test in
			"NSLookupDiagnostics")
				testPath="Device.DNS.Diagnostics"
			;;
			*)
				testPath="Device.IP.Diagnostics"
			;;
		esac
		cmclient SET "${testPath}.${test}.DiagnosticsState" "Requested"
		diagState="Requested"
		while [ $diagState = "Requested" ]; do
			sleep 1
			diagState=`cmclient GETV "${testPath}.${test}.DiagnosticsState"`
		done
	done
...
qoe_report_send()
{
	...
	if [ $MULTIPLE_REPORT -eq 1 ]; then
		...
		for DIAG_OBJ in $DIAG_OBJ; do
			_dobjs="$_dobjs $(cmclient GETO "$DIAG_OBJ.[Alias>QoeTest_].[DiagnosticsState!Requested].[DiagnosticsState!None]")"
		done
		if [ -z "$_dobjs" ]; then
			#nothing to send
			rm -f $FILE_REPORT
	...
	if [ "$res" = 0 ]; then
		...
		elif [ $MULTIPLE_REPORT -eq 2 ]; then
			count=0
		else
			count=0
			for dobj in $_dobjs
