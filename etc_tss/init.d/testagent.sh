...
start() {
  ...
  [ ! -d /tmp/eh ] && mkdir /tmp/eh
  [ ! -d /tmp/ec_time ] && mkdir /tmp/ec_time
  ...
  #run scheduler timers
  i=1
  while [ $i -lt 8 ]; do
    min=$(cmclient GETV "Device.X_ADB_Time.Event.[Alias>Scheduler_${i}].OccurrenceMinutes")
    cmclient SET "Device.X_ADB_Time.Event.[Alias>Scheduler_${i}].OccurrenceMinutes" "" >> /dev/null
    cmclient SET "Device.X_ADB_Time.Event.[Alias>Scheduler_${i}].OccurrenceMinutes" "$min" >> /dev/null
    i=$((i+1))
  done
